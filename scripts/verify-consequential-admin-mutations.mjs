  ...
  for (const absolutePath of routerFiles(routersRoot)) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      discovered.push({
        ...
        callsTwoPersonRail:
          /\bOperatorAuthorityService\.(?:request|approve|reject)\s*\(/u.test(block) ||
          /\bUniversalV1OpsCaseService\.decideTransition\s*\(/u.test(block) ||
          /\brecordUniversalV1RecoveryApproval\s*\(/u.test(block),
