...
| `routers/quotePayment.ts` | legacy quote payment/materialization entry | `TOMBSTONED`; preserve exact read-only replay and persisted-provider orphan recovery only. |
...
| Quote payment creation/finalization | `quotePayment.ts` preserves the historical route shape but deterministically tombstones create and test-confirm in every environment. `QuotePaymentFinalizationService.ts` permits only exact recovery-aware read-only replay of a previously materialized task/escrow. | The retired pay-first design treated a succeeded PaymentIntent as authority to create a task and fund legacy escrow, outside the Universal V1 operation/FSE chain. | `TOMBSTONED` for positive effects; `KEEP` the bounded persisted-provider orphan recovery lane; new synthetic effects use the Universal V1 `finance` boundary. |
...
| Engine bridge workload authority | The reusable bridge header/env credential has been removed from request authentication; all former bridge mutations are terminal-held in source, and the one preserved lifecycle read requires a named platform administrator. | The preserved handlers still lack the certified typed workload-command envelope and therefore remain deliberately unreachable. | `FROZEN`; `MIGRATE` only required behavior to short-lived named service identities, closed per-command capabilities, expected versions, idempotency, and immutable audit; human-admin equivalence is deleted. |
...
The legacy escrow `PaymentProvider` still exposes `createPaymentIntent` and `verifySucceededPayment`. Quote creation and verification were removed from the active quote contract: `LegacyQuotePaymentRecoveryPort` exposes only bounded negative-effect orphan recovery selected from the exact persisted provider. This remains compatibility cleanup, not Universal V1 financial authority.
