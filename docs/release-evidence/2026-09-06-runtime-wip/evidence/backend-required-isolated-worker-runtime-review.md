...
Completed 2026-09-06T21:37:01.362Z. Exit 1. **11043 passed, 19 failed, 15 skipped, 0 todo across 698 files. Gate FAIL.**

The isolated source snapshot is unchanged. This run includes all fifteen reviewed readiness, preparation, incident-guard and worker-runtime source changes copied from the primary tree. Later primary changes, if any, are excluded and require separate integration evidence. No commit, push, deployment or production effect is implied.
...
Exact JSON report: [archived report](backend-required-isolated-worker-runtime-vitest.json). Full log: [archived log](backend-required-isolated-worker-runtime.log). Failure register and hashes: [receipt](backend-required-isolated-worker-runtime-failures.json).
