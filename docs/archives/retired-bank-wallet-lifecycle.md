# Retired bank-wallet lifecycle test

Historical evidence only. This single end-to-end scenario requires the removed payment-event worker and wallet/cash-out service. It is not an executable test or a supported runtime contract. Retained local-test payment/completion behavior is covered by the current focused payment and completion tests. A new current-architecture PostgreSQL lifecycle test is still needed when disposable infrastructure is available.
