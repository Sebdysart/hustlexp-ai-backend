...
Keep the Supabase website backend contained while `D1_CI_INCIDENT_RECOVERY_20260825` closes payment creation, baseline CI, immutable evidence, and recovery boundaries. Freeze new Supabase canonical task/payment/assignment behavior; inventory and quarantine existing bypasses. This D1 work is containment, not the task-first fake-FSE vertical and not `/OPS` convergence. After its exact tree, migration lineage, and release controls are independently accepted, later authorized replacement slices must route typed commands through the engine with runtime wiring, authorization, observability, migration proof, and rollback/forward-repair. Lead ingress and lifecycle automation remain late cutover work because moving intake before dependent automation would silently drop or duplicate effects.
