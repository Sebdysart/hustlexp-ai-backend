...
export async function orchestrate(input: OrchestrateInput): Promise<OrchestrateResponse> {
    ...
    try {
        // SECURITY: Check for prompt injection before any processing
        const injectionAnalysis = PromptInjectionGuard.analyze(input.message);

        if (injectionAnalysis.decision === 'BLOCK') {
            aiLogger.warn({
                userId: input.userId,
                score: injectionAnalysis.score,
                matchedPatterns: injectionAnalysis.matchedPatterns,
        ...
        // If flagged, use sanitized input for downstream processing
        if (injectionAnalysis.decision === 'FLAG' && injectionAnalysis.sanitizedInput) {
            input = { ...input, message: injectionAnalysis.sanitizedInput };
        }
 ...
 */
async function handleCreateTask(input: OrchestrateInput): Promise<OrchestrateResponse> {
    try {
...
Extract and structure this into a task.`,
        ...
        const taskDraft = JSON.parse(composerResult.content) as TaskDraft;
 ...
 */
async function handleSearchTasks(input: OrchestrateInput): Promise<OrchestrateResponse> {
    try {
 ...
 */
async function handleAskPricing(input: OrchestrateInput): Promise<OrchestrateResponse> {
    try {
 ...
 */
async function handleHustlerPlan(input: OrchestrateInput): Promise<OrchestrateResponse> {
    try {
 ...
 */
async function handleSupport(input: OrchestrateInput): Promise<OrchestrateResponse> {
    try {
