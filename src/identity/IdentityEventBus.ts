...
export class IdentityEventBus {
     ...
     */
    static async emit(event: IdentityEvent): Promise<void> {
        serviceLogger.info({
            type: event.type,
            userId: event.userId
    ...
    private static async persistEvent(event: IdentityEvent): Promise<void> {
        if (!sql) return;
    ...
    private static async handleEvent(event: IdentityEvent): Promise<void> {
        if (!sql) return;
