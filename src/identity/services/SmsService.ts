 ...
 */
export async function sendVerificationSms(phone: string): Promise<SmsSendResult> {
    if (!client) {
    ...
    try {
        ...
        if (!TWILIO_VERIFY_SERVICE_SID) throw new Error('TWILIO_VERIFY_SERVICE_SID missing');
 ...
 */
export async function checkVerificationSms(phone: string, code: string): Promise<SmsVerifyResult> {
    if (!client) {
