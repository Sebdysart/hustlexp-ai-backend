...
import { CorrectionEngine, Correction, CorrectionReason, CorrectionType } from './CorrectionEngine.js';
...
export class TaskRoutingCorrection {
    ...
    private static readonly MAX_MAGNITUDE = 1.0;
    private static readonly MAX_EXPIRY_HOURS = 6;
    private static readonly MIN_REBOOSTING_HOURS = 6;

    static async apply(params: TaskRoutingParams): Promise<{ success: boolean; correctionId: string | null; error?: string }> {
    ...
    private static async wasRecentlyBoosted(taskId: string): Promise<boolean> {
        const db = getDb();
        if (!db) return false;
...
interface FrictionParams {
    ...
    reason: CorrectionReason;
    expiresAt: Date; // REQUIRED, no defaults
    triggeredBy: string;
}

export class FrictionCorrection {

    private static readonly MAX_EXPIRY_HOURS = 12;

    static async apply(params: FrictionParams): Promise<{ success: boolean; correctionId: string | null; error?: string }> {
...
export class SupplyNudgeCorrection {
    ...
    private static readonly MAX_PER_USER_PER_DAY = 3;
    private static readonly ZONE_DAILY_CAP = 50;
    private static readonly LOW_OPEN_RATE_THRESHOLD = 0.15;
    private static readonly SUPPRESS_HOURS = 24;

    static async apply(params: SupplyNudgeParams): Promise<{
        success: boolean;
    ...
    private static async getZoneNudgeCount(zone: string): Promise<number> {
        const db = getDb();
        if (!db) return 0;
    ...
    private static async isZoneSuppressed(zone: string): Promise<boolean> {
        // TODO: Implement open rate tracking
        // For now, never suppressed
        return false;
    }

    private static async filterEligibleHustlers(hustlerIds: string[]): Promise<string[]> {
        const db = getDb();
        if (!db) return hustlerIds.slice(0, 10); // Default limit
    ...
    private static async recordNudgesSent(hustlerIds: string[]): Promise<void> {
        // Tracking is implicit in correction_log
    }
}

// ============================================================
// PROOF TIMING CORRECTION
// ============================================================
...
export class ProofTimingCorrection {
    ...
    private static readonly MIN_DEADLINE_HOURS = 4;
    private static readonly MAX_DEADLINE_HOURS = 48;
    private static readonly MAX_ADJUSTMENTS_PER_TASK = 1;

    static async apply(params: ProofTimingParams): Promise<{ success: boolean; correctionId: string | null; error?: string }> {
        ...
        const boundedDeadline = Math.min(
            Math.max(adjustedDeadlineHours, this.MIN_DEADLINE_HOURS),
            this.MAX_DEADLINE_HOURS
        );
    ...
    private static async getAdjustmentCount(taskId: string): Promise<number> {
        const db = getDb();
        if (!db) return 0;
...
export class PricingGuidanceCorrection {
    ...
    private static readonly MAX_MULTIPLIER = 1.5;
    private static readonly MAX_DELTA_PER_24H = 0.25;

    static async apply(params: PricingGuidanceParams): Promise<{ success: boolean; correctionId: string | null; error?: string }> {
        const { category, zone, confidenceMultiplier, reason, triggeredBy } = params;

        // Bound multiplier
        const boundedMultiplier = Math.min(
            Math.max(confidenceMultiplier, this.MIN_MULTIPLIER),
            this.MAX_MULTIPLIER
        );
    ...
    private static async getRecentDelta(category: string, zone: string): Promise<number> {
        const db = getDb();
        if (!db) return 0;
