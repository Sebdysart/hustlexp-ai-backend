...
interface AccountDefinition {
    ...
    prefix: string; // Used for name generation
    scope: 'platform' | 'user' | 'task';
}

const TEMPLATE_DEFINITIONS: Record<LedgerAccountTemplate, AccountDefinition> = {
    // Platform
...
export class LedgerAccountService {
    ...
    static async getAccount(
        ownerId: string,
        template: LedgerAccountTemplate,
        client: any = sql // specific transaction client
    ): Promise<LedgerAccount> {
        const def = TEMPLATE_DEFINITIONS[template];
        if (!def) throw new Error(`[LedgerAccount] Invalid template: ${template}`);
        ...
        const [existing] = await client<LedgerAccount[]>`
        ...
        const [account] = await client<LedgerAccount[]>`
