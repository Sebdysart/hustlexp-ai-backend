...
class StripeServiceClass {
    ...
    async createConnectAccount(userId: string, email: string, metadata?: { name?: string; phone?: string }): Promise<ConnectAccountResult> {
        ...
        try {
            const existingAccountId = await this.getConnectAccountId(userId);
    ...
    async getAccountStatus(userId: string): Promise<AccountStatus | null> {
        ...
        const accountId = await this.getConnectAccountId(userId);
    ...
    async getConnectAccountId(userId: string): Promise<string | undefined> {
        ...
        if (sql) {
            try {
                ...
                serviceLogger.error({ error, userId }, 'Failed to fetch Connect account ID from database');
            }
        }
        return undefined;
    }
    ...
    async setConnectAccountId(userId: string, accountId: string): Promise<void> {
        ...
        if (sql) {
            try {
                ...
                serviceLogger.error({ error, userId, accountId }, 'Failed to persist Connect account ID to database');
            }
        }
    }
