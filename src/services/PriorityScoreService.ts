 ...
 */
function calculateXPComponent(totalXP: number): number {
 ...
 */
function calculateTrustComponent(trustTier: number): number {
 ...
 */
function calculateStreakComponent(currentStreak: number): number {
...
class PriorityScoreServiceClass {
   ...
   */
  async calculateScore(userId: string): Promise<PriorityScore> {
    const sql = getSql();
    ...
    if (hoursSinceLastTask > DECAY_GRACE_HOURS) {
      ...
      decayedScore = Math.max(
        baseScore * DECAY_FLOOR_PERCENT,
        baseScore * Math.pow(1 - DECAY_RATE_DAILY, daysIdle)
      );
    }
   ...
   */
  async getFeedbackModifiers(userId: string): Promise<{ penalties: number; bonuses: number }> {
    const sql = getSql();
   ...
   */
  async compareUsers(userAId: string, userBId: string): Promise<number> {
   ...
   */
  async recordTaskCompletion(userId: string): Promise<{ restored: boolean; newScore: number }> {
    const sql = getSql();
