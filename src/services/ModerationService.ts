...
class ModerationServiceClass {
     ...
     */
    async fastCheck(content: string): Promise<ModerationResult> {
        try {
     ...
     */
    async deepCheck(content: string, context?: string): Promise<ModerationResult> {
        try {
            const result = await routedGenerate('safety', {
...
Provide your final decision.`
     ...
     */
    async check(content: string, context?: string): Promise<ModerationResult> {
        // First, do fast check
