 ...
 */
const CHAINING_RULES: Record<AIAction, AIAction[]> = {
...
class AIChainingGuardClass {
  private recentActions: Map<string, AIActionRecord> = new Map();
  private violations: ChainingViolation[] = [];
  private readonly windowMs = 60000; // 1 minute window for chaining detection
  ...
  canProceed(
    ...
    targetAction: AIAction,
    _requestId?: string
  ): { allowed: boolean; reason?: string; violation?: ChainingViolation } {
    ...
    for (const record of unreviewedActions) {
      ...
      if (blockedTargets.includes(targetAction)) {
        const violation: ChainingViolation = {
   ...
   */
  private getUnreviewedActions(userId: string): AIActionRecord[] {
    const cutoff = Date.now() - this.windowMs;
    const results: AIActionRecord[] = [];
  ...
  private pruneOldRecords(): void {
    ...
    this.violations = this.violations.filter(
      v => v.timestamp.getTime() > violationCutoff
    );
  }
   ...
   */
  getViolations(limit: number = 100): ChainingViolation[] {
 ...
 */
export function enforceNoChaining(actionType: AIAction) {
  return function (
    _target: unknown,
    _propertyKey: string,
    ...
    descriptor.value = async function (...args: unknown[]) {
      ...
      const context = args[0] as { userId?: string; requestId?: string } | undefined;
