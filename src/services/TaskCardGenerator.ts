...
import {
    ...
    SEATTLE_CONTEXT_PROMPT,
    GAMIFICATION_PROMPT,
} from '../ai/prompts/taskCard.js';
...
const SEATTLE_HOTSPOTS: Record<string, { baseSurge: number; peakHours: number[]; peakDays: number[] }> = {
...
class TaskCardGeneratorClass {
    ...
    async generateCard(input: {
        ...
        const seattle = this.calculateSeattleContext(
            enriched.category,
        ...
        const priorityTags = this.generatePriorityTags(
            enriched,
            seattle,
            gamification,
            input.userLevel || 1
    ...
    private async enrichTask(
        ...
        categoryHint: TaskCategory | undefined,
        now: Date,
        dayOfWeek: string
    ): Promise<{
        title: string;
        ...
        safetyNotes: string;
        experienceLevel: ExperienceLevel;
        priceBreakdown: { min: number; recommended: number; max: number; hourlyEquivalent: number };
    }> {
    ...
    private calculatePrice(
        base: { min: number; recommended: number; max: number; hourlyEquivalent: number },
        surgeFactor: number
    ): PriceBreakdown {
    ...
    private calculateGamification(
        ...
        if (userCategoryCount === 0) potentialBadges.push('First Timer');
        if (userCategoryCount + 1 === 10) potentialBadges.push(CATEGORY_BADGES[category]);
        if (difficulty === 'hard') potentialBadges.push('Challenge Accepted');
    ...
    private generatePriorityTags(
        enriched: { difficulty: Difficulty; experienceLevel: ExperienceLevel; durationMinutes: number; priceBreakdown: { recommended: number } },
        seattle: SeattleContext,
        gamification: GamificationData,
        userLevel: number
    ): PriorityTags {
    ...
    private calculateVisualHints(
        enriched: { difficulty: Difficulty; category: TaskCategory },
        seattle: SeattleContext,
        gamification: GamificationData
    ): EnrichedTaskCard['visualHints'] {
        ...
        // Badges to show
        const badges: string[] = [];
        if (seattle.surgePercent > 0) badges.push(`+${seattle.surgePercent}% Surge`);
        if (gamification.doubleXPEligible) badges.push('2x XP');
        if (seattle.hotspotBonus) badges.push('🔥 Hot');

        // Animations
        const animations: string[] = ['hover-glow'];
        if (seattle.surgeFactor > 1.1) animations.push('pulse');
        if (gamification.streakMultiplier > 1) animations.push('fire');

        // Urgency
        let urgencyLevel: 'low' | 'medium' | 'high' = 'low';
        if (seattle.surgeFactor > 1.15) urgencyLevel = 'high';
        else if (seattle.hotspotBonus) urgencyLevel = 'medium';
     ...
     */
    private calculateLevelRequired(difficulty: Difficulty, experience: ExperienceLevel): number {
        if (difficulty === 'easy' && experience === 'none') return 1;
        if (difficulty === 'easy') return 2;
        if (difficulty === 'medium' && experience === 'none') return 2;
        if (difficulty === 'medium') return 4;
        if (difficulty === 'hard' && experience === 'experienced') return 8;
        return 5;
    }
     ...
     */
    private calculateSafetyRating(enriched: {
        difficulty: Difficulty;
        experienceLevel: ExperienceLevel;
        safetyNotes: string;
        category: TaskCategory;
     ...
     */
    private generateFallbackTitle(rawText: string): string {
