...
class TaskOutcomeServiceClass {
     ...
     */
    async recordOutcome(input: RecordOutcomeInput): Promise<TaskOutcome | null> {
     ...
     */
    async getOutcome(taskId: string): Promise<TaskOutcome | null> {
        if (!isDatabaseAvailable() || !sql) {
            return null;
        }
     ...
     */
    async getOutcomesByTPEEEvaluation(tpeeEvalId: string): Promise<TaskOutcome[]> {
        if (!isDatabaseAvailable() || !sql) {
            return [];
        }
     ...
     */
    async getTPEEDecisionQualityReport(): Promise<{
        total_tasks: number;
        ...
        try {
            ...
            const byDecision: Record<string, {
                count: number;
                completed_rate: number;
                disputed_rate: number;
                canceled_rate: number;
     ...
     */
    async getBlockedUserOutcomeAnalysis(): Promise<{
