...
export interface HustlerCandidate {
    ...
    dbMatchScore: number;      // From DB query
    aiMatchScore?: number;     // From AI re-ranking
    ...
    // History
    categoryExperience: number;  // Tasks in this category
    repeatClient?: boolean;      // Has worked with this poster before
    avgResponseTime: string;     // e.g., "< 1 hour"

    // AI insights (filled by re-ranking)
    matchReason?: string;
    aiNotes?: string;
}
...
${JSON.stringify(candidateSummaries, null, 2)}`,
            ...
            for (const ranking of rankings) {
                ...
                if (candidate) {
                    ...
                    candidate.finalScore = Math.round(
                        candidate.dbMatchScore * 0.4 + ranking.score * 0.6
                    );
                }
            }
    ...
    async explainMatch(
        task: TaskMatchContext,
        candidate: HustlerCandidate
    ): Promise<{ explanation: string; strengths: string[]; considerations: string[] }> {
        try {
...
- ${candidate.availableNow ? 'Available now' : 'May be busy'}`,
     ...
     */
    generateTestCandidates(count: number = 10): HustlerCandidate[] {
        const names = ['Alex', 'Jordan', 'Sam', 'Casey', 'Morgan', 'Taylor', 'Riley', 'Jamie', 'Drew', 'Quinn'];
        const categories: TaskCategory[] = ['cleaning', 'moving', 'handyman', 'delivery', 'pet_care'];
