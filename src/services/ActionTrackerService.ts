...
export interface UserActionStats {
    ...
    activeDays: Record<number, number>;   // day of week -> count
...
const actionHistory = new Map<string, TrackedAction[]>();
const actionStats = new Map<string, UserActionStats>();
...
class ActionTrackerServiceClass {
     ...
     */
    getRecentActions(userId: string, limit: number = RECENT_ACTIONS_DEFAULT): TrackedAction[] {
        const history = actionHistory.get(userId) || [];
     ...
     */
    getRecentActionsForAI(userId: string, limit: number = 5): string[] {
     ...
     */
    private initializeStats(userId: string): UserActionStats {
        return {
            userId,
            tasksViewed: 0,
            tasksAccepted: 0,
            ...
            tasksCompleted: 0,
            tasksCancelled: 0,
            categoryStats: {} as Record<TaskCategory, any>,
            activeHours: {},
            activeDays: {},
            avgAcceptedPrice: 0,
     ...
     */
    private updateStats(userId: string, action: TrackedAction): void {
    ...
    private updateCategoryStats(
        ...
        type: 'viewed' | 'accepted' | 'skipped' | 'completed'
    ): void {
        if (!category) return;
     ...
     * Update price stats
     */
    private updatePriceStats(stats: UserActionStats, price: number): void {
    ...
    analyzePatterns(userId: string): {
        ...
        const preferredCategories: TaskCategory[] = [];
        const avoidedCategories: TaskCategory[] = [];
        ...
        for (const [category, catStats] of Object.entries(stats.categoryStats)) {
            if (catStats.viewed >= 3) {  // Need enough data
                if (catStats.acceptanceRate >= 0.5) {
                    preferredCategories.push(category as TaskCategory);
                } else if (catStats.acceptanceRate < 0.2) {
                    avoidedCategories.push(category as TaskCategory);
                }
            }
        }
