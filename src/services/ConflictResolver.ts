...
export type AuthorityLayer = 
  | 'database'      // Layer 0: PostgreSQL constraints
  | 'state_machine' // Layer 1: Backend state machines
  | 'temporal'      // Layer 2: Time-based rules
  | 'payments'      // Layer 3: Stripe
  | 'ai'            // Layer 4: AI proposal
  | 'ui_state'      // Layer 5: UI state machines
  | 'client';       // Layer 6: Client rendering
...
const LAYER_PRIORITY: Record<AuthorityLayer, number> = {
  database: 0,
  state_machine: 1,
  temporal: 2,
  payments: 3,
...
class ConflictResolverClass {
  private violations: ConflictEvent[] = [];
  ...
  resolve(
    action: string,
    attemptedBy: AuthorityLayer,
    rejectedBy: AuthorityLayer,
    context: {
      entityType: string;
      entityId: string;
      reason: string;
    ...
    if (attemptPriority < rejectPriority) {
      ...
      const violation = this.recordViolation(
        ...
        context.entityType,
        context.entityId,
        `INVALID: Lower layer ${rejectedBy} rejected higher layer ${attemptedBy}`,
        context.metadata || {}
      );
    ...
    const violation = this.recordViolation(
      ...
      context.entityType,
      context.entityId,
      `Same-layer conflict: ${context.reason}`,
      context.metadata || {}
    );
   ...
   */
  getViolations(limit: number = 100): ConflictEvent[] {
