...
export class AdminServiceClass {
     ...
     */
    async overrideTaskState(input: AdminOverrideInput): Promise<{ success: boolean; message: string }> {
        ...
        try {
            await transaction(async (tx) => {
                ...
                if (action === 'force_refund') {
                    ...
                    if (!task.assigned_hustler_id) throw new Error('Cannot force payout: No hustler assigned');
