...
import {
    ...
    ONBOARDING_PROFILE_BUILDER_PROMPT,
    MONEY_PATH_PROMPT,
    FIRST_QUEST_PROMPT,
} from '../ai/prompts/onboarding.js';
...
export interface OnboardingSession {
    ...
    role: UserRole | null;
    step: number;
    totalSteps: number;  // Dynamic: 3 for clients, 5 for hustlers
...
const referralCodes = new Map<string, ReferralRecord>();
const userReferrals = new Map<string, string>(); // userId -> referralCode

// ============================================
// Verification Levels
// ============================================

const VERIFICATION_LEVELS: Record<VerificationLevel, { name: string; xpRequired: number; perks: string[] }> = {
...
class OnboardingServiceClass {
     ...
     */
    async startOnboarding(userId: string, referralCode?: string): Promise<OnboardingResponse> {
        ...
        const animations: AnimationType[] = ['confetti', 'sparkle'];
        if (referralBonus > 0) animations.push('glow');
     ...
     */
    private async resumeOnboarding(session: OnboardingSession): Promise<OnboardingResponse> {
        const animations: AnimationType[] = ['pulse'];
     ...
     */
    async chooseRole(sessionId: string, role: UserRole): Promise<OnboardingResponse> {
    ...
    async answerQuestion(
        ...
        const questionData = this.getQuestionData(session);
        const animations: AnimationType[] = skip ? ['pulse'] : ['sparkle'];
     ...
     */
    private getQuestionData(session: OnboardingSession): {
        questionKey?: string;
        question: string;
        options: string[];
     ...
     */
    private async completeOnboarding(session: OnboardingSession): Promise<OnboardingResponse> {
        ...
        if (session.verificationLevel > 0) animations.push('glow');
     ...
     */
    private async buildProfile(session: OnboardingSession): Promise<Record<string, unknown>> {
     ...
     */
    private async generateMoneyPath(profile: Record<string, unknown>): Promise<Record<string, unknown>> {
        const now = new Date();
     ...
     */
    private async generateFirstQuest(session: OnboardingSession): Promise<Record<string, unknown>> {
     ...
     */
    private async getInstantRecommendations(
        userId: string,
        profile: Record<string, unknown>
    ): Promise<Record<string, unknown>[]> {
        const skills = (profile.suggestedCategories || profile.skills || ['errands']) as string[];
     ...
     */
    getReferralStats(userId: string): { code: string; uses: number; xpEarned: number } | null {
     ...
     * Calculate verification level based on XP
     */
    private calculateVerificationLevel(xp: number): VerificationLevel {
        for (let level = 5; level >= 0; level--) {
            if (xp >= VERIFICATION_LEVELS[level as VerificationLevel].xpRequired) {
     ...
     */
    private getNextVerificationXP(currentLevel: VerificationLevel): number | null {
        if (currentLevel >= 5) return null;
        return VERIFICATION_LEVELS[(currentLevel + 1) as VerificationLevel].xpRequired;
    }
    ...
    // Session management
    private saveSession(session: OnboardingSession): void {
        sessionsMemory.set(session.sessionId, session);
        serviceLogger.debug({ sessionId: session.sessionId }, 'Session saved');
    }

    private getSession(sessionId: string): OnboardingSession | null {
