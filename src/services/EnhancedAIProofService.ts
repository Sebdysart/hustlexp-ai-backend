...
const CATEGORY_PROOF_REQUIREMENTS: Record<TaskCategory, ProofRequirement[]> = {
    cleaning: [
...
const proofWorkflows = new Map<string, ProofWorkflow>();
...
class EnhancedAIProofServiceClass {
     ...
     */
    async generateCaption(category: TaskCategory, phase: ProofPhase): Promise<string> {
        try {
...
Category: ${category}, Phase: ${phase}`,
            ...
            // Fallback captions
            const fallbacks: Record<ProofPhase, string> = {
     ...
     */
    async verifyConsistency(taskId: string): Promise<VerificationResult> {
        const workflow = proofWorkflows.get(taskId);
        if (!workflow) {
            return {
                isValid: false,
     ...
     */
    private updateCompletionStatus(workflow: ProofWorkflow): void {
        const requiredPhases = workflow.requirements.filter(r => r.required).map(r => r.phase);
    ...
    getProofInstructions(category: TaskCategory): {
        title: string;
        steps: { phase: ProofPhase; instruction: string; required: boolean }[];
    } {
