...
import type { ProofMetadata, ForensicsResult } from './types.js';
...
export class ProofVerificationService {
    ...
    static async analyzeAndDecide(
        ...
        try {
            ...
            // 3. Run forensics
            const forensicsResult = await ImageForensicsService.analyze(
                fileUrl,
                mimeType,
                metadata,
                timeline
    ...
    static async adminOverride(
        ...
        try {
            const result = await ProofService.finalizeProof(
                submissionId,
