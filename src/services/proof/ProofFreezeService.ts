...
export class ProofFreezeService {
     ...
     */
    static async setFreezeState(taskId: string, state: 'AWAITING_PROOF' | 'PROOF_VERIFIED' | null): Promise<void> {
        const db = getDb();
        if (!db) return;
     ...
     */
    static async isPayoutBlocked(taskId: string): Promise<{ blocked: boolean; reason?: string }> {
        const db = getDb();
     ...
     */
    static async getProofTruth(taskId: string): Promise<{
        ...
        const db = getDb();
        if (!db) return { hasValidProof: false, proofState: null };
        ...
        if (!proof) {
            ...
            return { hasValidProof: false, proofState: request.state as ProofState };
        }

        return {
            hasValidProof: true,
            proofState: proof.state as ProofState,
