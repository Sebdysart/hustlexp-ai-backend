...
export const TIER_TAKE_RATES: Record<TrustTier, number> = {
  1: 0.20,  // 20%
  2: 0.15,  // 15%
  3: 0.12,  // 12%
  4: 0.10,  // 10%
...
// ============================================================================
// TIER REQUIREMENTS (FROM BUILD_GUIDE)
// ============================================================================

interface TierRequirements {
  minCompletedTasks: number;
  maxDisputeRate: number;  // As decimal (0.02 = 2%)
  minAvgRating: number | null;  // null = no requirement
}

const TIER_REQUIREMENTS: Record<TrustTier, TierRequirements> = {
...
const DOWNGRADE_SEVERITY_MAP: Record<DowngradeReason['code'], number> = {
...
class TrustTierServiceClass {
  ...
  async getUserTrustStats(userId: string): Promise<UserTrustStats> {
    ...
    const currentTier = (user.trust_tier || 1) as TrustTier;
    ...
    const avgRating: number | null = null;  // TODO: Implement when rating table exists
    ...
    const eligibleTier = this.calculateEligibleTier(
      completedTasks, 
      disputeRate, 
      avgRating
    );
  ...
  private calculateEligibleTier(
    ...
    for (const tier of [4, 3, 2, 1] as TrustTier[]) {
      const req = TIER_REQUIREMENTS[tier];
      
      if (completedTasks < req.minCompletedTasks) continue;
      if (disputeRate > req.maxDisputeRate) continue;
      if (req.minAvgRating !== null && (avgRating === null || avgRating < req.minAvgRating)) continue;
   ...
   */
  async tryUpgrade(userId: string): Promise<{
    upgraded: boolean;
    oldTier: TrustTier;
    newTier: TrustTier;
    reason: string;
    ...
    const newTier = Math.min(stats.currentTier + 1, stats.eligibleTier) as TrustTier;
  ...
  async downgrade(
    ...
    const oldTier = (user.trust_tier || 1) as TrustTier;
    ...
    // INV-TRUST-7: Floor is Tier 1
    const newTier = Math.max(1, oldTier - tierDrop) as TrustTier;
   ...
   */
  async getTier(userId: string): Promise<TrustTier> {
    const sql = getSql();
    ...
    return (user?.trust_tier || 1) as TrustTier;
  }
  
  /**
   * Get take rate for user's tier
   */
  async getTakeRate(userId: string): Promise<number> {
   ...
   */
  async getTierHistory(userId: string, limit = 20): Promise<Array<{
    oldTier: TrustTier;
    newTier: TrustTier;
    reason: string;
    triggeredBy: string;
    ...
    return rows.map((row: any) => ({
      oldTier: row.old_tier as TrustTier,
      newTier: row.new_tier as TrustTier,
      reason: row.reason,
   ...
   */
  async checkUpgradeAfterCompletion(userId: string): Promise<void> {
    try {
  ...
  async checkDowngrade(
    userId: string,
    trigger: 'dispute_lost' | 'no_show' | 'sla_breach_pattern'
  ): Promise<{ downgraded: boolean; oldTier?: TrustTier; newTier?: TrustTier }> {
    const sql = getSql();
    ...
    try {
      ...
      switch (trigger) {
        case 'dispute_lost':
          ...
          newTier = Math.max(1, currentTier - 1) as TrustTier;
          cooldownDays = 30;
          break;
   ...
   */
  async checkRecovery(userId: string): Promise<{ recovered: boolean; newTier?: TrustTier }> {
    const sql = getSql();
    ...
    try {
      ...
      if (taskStats.completed >= 5 && taskStats.disputed === 0) {
        const currentTier = await this.getTier(userId);
        const recoveryTier = Math.min(4, lastDowngrade.old_tier) as TrustTier;
