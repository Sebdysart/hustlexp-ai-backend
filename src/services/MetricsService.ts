...
class MetricsServiceClass {
    ...
    getZoneHealth(range?: DateRange): ZoneMetrics[] {
        ...
        // Aggregate by zone
        for (const event of events) {
            const zone = (event.metadata?.zone as string) ||
                (event.metadata?.neighborhood as string) ||
                'Greater Seattle';
    ...
    getAIMetricsSummary(range?: DateRange): AIMetricsSummary[] {
        ...
        const grouped: Record<string, AIMetricRecord[]> = {};
    ...
    getOverallStats(): {
        ...
        ai: { totalCalls: number; totalCostUsd: number; avgLatencyMs: number };
        ...
        return {
            events: {
                ...
                byType: eventCounts as Record<string, number>,
            },
            ai: {
