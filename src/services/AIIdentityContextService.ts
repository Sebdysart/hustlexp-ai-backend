...
import { FraudDetectionService, IdentityContext, RiskAssessment } from './FraudDetectionService.js';
...
export interface AIOnboardingContext {
    ...
    verificationAge: string; // "2 hours ago", "just now"
...
function formatVerificationAge(ms: number): string {
...
function getTrustTier(score: number): AIOnboardingContext['trustTier'] {
...
function getSuggestedTone(trustScore: number, riskLevel: string): AIOnboardingContext['suggestedTone'] {
    if (riskLevel === 'critical' || riskLevel === 'high') return 'cautious';
    if (trustScore >= 70) return 'enthusiastic';
    if (trustScore >= 40) return 'friendly';
    return 'neutral';
}
...
class AIIdentityContextServiceClass {
     ...
     */
    async getOnboardingContext(userId: string): Promise<AIOnboardingContext | null> {
     ...
     */
    async getOnboardingPersonalization(userId: string): Promise<OnboardingPersonalization> {
     ...
     */
    async generateAIPromptContext(userId: string): Promise<string> {
        const context = await this.getOnboardingContext(userId);
...
`;
     ...
     */
    async recordOnboardingBehavior(
        userId: string,
    ...
    private getDefaultPersonalization(): OnboardingPersonalization {
        return {
            intro: {
    ...
    private getIntroMessage(context: AIOnboardingContext): string {
    ...
    private getXpMultiplier(trustTier: AIOnboardingContext['trustTier']): number {
        switch (trustTier) {
    ...
    private getAdditionalSteps(context: AIOnboardingContext): string[] {
        const steps: string[] = [];
    ...
    private getBlockedFeatures(context: AIOnboardingContext): string[] {
