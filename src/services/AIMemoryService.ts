...
export interface ExtractedFact {
    ...
    value: string;
    confidence: number;
    source: string;  // The message it came from
    extractedAt: Date;
}
...
export interface AIMemory {
    ...
    // Compressed summary (regenerated periodically)
    summary: string;
...
class AIMemoryServiceClass {
     ...
     */
    private initializeMemory(userId: string): AIMemory {
        return {
            userId,
            conversationHistory: [],
            extractedFacts: [],
    ...
    async addConversation(
        ...
        userMessage: string,
        aiResponse?: string
    ): Promise<{ factsExtracted: number; newSummary: boolean }> {
        ...
        const factsSinceSummary = memory.extractedFacts.filter(
            f => f.extractedAt > memory.summaryGeneratedAt
        ).length;
     ...
     */
    async extractFactsFromMessage(userId: string, message: string): Promise<number> {
        const memory = this.getMemory(userId);
     ...
     */
    async regenerateSummary(userId: string): Promise<string> {
        const memory = this.getMemory(userId);
     ...
     */
    getRecentConversation(userId: string, limit: number = 10): ConversationTurn[] {
        const memory = this.getMemory(userId);
