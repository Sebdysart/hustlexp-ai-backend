...
import type { Task, TaskCategory, HustlerProfile } from '../types/index.js';
...
export interface EarningsProjection {
    daily: { min: number; max: number; likely: number };
    weekly: { min: number; max: number; likely: number };
    monthly: { min: number; max: number; likely: number };
...
interface UserStats {
    ...
    tasksByDay: Record<string, number>; // date -> count
    ...
    // XP
    totalXP: number;
    xpEvents: { date: string; amount: number }[];
...
class AIGrowthCoachServiceClass {
    ...
    private getUserStats(userId: string): UserStats {
        if (!userStats.has(userId)) {
            userStats.set(userId, {
                ...
                totalEarnings: 0,
                earningsByDay: {},
                earningsByCategory: {} as Record<TaskCategory, number>,
                avgTaskDuration: 45,
     ...
     */
    async getGrowthPlan(userId: string): Promise<GrowthPlan> {
        const stats = this.getUserStats(userId);
        const today = new Date().toISOString().split('T')[0];
     ...
     * Calculate level progress
     */
    private calculateLevelProgress(stats: UserStats): LevelProgress {
     ...
     */
    private calculateEarningsProjection(stats: UserStats): EarningsProjection {
        const topCategory = this.getTopCategory(stats);
        ...
        const secondWeekTasks = recentDays
            ...
            .reduce((sum, [, count]) => sum + count, 0);
        const growthTrend: 'up' | 'stable' | 'down' =
            secondWeekTasks > firstWeekTasks * 1.1 ? 'up' :
     ...
     */
    private getEarningsTips(stats: UserStats, trend: 'up' | 'stable' | 'down'): string[] {
        const tips: string[] = [];
     ...
     * Calculate streak info
     */
    private calculateStreakInfo(stats: UserStats): {
        current: number;
        longest: number;
        daysToNextMilestone: number;
        nextMilestoneBonus: number;
        ...
        const bonusMap: Record<number, number> = { 3: 25, 7: 75, 14: 150, 30: 500 };
     ...
     */
    async getNextBestActions(userId: string, stats?: UserStats): Promise<NextBestAction[]> {
     ...
     */
    async getNextBestAction(userId: string): Promise<NextBestAction | null> {
     ...
     */
    async getOptimalTasks(userId: string, limit: number = 5): Promise<Task[]> {
        const stats = this.getUserStats(userId);
        const topCategory = this.getTopCategory(stats);
     ...
     */
    async getCoachingTip(userId: string, stats?: UserStats, context?: string): Promise<CoachingTip> {
        const s = stats || this.getUserStats(userId);
     ...
     */
    async getListingOptimization(task: Task): Promise<string[]> {
    ...
    private getTopCategory(stats: UserStats): TaskCategory {
        const entries = Object.entries(stats.tasksByCategory) as [TaskCategory, number][];
        if (entries.length === 0) return 'errands';
        return entries.sort((a, b) => b[1] - a[1])[0][0];
    }

    private getWeekEarnings(stats: UserStats): number {
        const now = new Date();
    ...
    private getMonthEarnings(stats: UserStats): number {
        const now = new Date();
        return Object.entries(stats.earningsByDay)
            .filter(([date]) => {
                const d = new Date(date);
    ...
    private calculateProfileStrength(stats: UserStats): number {
        let score = 0;
        if (stats.hasPhoto) score += 30;
        if (stats.hasBio) score += 25;
        if (stats.skillsCount >= 3) score += 25;
        if (stats.availabilitySet) score += 20;
        return score;
    }

    private getProfileSuggestions(stats: UserStats): string[] {
        const suggestions: string[] = [];
        if (!stats.hasPhoto) suggestions.push('Add a profile photo to increase trust (+30% matches)');
        if (!stats.hasBio) suggestions.push('Write a bio to stand out from other hustlers');
        if (stats.skillsCount < 3) suggestions.push('Add more skills to get matched with more tasks');
        if (!stats.availabilitySet) suggestions.push('Set your availability to get priority notifications');
        return suggestions;
    }

    private getUpcomingUnlocks(userId: string, stats: UserStats): GrowthPlan['upcomingUnlocks'] {
        const unlocks: GrowthPlan['upcomingUnlocks'] = [];
