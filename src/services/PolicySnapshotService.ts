...
class PolicySnapshotServiceClass {
    private cachedActivePolicy: PolicySnapshot | null = null;
     ...
     */
    async assignPolicyToTask(
        taskId: string,
        scope: PolicyScope
    ): Promise<{ policy_snapshot_id: string; config_hash: string }> {
     ...
     */
    private hashToBucket(taskId: string, buckets: number): number {
     ...
     */
    private async getActivePoliciesForScope(scope: PolicyScope): Promise<PolicySnapshot[]> {
     ...
     */
    async getPolicyForTask(taskId: string): Promise<PolicySnapshot | null> {
        if (!isDatabaseAvailable() || !sql) {
            return this.getDefaultPolicySnapshot();
        }
     ...
     */
    private generateConfigHash(config: PolicyConfig): string {
    ...
    async activatePolicy(id: string): Promise<void> {
        ...
        if (newTotal > 100) {
            throw new Error(
                `Cannot activate: allocation would exceed 100% (currently ${currentTotal}%, ` +
     ...
     */
    async deactivatePolicy(id: string, reason?: string): Promise<void> {
        if (!isDatabaseAvailable() || !sql) {
            throw new Error('Database not available');
        }
     ...
     */
    async checkSafetyRegression(
        policyId: string,
        baselineMetrics: { dispute_rate: number; injection_rate: number },
        currentMetrics: { dispute_rate: number; injection_rate: number }
    ): Promise<{ breached: boolean; reason: string | null }> {
    ...
    private getDefaultPolicySnapshot(): PolicySnapshot {
    ...
    private rowToPolicy(row: Record<string, unknown>): PolicySnapshot {
        return {
            id: row.id as string,
            ...
            trust_bucket: row.trust_bucket as PolicySnapshot['trust_bucket'],
            ...
            activated_at: row.activated_at ? new Date(row.activated_at as string) : null,
            deactivated_at: row.deactivated_at ? new Date(row.deactivated_at as string) : null,
        };
    }
     ...
     */
    async getActivePolicy(): Promise<PolicySnapshot> {
