...
class TimestampServiceClass {
  ...
  isWithinStreakGrace(lastTaskAt: Date): boolean {
    const now = this.now();
    const lastTaskDay = new Date(Date.UTC(
      lastTaskAt.getUTCFullYear(),
      lastTaskAt.getUTCMonth(),
      lastTaskAt.getUTCDate()
    ));
 ...
 */
export function sqlUTCTimestamp(): string {
