...
import { TrustTierService, TRUST_TIERS, TIER_NAMES } from '../services/TrustTierService.js';
...
interface EscrowStatusResponse {
  state: 'pending' | 'held' | 'locked_dispute' | 'released' | 'refunded' | 'partial_refund';
  amountCents: number;
...
interface ProofStatusResponse {
  state: 'not_started' | 'uploading' | 'pending_review' | 'accepted' | 'rejected' | 'expired';
  photos: Array<{
    uri: string;
    width: number;
    height: number;
...
export default async function frontendRoutes(
  ...
  fastify.get<{
  ...
  }>('/api/users/:userId/xp-progress', async (request, reply) => {
    ...
    try {
      ...
      const response: XPProgressResponse = {
        currentXP,
        level: currentLevel,
        xpToNextLevel,
        xpInCurrentLevel,
        ...
        trustTier: (user.trust_tier || 1) as 1 | 2 | 3 | 4,
  ...
  fastify.get<{
  ...
  }>('/api/tasks/:taskId/escrow-status', async (request, reply) => {
    ...
    try {
      ...
      const stateMap: Record<string, EscrowStatusResponse['state']> = {
        'pending': 'pending',
        'funded': 'held',
  ...
  fastify.get<{
    Params: { taskId: string };
  }>('/api/tasks/:taskId/proof-status', async (request, reply) => {
    const { taskId } = request.params;
    ...
    try {
      ...
      const stateMap: Record<string, ProofStatusResponse['state']> = {
      ...
      // Parse photo URLs
      let photos: ProofStatusResponse['photos'] = [];
      if (proof.photo_urls) {
        try {
  ...
  }>('/api/tasks/:taskId/submit-proof', async (request, reply) => {
    const { taskId } = request.params;
  ...
  fastify.get<{
    Params: { userId: string };
  }>('/api/users/:userId/trust', async (request, reply) => {
    const { userId } = request.params;
    ...
    try {
      ...
      return {
        ...
        tierName: TIER_NAMES[stats.currentTier as keyof typeof TIER_NAMES],
