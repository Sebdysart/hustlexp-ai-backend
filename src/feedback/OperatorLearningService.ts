...
export interface OperatorLearningEvent {
    ...
    entityId: string;          // Task ID or Recommendation ID
    operatorId: string;
    ...
    outcome?: {
        ...
        metric: string;       // E.g., "dispute_avoided" or "payout_delayed"
...
export class OperatorLearningService {
    ...
    static async recordDecision(params: {
        ...
        const {
            eventType, entityId, operatorId,
            aiAction, aiConfidence, aiReasoning,
            humanAction, humanReasoning
        } = params;

        // Determine agreement
        const agreement = this.calculateAgreement(aiAction, humanAction);

        const event: OperatorLearningEvent = {
            id: ulid(),
            eventType,
            entityId,
            operatorId,
    ...
    static async recordOutcome(params: {
        ...
        try {
            ...
            const event: OperatorLearningEvent = existing.data;
     ...
     */
    static async getSummary(days: number = 30): Promise<OperatorLearningSummary> {
        const db = getDb();

        const defaultSummary: OperatorLearningSummary = {
            periodDays: days,
            agreement: {
                fullAgreementRate: 0,
        ...
        try {
            ...
            const events: OperatorLearningEvent[] = rows.map((r: any) => r.data);
     ...
     */
    static async getImprovementRecommendations(): Promise<{
        forAI: string[];
        forOperators: string[];
        ...
        if (forAI.length === 0) forAI.push('AI performance is within expected range');
        if (forOperators.length === 0) forOperators.push('Operator decisions are well-calibrated');
    ...
    private static calculateAgreement(aiAction: string, humanAction: string): OperatorLearningEvent['agreement'] {
        if (aiAction.toLowerCase() === humanAction.toLowerCase()) return 'full';
    ...
    private static determineWhoWasRight(
        event: OperatorLearningEvent,
        result: 'success' | 'failure' | 'neutral'
    ): 'ai' | 'human' | 'unclear' {
        if (result === 'neutral') return 'unclear';
    ...
    private static calculateAgreementStats(events: OperatorLearningEvent[]): OperatorLearningSummary['agreement'] {
    ...
    private static calculateAccuracyStats(events: OperatorLearningEvent[]): OperatorLearningSummary['accuracy'] {
    ...
    private static identifyPatterns(events: OperatorLearningEvent[]): OperatorLearningSummary['patterns'] {
        const aiStrengths: string[] = [];
        const humanStrengths: string[] = [];
        const recommendations: string[] = [];

        // Group by event type and analyze
        const byType = new Map<string, OperatorLearningEvent[]>();
        ...
        if (aiStrengths.length === 0) aiStrengths.push('Not enough data to identify AI strengths');
        if (humanStrengths.length === 0) humanStrengths.push('Not enough data to identify human strengths');
        if (recommendations.length === 0) recommendations.push('Continue recording decisions to identify patterns');

        return { aiStrengths, humanStrengths, recommendations };
    }

    private static groupByEventType(events: OperatorLearningEvent[]): OperatorLearningSummary['byEventType'] {
        const result: OperatorLearningSummary['byEventType'] = {};
    ...
    private static async persistEvent(event: OperatorLearningEvent): Promise<void> {
        const db = getDb();
        if (!db) return;
