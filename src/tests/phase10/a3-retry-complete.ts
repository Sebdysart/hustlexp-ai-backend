...
async function completeTaskWithIdempotency(taskId: string, idempotencyKey: string): Promise<{ success: boolean; reason: string }> {
    try {
...
async function main() {
    ...
    const results = await Promise.all(
        Array.from({ length: RETRY_COUNT }, () => completeTaskWithIdempotency(taskId, idempotencyKey))
    );
    ...
    if (pass) {
        ...
        console.log('❌ A3 FAIL');
        if (!taskCompleted) console.log('  - Task not completed:', finalTask.status);
        if (!allAccountedFor) console.log('  - Requests not accounted for');
        if (!atMostOneActualComplete) console.log('  - Multiple completions:', completions.length);
    }
