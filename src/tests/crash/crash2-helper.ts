...
async function main() {
    ...
    if (command === 'post-crash') {
        ...
        stripeLog.forEach((s: any) => console.log(`  ${s.idempotency_key}: ${s.stripe_id} (${s.type})`));
        ...
        status.forEach((s: any) => console.log(`  ${s.status}: ${s.cnt}`));
        ...
        if (pass) {
            ...
            if (pending.length > 0) console.log('  - Pending transactions remain');
            if (nonZero.length > 0) console.log('  - Zero-sum violation');
            if (crash2Transfers.length > 1) console.log('  - DUPLICATE STRIPE TRANSFERS');
        }
    }
