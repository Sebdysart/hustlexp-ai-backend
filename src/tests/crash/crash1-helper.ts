...
`;
        ...
        console.log('\nRecent Transactions:');
        txs.forEach((t: any) => console.log(`  ${t.id}: ${t.status} (${t.type})`));

        console.log('\nRecent Prepares:');
        prepares.forEach((p: any) => console.log(`  ${p.ulid}: ${p.type}`));

        console.log('\nRecent Entries:');
        entries.forEach((e: any) => console.log(`  TX ${e.transaction_id}: ${e.direction} ${e.amount}`));
        ...
        statusDist.forEach((s: any) => console.log(`  ${s.status}: ${s.count}`));
        ...
        if (pass) {
            ...
            if (nonZeroSum.length > 0) console.log('  - Non-zero sum transactions found');
            if (pending.length > 0) console.log('  - Stuck pending transactions');
        }
