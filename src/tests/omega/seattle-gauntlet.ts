...
process.env.DATABASE_URL = 'postgresql://neondb_owner:npg_jsckB8AHbJa5@ep-young-shape-af9wgdv0-pooler.c-2.us-west-2.aws.neon.tech/neondb?sslmode=require';

// Module Scope Variables
let sql: any;
let transaction: any;
let TaskService: any;
let StripeMoneyEngine: any;
let DisputeService: any;
let AdminService: any;
let serviceLogger: any;
let KillSwitch: any;
let LedgerService: any;
let LedgerAccountService: any;
...
async function initServices() {
    ...
    try { await sql`DROP TABLE IF EXISTS money_events_processed`; } catch (e) { }
    await sql`
    ...
    try { await sql`DROP TABLE IF EXISTS admin_override_audit`; } catch (e) { }
    await sql`
    ...
    try { await sql`DROP TABLE IF EXISTS disputes`; } catch (e) { }
    await sql`
    ...
    try { await sql`ALTER TABLE tasks DROP CONSTRAINT IF EXISTS tasks_status_check`; } catch (e) { }
    await sql`
...
async function logResult(scenario: string, result: string, state: string, ledger: string, notes: string) {
...
async function setupTask(posterId: string) {
    const taskId = uuid();
...
async function runA1(poster: any, hustler: any) {
...
async function runA3(poster: any, hustler: any) {
    const taskId = await setupTask(poster.id);
    await TaskService.assignHustler(taskId, hustler.id);
...
async function runA4(poster: any, hustler: any) {
    const taskId = await setupTask(poster.id);
    await TaskService.assignHustler(taskId, hustler.id);
...
async function runB6(poster: any) {
    // Idempotency Test
    const taskId = await setupTask(poster.id);
    const eventId = ulid();
...
async function runC9(poster: any) {
    const taskId = await setupTask(poster.id);
...
async function runD12(poster: any, hustler: any) {
    const taskId = await setupTask(poster.id);
