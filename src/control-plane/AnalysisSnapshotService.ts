...
import { BetaMetricsService, THRESHOLDS as BetaThresholds } from '../services/BetaMetricsService.js';
...
export interface AnalysisSnapshot {
    ...
    // Operational Metrics (from BetaMetricsService)
    operations: {
    ...
    // Funnel Metrics (from MetricsService)
    funnel: {
        tasksCreated: number;
    ...
    // Risk Distribution (from RiskScoreService logs)
    riskDistribution: {
    ...
    // Shadow Policy Analysis (from AdaptiveProofPolicy)
    shadowAnalysis: {
...
export class AnalysisSnapshotService {
     ...
     * GET SNAPSHOT BY ID
     */
    static async getSnapshot(id: string): Promise<AnalysisSnapshot | null> {
        const db = getDb();
        if (!db) return null;
    ...
    static async listSnapshots(
        type?: SnapshotType,
        limit: number = 50
    ): Promise<{ id: string; type: SnapshotType; createdAt: Date }[]> {
        const db = getDb();
        if (!db) return [];
     ...
     */
    static async exportForAI(snapshotId: string): Promise<SnapshotExport | null> {
     ...
     * GET LATEST SNAPSHOT
     */
    static async getLatest(type?: SnapshotType): Promise<AnalysisSnapshot | null> {
        const db = getDb();
        if (!db) return null;
    ...
    private static async collectOperationsMetrics() {
    ...
    private static async collectFunnelMetrics() {
    ...
    private static async collectAIUsageMetrics() {
        // Uses MetricsService - already exists
    ...
    private static async collectRiskDistribution() {
        const db = getDb();
        ...
        try {
            ...
            const byTier: Record<string, number> = { minimal: 0, low: 0, medium: 0, high: 0, critical: 0 };
    ...
    private static async collectShadowAnalysis() {
    ...
    private static async collectSystemHealth() {
        const db = getDb();
    ...
    private static async storeSnapshot(snapshot: AnalysisSnapshot): Promise<void> {
        const db = getDb();
        if (!db) return;
