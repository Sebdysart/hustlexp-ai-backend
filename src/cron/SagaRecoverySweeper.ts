...
export class SagaRecoverySweeper {
    ...
    private static async recoverSaga(
        ...
        if (recoveryAttempts >= this.MAX_RECOVERY_ATTEMPTS) {
            ...
            await AlertService.fire(
                ...
                `Saga ${taskId} failed to recover after ${recoveryAttempts} attempts`,
                { taskId, state: currentState }
            );
        ...
        await AlertService.fire(
            'SAGA_STUCK',
            `Attempting recovery for saga ${taskId} (attempt ${recoveryAttempts + 1})`,
            { taskId, state: currentState, attempt: recoveryAttempts + 1 }
        );
     ...
     */
    private static async getOutboundIntent(
        db: ReturnType<typeof neon>,
        taskId: string
    ): Promise<any | null> {
        try {
     ...
     * MARK COMMITTED
     */
    private static async markCommitted(db: ReturnType<typeof neon>, taskId: string): Promise<void> {
        try {
            await db`
     ...
     * MARK FAILED
     */
    private static async markFailed(db: ReturnType<typeof neon>, taskId: string, reason: string): Promise<void> {
        try {
            await db`
                UPDATE money_state_lock
     ...
     * INCREMENT RECOVERY ATTEMPTS
     */
    private static async incrementRecoveryAttempts(db: ReturnType<typeof neon>, taskId: string): Promise<void> {
        try {
            await db`
                UPDATE money_state_lock
