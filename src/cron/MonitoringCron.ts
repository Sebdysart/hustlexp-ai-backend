...
class MonitoringCronClass {
  ...
  private metricsCheckTimer?: NodeJS.Timer;
  private metricsPersistTimer?: NodeJS.Timer;
    ...
    try {
      ...
      if (health.status === 'unhealthy') {
        await AlertingService.critical(
          ...
          `System is unhealthy: ${health.components.filter(c => c.status === 'unhealthy').map(c => c.name).join(', ')}`,
