...
| ID | Scenario | Preconditions | Steps | Expected | Status | Evidence | Sev | Owner |
|----|----------|---------------|-------|----------|--------|----------|-----|-------|
| MN-01 | 4242 success (real keystrokes) | Task created; Stripe test; real browser | Type `4242…` into Payment Element, Pay | `createPaymentIntent` 200 `{paymentIntentId,clientSecret,amount,escrowId}`; charge `succeeded`; "Task funded. Next: Hustler matching." only after backend FUNDED | **PASS (API-substituted)** / literal-keystrokes **BLOCKED→Human** | Backend path fully proven via pm_card_visa (=4242): `createPaymentIntent` returned `{paymentIntentId,clientSecret,amount:3000,escrowId}`; charge `ch_3Td4O9…` succeeded; UI "Task funded. Next: Hustler matching." only after backend FUNDED. **Literal in-iframe 4242 keystrokes still owed by Human own-browser pass.** | Critical | Human |
| MN-02 | Declined card recovery | Funding step | Enter `4000 0000 0000 0002`, Pay | Clean inline error; escrow stays PENDING; retry possible; `payment_failed {error_code}` (no message) | **BLOCKED→Human** | Decline requires in-iframe keystrokes; deferred to Human own-browser pass. (Backend stays PENDING on a declined PI — `confirmFunding` rejects non-succeeded — but the literal decline path is unrun.) | Critical | Human |
| MN-03 | 3DS redirect/resume | Funding step | Enter `4000 0027 6000 3184`, complete 3DS | Redirect to `return_url`, query-params stripped, polling resumes, → FUNDED | **PARTIAL** — resume mechanism PASS; literal 3DS **BLOCKED→Human** | Redirect-resume mechanism proven: navigated the real `return_url?payment_intent=…&…_client_secret=…&redirect_status=succeeded` → FundingStep resumed to polling, **query params stripped** (URL back to `/`), → FUNDED. **Literal 3DS challenge (4000 0027 6000 3184) still owed by Human.** | High | Human |
...
| ID | Scenario | Preconditions | Steps | Expected | Status | Evidence | Sev | Owner |
|----|----------|---------------|-------|----------|--------|----------|-----|-------|
| AN-01 | All 22 events fire | Full funnel run incl. fund+dashboard | Drive estimate→dispatch→fund→dashboard, capture each | All 22 events fire at their anchors; each once per ref-guard | **PARTIAL (17/22)** | Confirmed fired: landing_view, task_input_started, zip_entered, category_selected, local_availability_viewed, draft_estimate_started/succeeded, dispatch_clicked, signup_started/completed, terms_accepted, task_create_started/succeeded, payment_intent_created, payment_funded_backend, dashboard_viewed, task_detail_viewed. Remaining 5 (payment_started, payment_succeeded_client = iframe-Pay→Human; draft_estimate_failed, task_create_failed, payment_failed = failure paths) ride the same `capture()`. | High | Claude/Human |
