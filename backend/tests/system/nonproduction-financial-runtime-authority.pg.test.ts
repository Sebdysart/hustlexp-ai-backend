...
const authorityDatabase = `hx_ci_stage1_containment_${createHash('sha256')
  .update(`runtime-authority:${process.pid}`)
  .digest('hex')
  .slice(0, 20)}`;
...
describePg('nonproduction financial runtime PostgreSQL authority', () => {
  beforeAll(async () => {
    ...
    const actualRunnerRuntime: NonproductionFinancialMigrationRuntime = {
      env: {
        NODE_ENV: 'test',
    ...
    expect(
      runnerResult.migrations.slice(0, 8).every(({ status }) => status === 'already_applied')
    ).toBe(true);
    expect(runnerResult.migrations.slice(8).every(({ status }) => status === 'applied')).toBe(true);
    await ownerDatabaseClient.query(
    ...
    const captured: SchemaIdentityRow[] = [];
    const placeholderEvidence = schemaIdentityNames.map((identityName, index) => ({
      identityName,
      sha256: String(index + 1).repeat(64),
    }));
    const discovery = await readNonproductionFinancialBootstrapReadiness({
      ...readinessOptions(
        runtimeReadinessDatabase(runtimeClient, (rows) => captured.push(...rows))
      ),
    ...
    expect(captured).toHaveLength(schemaIdentityNames.length);
    for (const identityName of schemaIdentityNames) {
      const observed = captured.find(({ identity_name }) => identity_name === identityName);
      expect(observed?.identity_sha256).toMatch(/^[a-f0-9]{64}$/u);
    }
  }, 180_000);
  ...
  afterAll(async () => {
    ...
    runtimeClient = null;
    await ownerDatabaseClient?.end().catch(() => undefined);
    ownerDatabaseClient = null;
    if (adminClient) {
      await cleanupDisposableAuthorityDatabase().catch(() => undefined);
      await adminClient.end().catch(() => undefined);
      adminClient = null;
    }
  }, 60_000);

  it('attests one NOLOGIN owner and one distinct non-elevated LOGIN runtime as ready', async () => {
    ...
    const session = await runtimeClient!.query<{ current_role: string; session_role: string }>(
    ...
    expect(authorityQueryExecutions).toBeGreaterThanOrEqual(2);
  });
  ...
  it('allows only the exact sealed ports plus safe expression dependencies and makes recovery wrappers idempotent', async () => {
  ...
  it('fails closed when runtime loses either exact expression dependency', async () => {
    for (const signature of securityDefinerDependencies) {
      await ownerDatabaseClient!.query(
        `REVOKE EXECUTE ON FUNCTION ${signature} FROM ${quotedIdentifier(runtimeRole)}`
      );
  ...
  it('fails closed when one required sealed-wrapper grant is absent', async () => {
    const signature = runtimeFunctions[0];
    await ownerDatabaseClient!.query(
      `REVOKE EXECUTE ON FUNCTION ${signature} FROM ${quotedIdentifier(runtimeRole)}`
    );
    try {
