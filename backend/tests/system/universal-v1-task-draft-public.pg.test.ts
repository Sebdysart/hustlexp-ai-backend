...
describePg('Universal V1 TaskDraft public ingress PostgreSQL concurrency', () => {
  ...
  it('publishes one active version-one route across concurrent identical creates', async () => {
    ...
    expect(results.every((result) => result.ok &&
      result.relationship_origin?.kind === 'MARKETPLACE' &&
      result.relationship_origin.routing_state === 'ROUTING_READY')).toBe(true);
    ...
    const authority = await pool.query<{
      ...
      relationship_origin_contract_version: number;
      relationship_origin_kind: string;
      relationship_origin_id: string;
      relationship_origin_policy_version: number;
      relationship_origin_version: number;
      relationship_origin_routing_state: string;
      relationship_origin_evidence_digest: string;
      routes: number;
    }>(
      `SELECT draft.id, draft.card_token_hash, draft.universal_contract_version,
              draft.structured, draft.region, draft.active_routing_decision_id,
              route.id AS routing_id, route.decision_version,
              route.category_snapshot, route.service_cell_snapshot,
              route.service_cell_authority_id, route.evidence,
              draft.relationship_origin_contract_version,
              draft.relationship_origin_kind,
              ...
              origin.routing_state AS relationship_origin_routing_state,
              origin.evidence_digest AS relationship_origin_evidence_digest,
         ...
         JOIN LATERAL (
           SELECT candidate.*
             FROM universal_v1_relationship_origins candidate
    ...
    expect(authority.rows[0]).toMatchObject({
      ...
      decision_version: 1,
      region: SYNTHETIC_SERVICE_CELL_ROUGH_LOCATION,
      ...
      relationship_origin_evidence_digest: expect.stringMatching(/^[a-f0-9]{64}$/u),
      routes: 1,
    });
    ...
    expect(authority.rows[0]?.evidence).toMatchObject({
      route_context_contract_version: 1,
      work_category_code: 'furniture_assembly',
      postal_code: SYNTHETIC_SERVICE_CELL_POSTAL_CODE,
      region_code: SYNTHETIC_SERVICE_CELL_REGION_CODE,
      rough_location: SYNTHETIC_SERVICE_CELL_ROUGH_LOCATION,
      service_cell_authority_id: SYNTHETIC_SERVICE_CELL_AUTHORITY_ID,
      ...
      relationship_origin_contract_version: 1,
      relationship_origin_id: authority.rows[0]?.relationship_origin_id,
      relationship_origin_kind: 'MARKETPLACE',
      relationship_origin_policy_version: 1,
      relationship_origin_version: 1,
      relationship_origin_routing_state: 'ROUTING_READY',
      relationship_origin_evidence_digest:
        authority.rows[0]?.relationship_origin_evidence_digest,
    });
