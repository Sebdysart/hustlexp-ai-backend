...
describePg('Universal V1 TaskDraft account-claim PostgreSQL authority', () => {
  ...
  it('serializes concurrent exact claims into one event and no money or assignment state', async () => {
    ...
    const routes = await pool.query<{
      ...
      route_context_contract_version: number;
      work_category_code: string;
      region_code: string;
      evidence_service_cell_authority_id: string;
    }>(
      `SELECT route.decision_version, route.category_snapshot,
              route.service_cell_snapshot, route.service_cell_authority_id,
              (route.evidence ->> 'route_context_contract_version')::integer
                AS route_context_contract_version,
              route.evidence ->> 'work_category_code' AS work_category_code,
              route.evidence ->> 'region_code' AS region_code,
              route.evidence ->> 'service_cell_authority_id'
                AS evidence_service_cell_authority_id
         FROM task_routing_decisions route
        WHERE route.task_draft_id = $1
