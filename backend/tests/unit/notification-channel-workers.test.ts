...
describe('provider acceptance evidence', () => {
  ...
  it('records Twilio acceptance without claiming handset delivery', async () => {
    ...
    mocks.query.mockResolvedValueOnce({ rows: [], rowCount: 1 }).mockResolvedValueOnce({
      rows: [
        {
          id: 'sms-1',
          status: 'sent',
    ...
    expect(mocks.accepted).toHaveBeenCalledWith('notification-1', 'sms', 'twilio', 'SM123');
