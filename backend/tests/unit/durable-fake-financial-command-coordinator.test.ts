...
const ids = {
  ...
  void: 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee',
  adjust: 'ffffffff-ffff-4fff-8fff-ffffffffffff',
  reversal: '12121212-1212-4212-8212-121212121212',
  scope: '13131313-1313-4313-8313-131313131313',
  changeOrder: '14141414-1414-4414-8414-141414141414',
} as const;
...
function voidInput(
  ...
  return {
    operationKind: 'VOID',
    operationId: ids.void,
    providerKind: 'FAKE',
    idempotencyKey: 'foreground-fake:void:0001',
    providerExpectedVersion: 0,
    exactRequest: {
      operationId: ids.void,
      idempotencyKey: 'foreground-fake:void:0001',
...
function adjustInput(
  ...
  return {
    ...
    operationId: ids.adjust,
    providerKind: 'FAKE',
    idempotencyKey: 'foreground-fake:adjust:0001',
    providerExpectedVersion: 0,
    exactRequest: {
      operationId: ids.adjust,
      idempotencyKey: 'foreground-fake:adjust:0001',
...
function reversalInput(
  ...
  return {
    ...
    providerKind: 'FAKE',
    idempotencyKey: 'foreground-fake:reversal:0001',
    providerExpectedVersion: 0,
    exactRequest: {
      operationId: ids.reversal,
      idempotencyKey: 'foreground-fake:reversal:0001',
...
class CoordinatorRepository implements ForegroundFinancialProviderCommandRepository {
  ...
  private command(): FinancialProviderCommandRecoveryCommand {
    ...
    const actor = this.journal.input?.actor;
    const release = this.journal.input?.release;
    return {
      ...
      workOrderId:
        this.workOrderId === undefined ? (evidence?.workOrderId ?? null) : this.workOrderId,
      ...
      recordedActorId: actor?.actorId.toLowerCase() ?? null,
      recordedActorKind: actor?.actorKind ?? null,
      releaseManifestDigest: release?.manifestDigest ?? null,
      releaseId: release?.releaseId ?? null,
      releaseRevision: release?.revision ?? null,
      releaseEnvironment: release?.environment ?? null,
      releaseAuthenticationStatus: release?.authenticationStatus ?? null,
  ...
  async recordOutcome(
    ...
    this.outcome = {
      ...
      effectCertainty: observed
        ? input.effectCertainty
        : unknown
          ? 'UNKNOWN'
          : 'CONFIRMED_NO_EFFECT',
...
function fixture() {
  ...
  const coordinator = new DurableFakeFinancialProviderCommandCoordinator(repository, fakeEvents, {
...
describe('durable fake financial foreground coordinator', () => {
  ...
  it('durably dispatches an exact WorkOrder-bound ADJUST command', async () => {
    const { provider, invoker } = fixture();
    const result = await invoker.invokeAfterCommit(adjustInput(), (request) =>
      provider.adjust(request as never)
    );
  ...
  it('durably dispatches an exact pre-WorkOrder compensation VOID command', async () => {
    const { provider, invoker } = fixture();
    const result = await invoker.invokeAfterCommit(voidInput(), (request) =>
      provider.void(request as never)
    );
  ...
  it('refuses VOID with a WorkOrder binding and ADJUST without one', async () => {
    const voidFixture = fixture();
    const voidAdapter = vi.fn((request: Record<string, unknown>) =>
      voidFixture.provider.void(request as never)
    );
    await expect(
      voidFixture.invoker.invokeAfterCommit(voidInput(ids.workOrder), voidAdapter)
    ).rejects.toThrow('FAKE_FINANCIAL_FOREGROUND_PRE_WORK_ORDER_BINDING_REQUIRED');
    expect(voidAdapter).not.toHaveBeenCalled();

    const adjustFixture = fixture();
    const adjustAdapter = vi.fn((request: Record<string, unknown>) =>
      adjustFixture.provider.adjust(request as never)
    );
    await expect(
      adjustFixture.invoker.invokeAfterCommit(adjustInput(null), adjustAdapter)
    ).rejects.toThrow('FAKE_FINANCIAL_FOREGROUND_WORK_ORDER_BINDING_REQUIRED');
    expect(adjustAdapter).not.toHaveBeenCalled();
  });

  it('refuses a lifecycle command whose prepared authority binding is missing', async () => {
    const { provider, invoker } = fixture();
    const exact = voidInput();
    const adapter = vi.fn((request: Record<string, unknown>) => provider.void(request as never));
    await expect(invoker.invokeAfterCommit({
      ...exact,
      evidence: {
        taskDraftId: ids.draft,
        taskId: ids.task,
    ...
    }, adapter)).rejects.toThrow('FAKE_FINANCIAL_FOREGROUND_LIFECYCLE_AUTHORITY_REQUIRED');
    expect(adapter).not.toHaveBeenCalled();
  });

  it('durably dispatches WorkOrder-bound REVERSAL and refuses a missing WorkOrder binding', async () => {
    const { journal, provider, invoker } = fixture();
    const adapter = vi.fn((request: Record<string, unknown>) =>
      provider.reverse(request as never)
    );

    await expect(invoker.invokeAfterCommit(reversalInput(), adapter)).resolves.toMatchObject({
      result: { operationKind: 'REVERSAL', state: 'REVERSED' },
    });
    expect(adapter).toHaveBeenCalledOnce();
    expect(journal.order).toContain('OUTCOME_OBSERVED_COMMITTED');

    const missing = fixture();
    const missingAdapter = vi.fn((request: Record<string, unknown>) =>
      missing.provider.reverse(request as never)
    );
    await expect(
      missing.invoker.invokeAfterCommit(reversalInput(null), missingAdapter)
    ).rejects.toThrow('FAKE_FINANCIAL_FOREGROUND_WORK_ORDER_BINDING_REQUIRED');
    expect(missingAdapter).not.toHaveBeenCalled();
  });

  it('refuses a post-security lifecycle command without an exact WorkOrder binding', async () => {
    const { provider, invoker } = fixture();
    const adapter = vi.fn((request: Record<string, unknown>) => provider.capture(request as never));
