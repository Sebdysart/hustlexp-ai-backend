...
describe('legacy browser Operations authority retirement', () => {
  ...
  it('uses explicit allowlisted projections for legacy TaskDraft and provider roster reads', () => {
    const ops = routerSources[0].source;
    expect(ops).toContain('TASK_DRAFT_OPERATIONS_SELECT');
    expect(ops).toContain('TASK_DRAFT_OPERATIONS_RESPONSE_KEYS');
    expect(ops).toContain('HUSTLER_OPERATIONS_SELECT');
    expect(ops).toContain('HUSTLER_OPERATIONS_RESPONSE_KEYS');
    expect(ops).toContain('projectOperationsRow');
    expect(ops).not.toContain('SELECT d.*');

    const rosterProjection = ops.slice(
      ops.indexOf('const HUSTLER_OPERATIONS_SELECT'),
      ops.indexOf('const HUSTLER_OPERATIONS_RESPONSE_KEYS'),
    );
    expect(rosterProjection).toContain('AS name_initial');
    expect(rosterProjection).not.toMatch(/\bphone\b/i);
    expect(rosterProjection).not.toMatch(/\bemail\b/i);
    expect(rosterProjection).not.toMatch(/\bnotes\b/i);
  });
});
