...
describe('new-payment incident guard', () => {
  ...
  it('keeps every runtime task and escrow SQL writer in an explicit incident allowlist', () => {
    const allowedWriters = new Map<string, { occurrences: number; rationale: string }>([
      [
        'backend/src/services/EscrowReadService.ts:escrows',
        {
          occurrences: 1,
          rationale:
            'Historical createEscrow compatibility writer; retained as an incident-visible legacy surface.',
        },
      ],
      [
        'backend/src/services/TaskCreatePersistence.ts:escrows',
        {
          occurrences: 1,
          rationale:
            'Historical canonical task-creation persistence creates the task-dependent pending escrow.',
        },
      ],
      [
        'backend/src/services/TaskCreatePersistence.ts:tasks',
        {
          occurrences: 1,
          rationale:
            'Historical canonical task-creation persistence remains the legacy direct Task writer.',
        },
      ],
      [
        'backend/src/services/UniversalV1EstimatePostgresRepository.ts:tasks',
        {
          occurrences: 1,
          rationale:
            'Current Universal V1 estimate acceptance materializes a payment-free canonical Task.',
        },
      ],
    ]);
    const observedWriters = new Map<string, number>();
    const legacyInsert = /\bINSERT\s+INTO\s+(?:"?public"?\s*\.\s*)?"?(tasks|escrows)"?\b/giu;

    for (const file of sourceFiles(resolve(process.cwd(), 'backend/src'))) {
      const repositoryPath = relative(process.cwd(), file).replaceAll('\\', '/');
      const source = readFileSync(file, 'utf8');
      for (const match of source.matchAll(legacyInsert)) {
        const table = match[1]?.toLowerCase();
        expect(table, `${repositoryPath} matched a legacy insert without a table`).toBeDefined();
        const writer = `${repositoryPath}:${table}`;
        observedWriters.set(writer, (observedWriters.get(writer) ?? 0) + 1);
      }
    }

    expect(
      [...allowedWriters.values()].every(({ rationale }) => rationale.trim().length > 0),
      'Every allowlisted legacy writer requires an explicit rationale'
    ).toBe(true);
    expect(Object.fromEntries([...observedWriters].sort())).toEqual(
      Object.fromEntries(
        [...allowedWriters]
          .map(([writer, { occurrences }]) => [writer, occurrences] as const)
