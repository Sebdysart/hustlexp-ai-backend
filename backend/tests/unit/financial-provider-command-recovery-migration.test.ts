...
const recoveryRepository = readFileSync(
  resolve(process.cwd(), 'backend/src/services/payment/FinancialProviderCommandRecovery.ts'),
  'utf8'
);
...
describe('financial provider command recovery migration', () => {
  ...
  it('preserves pending and retryable observations as durable nonterminal evidence', () => {
    ...
    expect(recoveryWorker).toContain(
      "new Set<FinancialOperationState>(['PENDING', 'RETRYABLE_FAILURE'])"
    );
  });
  ...
  it('uses the shared nonproduction gate and reports wired versus blocked integrations', () => {
