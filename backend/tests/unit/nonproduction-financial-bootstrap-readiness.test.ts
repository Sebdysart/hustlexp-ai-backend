...
describe('nonproduction fake-finance bootstrap readiness', () => {
  ...
  it('attests one exact manifest/bootstrap record and every current SQL checksum', async () => {
    ...
    expect(schemaEvidenceSql).toContain('hxos_fake_financial_schema_evidence_v6');
    expect(schemaEvidenceSql).toContain('hxos_fake_financial_schema_evidence_v7');
    expect(schemaEvidenceSql).toContain('hxos_fake_financial_schema_evidence_v8');
  ...
  it('derives all twelve critical applied checksums from exact registered SQL bytes', async () => {
    const query = readinessQuery({
  ...
  it('fails closed when exact v8 append-only schema evidence is absent', async () => {
    const result = await readNonproductionFinancialBootstrapReadiness(
