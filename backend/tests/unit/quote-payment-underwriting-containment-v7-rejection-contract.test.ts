...
describe('quote payment underwriting containment v7', () => {
  ...
  it.each([
  ...
  ])('rejects null, conflicting, and email-lookalike ownership before verification', async (actorLinks) => {
    ...
    const result = await finalizePaidQuote({
      ...
      quoteVersionId: '10000000-0000-4000-8000-000000000002',
      posterId: '10000000-0000-4000-8000-000000000003',
      paymentIntentId: 'pi_actor_authority_rejected',
    });

    expect(result).toMatchObject({
      success: false,
      error: { code: 'QUOTE_POSTER_MISMATCH' },
    });
    const ownershipSql = String(mocks.dbQuery.mock.calls[1]?.[0]);
  ...
  it('revalidates canonical actor links under lock before task or escrow materialization', async () => {
    enableControlledStripePaymentTestCohortV7();
    vi.stubEnv('STRIPE_SECRET_KEY', 'sk_test_actor_revalidation');
    const posterId = '10000000-0000-4000-8000-000000000003';
    mocks.dbQuery
      .mockResolvedValueOnce({ rows: [], rowCount: 0 })
      .mockResolvedValueOnce({
        rows: [{
          quote_id: '10000000-0000-4000-8000-000000000001',
          quote_version_id: '10000000-0000-4000-8000-000000000002',
          draft_poster_id: posterId,
          lead_user_id: posterId,
          total_cents: 12500,
        }],
        rowCount: 1,
      });
    ...
    const query = vi.fn()
      ...
      .mockResolvedValueOnce({
        rows: [{
          id: 'draft-1',
          lead_id: 'lead-1',
          poster_user_id: '20000000-0000-4000-8000-000000000001',
          category: 'other',
          title: null,
          scope_summary: null,
          structured: {},
      ...
      .mockResolvedValueOnce({
        rows: [{ id: 'lead-1', user_id: posterId }],
        rowCount: 1,
      });
    ...
    const result = await finalizePaidQuote({
      ...
      quoteVersionId: '10000000-0000-4000-8000-000000000002',
      posterId,
      paymentIntentId: 'pi_actor_link_changed_after_verify',
    });
    ...
    expect(String(query.mock.calls[3]?.[0])).toContain('poster_user_id');
    expect(String(query.mock.calls[4]?.[0])).toContain('user_id');
