...
const enabled = {
  ...
  DATABASE_URL: 'postgresql://hx_test_payment@127.0.0.1:5432/hx_payment_test',
  ...
  HXOS_LOCAL_TEST_DATABASE_ROLE: 'hx_test_payment',
};
...
describe('local certification payment provider', () => {
  it('requires every non-production TEST guard and a strong secret', () => {
    ...
    for (const override of [
      ...
      { DATABASE_URL: 'postgresql://hx_test_payment@prod.internal:5432/hx_payment_test' },
      { DATABASE_URL: 'postgresql://hx_test_payment@127.0.0.1:5432/hustlexp' },
  ...
  it('creates a deterministic TEST intent and reuses only equivalent state', async () => {
    ...
    vi.mocked(db.query).mockResolvedValueOnce({
      rows: [{ database_name: 'hx_payment_test', database_user: 'hx_test_payment' }],
      rowCount: 1,
    } as never);
    const query = vi.fn();
