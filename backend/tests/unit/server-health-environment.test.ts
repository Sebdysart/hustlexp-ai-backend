...
describe('API deployment health environment', () => {
  ...
  it('routes every API release-health evaluation through the runtime environment resolver', () => {
    ...
    expect(
      source.match(/nonproductionFinancialBootstrap: financialBootstrap/gu)?.length
    ).toBeGreaterThanOrEqual(5);
    expect(source.match(/&&\s*financialBootstrap\.ready/gu)?.length).toBeGreaterThanOrEqual(3);
    ...
    expect(
      source.match(/legacyTaskMaterialization: legacyTaskMaterializationHealth\(\)/gu)?.length
    ).toBeGreaterThanOrEqual(6);
    expect(
      source.match(/hardAssignment: hardAssignmentHealth\(\)/gu)?.length
    ).toBeGreaterThanOrEqual(6);
  });
});
