...
describe('provider-neutral outbound communication ports', () => {
  ...
  it('keeps the deterministic receipt when the sink closes after final DATA acceptance', async () => {
    ...
    const address = server.address();
    if (!address || typeof address === 'string')
      throw new Error('SMTP post-accept close test server did not bind TCP');
  ...
  it('rejects a pending SMTP response when the sink closes after a command', async () => {
    const server = createServer((socket) => {
      socket.write('220 synthetic-mail-sink ready\r\n');
    ...
    const address = server.address();
    if (!address || typeof address === 'string')
      throw new Error('SMTP close test server did not bind TCP');
    ...
    try {
      ...
      ).rejects.toThrow(/closed the connection/u);
    } finally {
      await new Promise<void>((resolveClose) => server.close(() => resolveClose()));
    }
  });

  it('rejects a pending SMTP response on timeout without leaking sink response text', async () => {
    const server = createServer((socket) => {
      socket.write('220 synthetic-mail-sink ready\r\n');
      socket.once('data', () => {
        // Leave the EHLO response pending so the client timeout must reject
        // the exact waiter instead of hanging the worker forever.
      });
    });
    await new Promise<void>((resolveListen) => server.listen(0, '127.0.0.1', resolveListen));
    const address = server.address();
    if (!address || typeof address === 'string')
      throw new Error('SMTP timeout test server did not bind TCP');
    ...
    try {
      ...
      ).rejects.toThrow('SMTP sink connection timed out');
    } finally {
      await new Promise<void>((resolveClose) => server.close(() => resolveClose()));
    }
  });

  it('reports only the SMTP response code when the sink rejects a command', async () => {
    const privateSinkText = 'private-recipient@example.invalid must never enter logs';
    const server = createServer((socket) => {
      socket.write('220 synthetic-mail-sink ready\r\n');
      socket.once('data', () => socket.write(`550 ${privateSinkText}\r\n`));
    });
    await new Promise<void>((resolveListen) => server.listen(0, '127.0.0.1', resolveListen));
    const address = server.address();
    if (!address || typeof address === 'string')
      throw new Error('SMTP rejection test server did not bind TCP');

    try {
      const failure = await sendEmailToSmtpSink(
        new URL(`smtp://127.0.0.1:${address.port}`),
        emailMessage,
        { timeoutMs: 1_000 }
      ).catch((error: unknown) => error);
      expect(failure).toBeInstanceOf(Error);
      expect((failure as Error).message).toBe('SMTP sink expected 250, received code 550');
      expect((failure as Error).message).not.toContain(privateSinkText);
    } finally {
      await new Promise<void>((resolveClose) => server.close(() => resolveClose()));
    }
  });
