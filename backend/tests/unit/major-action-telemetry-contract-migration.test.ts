...
const SQL = readFileSync(
  resolve(
    process.cwd(),
    'backend/database/migrations/20260720_major_action_telemetry_contract.sql'
  ),
  'utf8'
);
const RUNNER = [
...
const REPAIR = readFileSync(
  resolve(
    process.cwd(),
    'backend/database/migrations/20260720_major_action_telemetry_contract_repair.sql'
  ),
  'utf8'
);
const SOURCE_REPAIR = readFileSync(
  resolve(
    process.cwd(),
    'backend/database/migrations/20260720_major_action_source_registry_repair.sql'
  ),
  'utf8'
);
...
describe('HX/OS major-action telemetry migration', () => {
  ...
  it('is payload-free and purpose-separates sensitive source evidence', () => {
    const tableBody =
      SQL.match(/CREATE TABLE IF NOT EXISTS major_action_events \(([\s\S]*?)\n\);/)?.[1] ?? '';
