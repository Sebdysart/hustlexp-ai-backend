...
describe('PostgresUniversalV1ChangeOrderRepository', () => {
  ...
  it.each([
    ...
    async (changeOrderKind, customerTotal, providerPayout, expectsAdjustment) => {
      ...
      expect(statements[4]).toContain("customer_approval_actor.account_status = 'ACTIVE'");
      expect(statements[4]).toContain("provider_approval_actor.account_status = 'ACTIVE'");
