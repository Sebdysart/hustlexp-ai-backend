  ...
  it('filters the roster by both status and availability', async () => {
    ...
    mocks.query.mockResolvedValueOnce({
      rows: [{
        ...
        phone: '+12065550100',
        email: 'worker@example.com',
        notes: 'private screening notes',
      }],
      rowCount: 1,
    });
    await expect(caller().listHustlers({ status: 'approved', available: false }))
      .resolves.toEqual({ ok: true, hustlers: [{ id: HUSTLER_ID, name_initial: 'R.' }] });
    expect(mocks.query.mock.calls[1][1]).toEqual(['approved', false]);
    const rosterSql = String(mocks.query.mock.calls[1][0]);
    expect(rosterSql).toContain('status = $1');
    expect(rosterSql).toContain('available = $2');
    expect(rosterSql).toContain('AS name_initial');
    expect(rosterSql).not.toMatch(/\bphone\b/i);
    expect(rosterSql).not.toMatch(/\bemail\b/i);
    expect(rosterSql).not.toMatch(/\bnotes\b/i);
  });

  it('returns only the explicit TaskDraft Operations projection', async () => {
    authorizeOperations();
    mocks.query.mockResolvedValueOnce({
      rows: [{
        id: DRAFT_ID,
        ...
        quote_status: 'draft',
        total_cents: 5_000,
        card_token_hash: 'must-not-escape',
        ip_hash: 'must-not-escape',
        raw_input: 'private raw intake',
        structured: { private: true },
        utm: { campaign: 'private-attribution' },
      }],
      rowCount: 1,
    });

    const result = await caller().getTaskDraft({ id: DRAFT_ID });
    ...
    const draftSql = String(mocks.query.mock.calls[1][0]);
    expect(draftSql).not.toContain('d.*');
    expect(draftSql).not.toMatch(/card_token_hash|ip_hash|raw_input|structured|\butm\b/i);
  });
