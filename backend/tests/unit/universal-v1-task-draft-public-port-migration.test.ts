...
describe('Universal V1 task-draft-public compatibility migration', () => {
  ...
  it('classifies exact ingress, token, IP-hash, and legacy evidence contracts', () => {
    ...
    for (const scheme of ['UNKNOWN_V0', 'LEGACY_SHA256_IP_SUFFIX_V1', 'HMAC_SHA256_V1']) {
    ...
    expect(migration).toContain('ON public.task_drafts(ip_hash_scheme, ip_hash, created_at DESC)');
  });
  ...
  it('backfills only compatibility metadata and never guesses canonical authority', () => {
    ...
    const assignments = Array.from((match?.[1] ?? '').matchAll(/^\s*([a-z_]+)\s*=/gmu)).map(
      ([, column]) => column
    );
