...
import { gracefulShutdown, registerWorkers, shutdownWorkerResources } from '../../src/jobs/workers';
...
describe('worker runtime shutdown', () => {
  it('drains workers before health, Redis, and database cleanup', async () => {
    ...
    await shutdownWorkerResources({
      ...
      closeFakeFinancialCommandRecovery: async () => {
        order.push('fake-recovery');
      },
      closeWorkOrderCompensation: async () => {
        order.push('work-order-compensation');
      },
      closeChangeOrderRecovery: async () => {
        order.push('change-order-recovery');
      },
