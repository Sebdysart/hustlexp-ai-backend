...
const ids = {
  ...
  adjustment: '10000000-0000-4000-8000-000000000007',
  operation: '10000000-0000-4000-8000-000000000008',
  financial: '10000000-0000-4000-8000-000000000009',
  task: '10000000-0000-4000-8000-000000000010',
  draft: '10000000-0000-4000-8000-000000000011',
  eligibility: '10000000-0000-4000-8000-000000000012',
};
...
function repositoryFixture() {
  return {
    ...
    preparePriceAndScopeMaterialization: vi.fn().mockResolvedValue({
      completed: false,
    ...
    finalizePriceAndScopeMaterialization: vi.fn().mockResolvedValue({
      amendment_id: ids.amendment,
      amendment_version: 1,
      proposal_id: ids.proposal,
      scope_version_id: ids.scope,
      scope_version: 2,
      adjustment_event_id: ids.adjustment,
      provider_kind: 'FAKE',
      replayed: false,
      payment_creation_performed: false,
      hard_assignment_created: false,
...
describe('UniversalV1ChangeOrderApplication', () => {
  it.each([
    ...
    async (kind, ts) => {
      ...
      expect(repository.preparePriceAndScopeMaterialization).not.toHaveBeenCalled();
      expect(repository.finalizePriceAndScopeMaterialization).not.toHaveBeenCalled();
  ...
  it('commits price authority before provider I/O and finalizes the exact fake adjustment', async () => {
    const repository = repositoryFixture();
    vi.mocked(repository.readFinalizationKind).mockResolvedValue('PRICE_AND_SCOPE');
    const executeFinancialEvent = vi.fn().mockResolvedValue({ id: ids.adjustment });
    const authorizeFinance = vi.fn().mockReturnValue({ executeFinancialEvent });
    const application = new UniversalV1ChangeOrderApplication(
      repository,
      authorizeFinance,
      () => nowMs
    ...
    expect(authorizeFinance.mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(repository.preparePriceAndScopeMaterialization).mock.invocationCallOrder[0]!
    );
    expect(repository.preparePriceAndScopeMaterialization).toHaveBeenCalledWith(
      ids.actor,
      finalization
    );
    expect(executeFinancialEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        providerKind: 'FAKE',
        operationId: ids.adjustment,
        idempotencyKey: `${finalization.idempotency_key}:adjust`,
        taskDraftId: ids.draft,
        taskId: ids.task,
        scopeVersionId: ids.scope,
    ...
    expect(repository.finalizePriceAndScopeMaterialization).toHaveBeenCalledWith(
      expect.objectContaining({ completed: false, idempotencyKey: finalization.idempotency_key }),
      ids.adjustment,
      ids.actor
    );
    expect(repository.authorizeAndMaterializeFakeChangeOrder).not.toHaveBeenCalled();
  ...
  it('authorizes the fake-only lane but performs no provider I/O for an exact completed replay', async () => {
    const repository = repositoryFixture();
    vi.mocked(repository.readFinalizationKind).mockResolvedValue('PRICE_AND_SCOPE');
    const completed = {
      amendment_id: ids.amendment,
    ...
    vi.mocked(repository.preparePriceAndScopeMaterialization).mockResolvedValue({
      completed: true,
      result: completed,
    });
  ...
  it('refuses a closed fake-finance capability gate before committing Phase A', async () => {
    const repository = repositoryFixture();
    vi.mocked(repository.readFinalizationKind).mockResolvedValue('PRICE_AND_SCOPE');
    const authorizeFinance = vi.fn(() => {
      throw new Error('NONPRODUCTION_FAKE_FINANCE_REFUSED');
    });
    const application = new UniversalV1ChangeOrderApplication(
      repository,
      authorizeFinance,
  ...
  it('leaves the committed witness for recovery when provider execution fails', async () => {
    const repository = repositoryFixture();
    vi.mocked(repository.readFinalizationKind).mockResolvedValue('PRICE_AND_SCOPE');
    const executeFinancialEvent = vi.fn().mockRejectedValue(new Error('FAKE_PROVIDER_TIMEOUT'));
    const application = new UniversalV1ChangeOrderApplication(
      repository,
      vi.fn().mockReturnValue({ executeFinancialEvent }),
      () => nowMs
    );

    await expect(
      application.authorizeAndMaterializeFakeChangeOrder(ids.actor, finalization)
    ).rejects.toThrow('FAKE_PROVIDER_TIMEOUT');
    expect(repository.preparePriceAndScopeMaterialization).toHaveBeenCalledOnce();
    expect(executeFinancialEvent).toHaveBeenCalledOnce();
    expect(repository.finalizePriceAndScopeMaterialization).not.toHaveBeenCalled();
  });
