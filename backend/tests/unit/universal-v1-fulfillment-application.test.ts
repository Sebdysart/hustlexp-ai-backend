...
describe('UniversalV1FulfillmentApplication', () => {
  ...
  it('reports an unavailable completion-notice dependency without invoking other lifecycle effects', async () => {
    const privateMessage = 'recipient email and synthetic sink actor database detail';
    const completionNoticeError = new UniversalV1FulfillmentError(
    ...
    const fulfillment = {
      ...
      submitCompletionEvidence: vi.fn().mockRejectedValue(completionNoticeError),
      decideCompletion: vi.fn(),
      completeFakeFinancialLifecycle: vi.fn(),
    };
    const caller = createUniversalContractRouter(estimates, workOrders, fulfillment).createCaller({
      user: fulfillmentActor,
      firebaseUid: 'firebase-fulfillment-actor',
      ip: '203.0.113.44',
    });
    const input = {
      ...
      idempotency_key: 'fulfillment:test:router:notice-unavailable',
      client_ts: new Date().toISOString(),
    };

    const request = caller.submitCompletionEvidence(input);
    await expect(request).rejects.toMatchObject({
      code: 'SERVICE_UNAVAILABLE',
      message: 'The required completion-notice service is temporarily unavailable.',
    });
    await expect(request).rejects.not.toMatchObject({ message: expect.stringContaining(privateMessage) });
    expect(fulfillment.submitCompletionEvidence).toHaveBeenCalledOnce();
    expect(fulfillment.submitCompletionEvidence).toHaveBeenCalledWith(fulfillmentActor.id, input);
    expect(fulfillment.recordExecutionEvidence).not.toHaveBeenCalled();
    expect(fulfillment.decideCompletion).not.toHaveBeenCalled();
    expect(fulfillment.completeFakeFinancialLifecycle).not.toHaveBeenCalled();
    expect(Object.values(workOrders).every((operation) => operation.mock.calls.length === 0)).toBe(
      true
    );
  });
  ...
  it('maps fulfillment authority and invariant failures without raw details', () => {
    ...
    expect(
      universalV1EstimateRouteError(
        new UniversalV1FulfillmentError(
          'FULFILLMENT_COMPLETION_NOTICE_UNAVAILABLE',
          'private completion-notice database detail'
        )
      )
    ).toMatchObject({
      code: 'SERVICE_UNAVAILABLE',
