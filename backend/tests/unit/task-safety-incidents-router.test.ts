...
describe('task safety incident intake', () => {
  ...
  it.each([
    ...
    'opens severe class %s with enforced %s urgency and an Operations mirror',
    async (category, effectiveUrgency) => {
        ...
        rows: [
          {
            ...
            contact_permission: 'in_app_only',
            created_at: new Date().toISOString(),
            request_hash: RECONCILED_SYNC_REPORT_HASH,
            sync_contract_version: 1,
        ...
        rows: [
          {
            ...
            request_hash: SYNC_REPORT_HASH,
            sync_contract_version: 1,
            reconciliation_contract_version: 0,
            created: false,
          },
        ],
        rowCount: 1,
  ...
  it('rejects a stale task version before storing a synchronized safety command', async () => {
    ...
    expect(
      mockDb.query.mock.calls.some(([sql]) =>
        String(sql).includes('INSERT INTO task_safety_incidents')
      )
    ).toBe(false);
  });
  ...
  it('rejects a non-participant without revealing that the task exists', async () => {
    ...
    await expect(
      caller('66666666-6666-4666-8666-666666666666').reportSafety({
        taskId: TASK_ID,
        category: 'injury',
        urgency: 'urgent',
        ...
        rows: [
          {
            ...
            contact_permission: 'in_app_only',
            created_at: new Date().toISOString(),
            request_hash: locationHash,
            created: true,
            location_ciphertext: 'encrypted-only',
            location_nonce: 'nonce',
            location_auth_tag: 'tag',
            location_key_id: 'safety-location-test-v1',
  ...
  it('rejects a replay key when the captured coordinates change', async () => {
    ...
    mockDb.query
      .mockResolvedValueOnce({ rows: [task()], rowCount: 1 } as any)
      .mockResolvedValueOnce({
        rows: [{ id: INCIDENT_ID, request_hash: firstHash, created: false }],
        rowCount: 1,
      } as any);
        ...
        rows: [
          {
            ...
            status: 'acknowledged',
            delivery_state: 'contact_attempted',
            contact_permission: 'call',
            location_sharing_enabled: true,
            owner_assigned: true,
            owned_by_current_operator: true,
            location_evidence_available: true,
            created_at: new Date().toISOString(),
          },
        ],
        rowCount: 1,
  ...
  it('rejects acknowledgment replay by an operator who does not own the case', async () => {
      ...
      rows: [
        {
          ...
          delivery_state: 'received',
          acknowledged_at: new Date().toISOString(),
          assigned_admin_id: OTHER_ADMIN_ID,
          changed: false,
        },
      ],
      rowCount: 1,
    ...
    await expect(
      caller(ADMIN_ID, true).acknowledgeSafety({
        incidentId: INCIDENT_ID,
        publicMessage: 'A safety operator acknowledged this report.',
      })
    ).rejects.toMatchObject({
  ...
  it.each([
    ...
    'rejects resolution from status %s with owner %s',
    async (status, assignedAdminId, code) => {
      seedSafetyAdminCapability();
      mockDb.query
        .mockResolvedValueOnce({
          rows: [
      ...
      await expect(
        caller(ADMIN_ID, true).resolveSafety({
          incidentId: INCIDENT_ID,
          idempotencyKey: RESOLUTION_KEY,
      ...
      expect(
        mockDb.query.mock.calls.some(([sql]) => String(sql).includes("SET status = 'resolved'"))
      ).toBe(false);
    }
  );
  ...
  it('records attempted contact without conflating it with human acknowledgment', async () => {
    ...
    expect(result).toEqual({
      incidentId: INCIDENT_ID,
      deliveryState: 'contact_attempted',
      idempotencyReplayed: false,
    });
  ...
  it('replays a provider contact event and rejects provider ID conflicts', async () => {
    ...
    mockDb.query
        ...
        rows: [
          {
            ...
            delivery_state: 'contact_delivered',
            status: 'acknowledged',
          },
        ],
        rowCount: 1,
      ...
      .mockResolvedValueOnce({
        rows: [{ incident_id: INCIDENT_ID, request_hash: requestHash }],
        rowCount: 1,
      } as any);
    await expect(caller(ADMIN_ID, true).recordSafetyContact(input)).resolves.toEqual({
      incidentId: INCIDENT_ID,
      deliveryState: 'contact_delivered',
    ...
    mockDb.query
      ...
      .mockResolvedValueOnce({
        rows: [{ incident_id: INCIDENT_ID, request_hash: 'f'.repeat(64) }],
        rowCount: 1,
      } as any);
    await expect(caller(ADMIN_ID, true).recordSafetyContact(input)).rejects.toMatchObject({
      code: 'CONFLICT',
    });
  });
});
