...
const ids = {
  ...
  serviceCellAuthority: '00000000-0000-4000-8000-000000000118',
} as const;
...
const priorRouteContextEvidence = {
  ...
  postal_code: '98004',
  service_cell_authority_id: ids.serviceCellAuthority,
...
function acceptanceContext(overrides: Record<string, unknown> = {}) {
  return {
    ...
    prior_category_snapshot: 'yard',
    prior_service_cell_snapshot: 'US-WA',
    prior_service_cell_authority_id: ids.serviceCellAuthority,
    prior_route_evidence: priorRouteContextEvidence,
    provider_estimate_submission_id: ids.submission,
    quote_id: ids.quote,
    quote_version_id: ids.quoteVersion,
...
describe('UniversalV1EstimateService customer acceptance', () => {
  it('atomically creates one unassigned payment-frozen Task, scope v1, route, and acceptance fact', async () => {
    ...
    const routeInsert = fixture.calls.find(({ sql }) =>
      sql.includes('INSERT INTO task_routing_decisions')
    )!;
    expect(routeInsert.params.slice(6, 9)).toEqual([
      'yard',
      'US-WA',
      ids.serviceCellAuthority,
    ]);
    expect(JSON.parse(String(routeInsert.params[9]))).toEqual({
      ...priorRouteContextEvidence,
      providerEstimateSubmissionId: ids.submission,
      quoteId: ids.quote,
      quoteVersionId: ids.quoteVersion,
      scopeSha256: 'a'.repeat(64),
      requestSha256: universalV1EstimateAcceptanceRequestSha256(
        AcceptUniversalV1ProviderEstimateSchema.parse(command)
      ),
