...
const expectedProductionFiles = [
  ...
  'backend/src/routers/hustlerWallet.ts',
  'backend/src/routers/quotePayment.ts',
  'backend/src/routers/subscription.ts',
  'backend/src/routers/xpTax.ts',
  'backend/src/services/EscrowPartialRefundProvider.ts',
  'backend/src/services/HustlerCashOutRequestService.ts',
  'backend/src/services/HustlerWalletProvider.ts',
  ...
  'backend/src/services/SelfInsurancePoolService.ts',
  ...
  it('pins the exact authoritative document export and current-main stack base', () => {
    expect(PAYMENT_UNDERWRITING_AUTHORITY_V7).toEqual(
      expect.objectContaining({
        ...
        currentMainBase: '73c44eee22fa79c2957583217e69aa972291776f',
  ...
  it('ships one machine-readable D0+D1 traceability manifest bound to the executable authority', () => {
    expect(traceability).toEqual(
      expect.objectContaining({
        ...
        productionLaunchState: 'NO_GO_INCIDENT_RECOVERY_NOT_CERTIFIED',
    ...
    const manifest = traceability as {
      ...
      stack: {
        migration: {
          ...
          retainsHistoricalPr276Migrations: number;
          retainsQuotePaymentRecovery: string;
          persistentTargetApplicationAuthorized: boolean;
        };
      };
    ...
    expect(manifest.guard.lanes).toHaveLength(9);
    expect(manifest.guard.authorityCode).toBe('UNDERWRITING_DECISIONS_UNRESOLVED');
    expect(manifest.stack.migration).toEqual({
      disposition: 'APPEND_ONLY_FORWARD_CONTAINMENT',
      name: '20260825_pr276_incident_containment',
      file: 'backend/database/migrations/20260825_pr276_incident_containment.sql',
      registry: 'backend/src/jobs/engine-automation-migration-files.ts',
      requiredChainLength: 115,
      retainsHistoricalPr276Migrations: 9,
      retainsQuotePaymentRecovery: '20260823_quote_payment_recovery',
      persistentTargetApplicationAuthorized: false,
    });
    expect(REQUIRED_MIGRATION_FILES).toHaveLength(
    ...
    expect(REQUIRED_MIGRATION_FILES.at(-1)).toEqual({
      ...
      fileName: '20260825_pr276_incident_containment.sql',
    });
