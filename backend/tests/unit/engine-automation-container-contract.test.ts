...
describe('engine automation production container contract', () => {
  ...
  it('keeps schema writes out of web and worker startup and exposes one explicit writer', () => {
    const pkg = JSON.parse(read('package.json')) as { scripts: Record<string, string> };
    expect(pkg.scripts.start).not.toMatch(/engine-automation-migration|migration-command/);
    ...
    const composition = read('backend/src/server.ts');
    expect(composition).toContain('await bootHttpServer({');
    expect(composition).toContain('startup: startServer');
    expect(composition.indexOf('startup: startServer')).toBeLessThan(
      composition.indexOf('listen: () => serve(')
    );
