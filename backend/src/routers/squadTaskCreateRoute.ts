...
async function compensateTask(taskId: string, taskVersion: number, squadId: string): Promise<void> {
  const result = await TaskService.cancelForInternalPurpose({
    taskId,
...
export const squadTaskCreateProcedures = {
  createTeamTask: posterProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const taskVersion = created.data.version;
      if (
        ...
        log.error(
          { taskId, taskVersion: taskVersion ?? null, squadId: input.squadId },
          'CRITICAL: created task did not expose a canonical version; refusing unversioned squad assignment'
        );
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Created task did not expose a canonical version.',
        });
      }
      try {
        ...
        await compensateTask(taskId, taskVersion, input.squadId);
        throw error;
      }
    }),
};
