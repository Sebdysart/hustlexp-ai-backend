...
const TASK_DRAFT_OPERATIONS_SELECT = `
  d.id, d.submission_id, d.category, d.title, d.scope_summary,
  d.est_price_min_cents, d.est_price_max_cents, d.photo_count,
  d.zip, d.region, d.status, d.source, d.lead_id, d.poster_user_id,
  d.task_id, d.quote_id, d.claimed_at, d.quote_send_ready_at,
  d.created_at, d.updated_at, d.universal_contract_version,
  d.active_routing_decision_id, q.id AS quote_id_linked,
  qv.status AS quote_status, qv.total_cents
`;

const TASK_DRAFT_OPERATIONS_RESPONSE_KEYS = [
  'id',
  'submission_id',
  'category',
  'title',
  ...
  'active_routing_decision_id',
  'quote_id_linked',
  'quote_status',
  'total_cents',
] as const;

const HUSTLER_OPERATIONS_SELECT = `
  id,
  COALESCE(NULLIF(LEFT(BTRIM(name), 1), ''), '?') || '.' AS name_initial,
  home_zip, radius_miles, vehicle, max_lift_lbs, trust_tier, checkr_status,
  status, available, availability_note, completed_jobs, cancel_count,
  rating_avg, response_minutes, skills, user_id, created_at, updated_at
`;

const HUSTLER_OPERATIONS_RESPONSE_KEYS = [
  'id',
  'name_initial',
  'home_zip',
  'radius_miles',
...
export const webOpsRouter = router({
  ...
  getTaskDraft: operationsAdminProcedure
    ...
    .query(async ({ input }) => {
      const result = await db.query<Record<string, unknown>>(
        `SELECT ${TASK_DRAFT_OPERATIONS_SELECT}
         FROM task_drafts d
      ...
      return {
        ok: true,
        draft: projectOperationsRow(result.rows[0], TASK_DRAFT_OPERATIONS_RESPONSE_KEYS),
      };
    }),
  ...
  listHustlers: operationsAdminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const result = await db.query<Record<string, unknown>>(
        `SELECT ${HUSTLER_OPERATIONS_SELECT}
      ...
      return {
        ok: true,
        hustlers: result.rows.map((row) =>
          projectOperationsRow(row, HUSTLER_OPERATIONS_RESPONSE_KEYS)),
      };
    }),
