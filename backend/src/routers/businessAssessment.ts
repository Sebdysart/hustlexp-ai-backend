...
export const businessAssessmentRouter = router({
  createPaymentIntent: protectedProcedure
    ...
    .mutation(async ({ctx,input}) => {
      const r=await db.query<any>(`SELECT a.id,a.task_draft_id,a.business_organization_id,a.assessment_fee_cents,a.assessment_platform_fee_cents,a.status FROM business_assessment_requests a JOIN task_drafts d ON d.id=a.task_draft_id WHERE a.id=$1 AND d.poster_user_id=$2 LIMIT 1`,[input.assessmentRequestId,ctx.user.id]);
      ...
      if(a.assessment_fee_cents===null||a.assessment_fee_cents<=0||a.assessment_platform_fee_cents===null) throw new TRPCError({code:'PRECONDITION_FAILED',message:'This assessment does not require payment.'});
