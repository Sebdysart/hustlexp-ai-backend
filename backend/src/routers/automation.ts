...
export const automationRouter = router({
  applyControlledTestDurationEvidence: heldAdminOrEngineBridgeProcedure
    .input(
      z
        .object({
        ...
        .superRefine((value, context) => {
          if (
            value.minimumMinutes > value.expectedMinutes ||
            value.expectedMinutes > value.maximumMinutes
          ) {
            context.addIssue({
              code: z.ZodIssueCode.custom,
              message: 'Duration estimate must remain inside its range.',
            });
          }
        })
    )
  ...
  recordControlledTestProviderCapability: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
  ...
  linkHustlerIdentity: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
  ...
  completeLocalTestScreening: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
  ...
  prepareLocalTestLiquidity: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        workerId: Schemas.uuid,
  ...
  settleLocalTestPayout: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        idempotencyKey,
    ...
    .mutation(async ({ input }) => {
      ...
      if (!row)
        throw new TRPCError({ code: 'NOT_FOUND', message: 'Task payout context not found' });
      ...
      if (!released.success) {
        if (
          released.error.code !== ErrorCodes.ESCROW_TERMINAL &&
  ...
  getBridgeTaskState: platformAdminProcedure
  ...
  expireUnfilled: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        idempotencyKey,
  ...
  expireDue: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
  ...
  recordCompletionDelivery: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
  ...
  completeUnattended: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        idempotencyKey,
  ...
  confirmPosterCompletion: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        providerConfirmationId: z
          .string()
          .trim()
          .min(8)
          .max(255)
  ...
  markWorkerTraveling: heldAdminOrEngineBridgeProcedure
    .input(z.object({ engineTaskId: Schemas.uuid }))
    .mutation(async ({ input }) => {
      ...
      if (!task.data.worker_id) {
        throw new TRPCError({
          code: 'PRECONDITION_FAILED',
          message: 'Task has no engine-reserved hustler',
        });
      }
  ...
  submitPosterRating: heldAdminOrEngineBridgeProcedure
    .input(
      z.object({
        engineTaskId: Schemas.uuid,
        providerReviewId: z
