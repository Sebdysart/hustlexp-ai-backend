...
export const escrowPaymentProcedures = {
  createPaymentIntent: posterProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const taskRow = await db.query<{ price: number | string | null }>(
      ...
      const escrowRow = await db.query<{ id: string }>(
        `SELECT id FROM escrows WHERE task_id = $1 AND state = 'PENDING' AND amount = $2
      ...
      const result = await StripeService.createPaymentIntent({
        taskId: input.taskId, posterId: ctx.user.id, escrowId, amount,
      });
