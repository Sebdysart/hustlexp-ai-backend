import { z } from 'zod';
import { heldAdminOrEngineBridgeProcedure, posterProcedure, Schemas } from '../trpc.js';
...
export const recurringControlledProcedures = {
  createControlled: posterProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      controlledResult(await createControlledTemplate(input, ctx.user.id))
    ),

  listControlled: posterProcedure
    .input(z.object({}).strict().optional())
    .query(async ({ ctx }) =>
      controlledResult(await listControlledRecurringTemplates(ctx.user.id))
    ),

  generateControlled: heldAdminOrEngineBridgeProcedure
    .input(
      z
        .object({
  ...
  recordControlledSafeguard: heldAdminOrEngineBridgeProcedure
    .input(
      z
        .object({
          seriesId: Schemas.uuid,
  ...
  recoverControlled: heldAdminOrEngineBridgeProcedure
    .input(
      z
        .object({
          seriesId: Schemas.uuid,
