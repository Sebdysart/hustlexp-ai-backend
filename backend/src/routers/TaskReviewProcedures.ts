...
} from '../services/TaskCloseService.js';
...
function reviewDecision(input: ReviewProofInput): { decision: ReviewDecision; reason?: string } {
  const decision =
    input.decision ??
    (input.approved === true ? 'ACCEPTED' : input.approved === false ? 'REJECTED' : undefined);
  if (!decision)
  ...
  if (decision === 'REJECTED' && !reason) {
    throw new TRPCError({
      code: 'BAD_REQUEST',
      message: 'A reason is required when rejecting proof',
    });
  }
...
async function resolveReviewProofId(input: ReviewProofInput, posterId: string): Promise<string> {
  ...
  const proofId =
    input.proofId ?? (input.taskId ? await latestSubmittedProofId(input.taskId) : undefined);
  if (!proofId)
...
async function loadSubmittedProof(proofId: string) {
  const state = await db.query<{ state: string }>('SELECT state FROM proofs WHERE id = $1', [
    proofId,
  ]);
...
async function verifyProofTaskContext(
  ...
  if (input.taskId && input.proofId && taskId !== input.taskId) {
    throw new TRPCError({
      code: 'FORBIDDEN',
      message: 'Proof does not belong to the specified task',
    });
  }
...
async function handleRejectedProof(
  ...
  if (task.worker_id)
    await notifyProofRejected(task.worker_id, taskId, task.title ?? 'your task', reason);
}
...
async function reviewProof(ctx: AuthedContext, input: ReviewProofInput) {
  ...
  if (!reviewed.success)
    throw new TRPCError({ code: 'BAD_REQUEST', message: reviewed.error.message });
  await handleRejectedProof(decision, proof.task_id, reason);
  await invalidateTask(proof.task_id);
  return reviewed.data;
}

function completeErrorCode(
  code: string
): 'NOT_FOUND' | 'FORBIDDEN' | 'PRECONDITION_FAILED' | 'BAD_REQUEST' {
...
async function completeTask(ctx: AuthedContext, taskId: string) {
  ...
  if (!result.success) {
    throw new TRPCError({
      code: completeErrorCode(result.error.code),
...
function cancelErrorCode(
  code: string
): 'NOT_FOUND' | 'FORBIDDEN' | 'PRECONDITION_FAILED' | 'CONFLICT' | 'BAD_REQUEST' {
  if (code === ErrorCodes.NOT_FOUND) return 'NOT_FOUND';
  if (code === ErrorCodes.FORBIDDEN) return 'FORBIDDEN';
  if (code === TASK_CANCEL_VERSION_CONFLICT_CODE) return 'CONFLICT';
  if (
    code === TASK_CANCEL_WORK_ORDER_AUTHORITY_REQUIRED_CODE ||
    code === TASK_CANCEL_UNIVERSAL_AUTHORITY_REQUIRED_CODE
  )
    return 'PRECONDITION_FAILED';
  return 'BAD_REQUEST';
}

async function cancelTask(ctx: AuthedContext, input: { taskId: string; expectedVersion: number }) {
  ...
  if (!result.success) {
    throw new TRPCError({
      code: cancelErrorCode(result.error.code),
      message: result.error.message,
    });
  }
...
export const TaskReviewProcedures = {
  ...
  cancel: posterProcedure
    .input(z.object({ taskId: Schemas.uuid, expectedVersion: z.number().int().positive() }))
    .mutation(async ({ ctx, input }) => cancelTask(ctx, input)),
};
