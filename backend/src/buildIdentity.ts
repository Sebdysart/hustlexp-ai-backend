...
function compiledArtifactEvidence(root: string): { digest: string; entryCount: number } {
...
export function compiledArtifactDigest(root: string): string {
  return compiledArtifactEvidence(root).digest;
}
...
export function readBuildIdentity(
  ...
  try {
    ...
    if (isBuildIdentity(parsed)) {
      const measuredArtifact = compiledArtifactEvidence(artifactRoot);
      return {
        ...parsed,
        artifact_verified:
