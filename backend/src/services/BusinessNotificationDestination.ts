...
export async function businessNotificationDestinations(
  ...
  const result = await query<DestinationContext>(`
    WITH refs AS (
      ...
      SELECT r.id, q.business_organization_id AS organization_id, q.task_draft_id AS draft_id,
        q.id AS quote_id, q.acquisition_origin
      FROM refs r JOIN quotes q ON r."entityType" = 'quote' AND q.id = r."entityId"
      UNION ALL
      SELECT r.id, a.business_organization_id, a.task_draft_id, NULL::uuid, NULL::text
