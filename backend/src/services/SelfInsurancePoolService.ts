...
interface InsuranceClaim {
  ...
  paid_at: Date | null;
  created_at: Date;
  stripe_transfer_id: string | null; // Historical database field; no automated payout adapter.
}
...
export const SelfInsurancePoolService = {
   ...
   * Automatic payouts are unavailable; operations must resolve manually.
   */
  payClaim: async (_claimId: string): Promise<ServiceResult<{ already_paid?: boolean; claim?: InsuranceClaim }>> => ({
    success: false,
    error: { code: 'MANUAL_COMPENSATION_REQUIRED', message: 'Insurance payouts require manual review; automated payout is unavailable.' },
  }),
