...
async function assertDisposableStorage(env: Environment = process.env): Promise<ServiceResult<void>> {
  if (!configuredDisposableStorage(env)) {
    return failure(
      ...
      'Local certification payouts require an explicitly named loopback disposable database.',
    );
  }
...
function localCertificationPayoutEvidenceReadable(env: Environment = process.env): boolean {
...
export const LocalCertificationPayoutProvider = {
  ...
  verifyPaidTransfer: async (
    ...
    if (!localCertificationPayoutEvidenceReadable() || !isLocalCertificationPayoutTransferId(params.transferId)) {
      return false;
    }
    const storage = await assertDisposableStorage();
    if (!storage.success) return false;
