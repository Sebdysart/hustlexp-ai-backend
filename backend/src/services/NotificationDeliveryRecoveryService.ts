 ...
 */
export async function reconcileNotificationProviderReceipts(
  limit = 100,
  database: Database = db,
): Promise<number> {
  const boundedLimit = Math.max(1, Math.min(Number(limit) || 100, 100));
  const candidates = await database.query<ProviderReceiptCandidate>(
    `SELECT * FROM (
       SELECT 'email'::TEXT AS channel,email.id AS channel_outbox_id,
              ...
              email.idempotency_key,email.task_completion_notice_request_id
       ...
       WHERE email.notification_id IS NULL
         ...
         AND (
           email.status<>'sent'
           OR outbox.status<>'processed'
           OR (
             email.task_completion_notice_request_id IS NOT NULL
             AND NOT EXISTS (
               SELECT 1
                 FROM task_completion_delivery_events completion_delivery
                WHERE completion_delivery.completion_notice_request_id =
                      email.task_completion_notice_request_id
             )
           )
         )
       UNION ALL
       SELECT 'sms'::TEXT AS channel,sms.id AS channel_outbox_id,
              ...
              sms.idempotency_key,NULL::UUID AS task_completion_notice_request_id
       FROM sms_outbox sms
       JOIN notification_deliveries delivery
     ...
     ORDER BY (task_completion_notice_request_id IS NOT NULL) DESC,
              notification_id NULLS LAST,channel,channel_outbox_id
     LIMIT $1`,
    [boundedLimit],
  );
  ...
  for (const candidate of candidates.rows) {
    try {
      ...
      const committed = await database.transaction(async (query) => {
        if (candidate.channel === 'email') {
          const proof = await query<{
            ...
            `SELECT email.id AS channel_outbox_id,outbox.id AS outbox_id,
             ...
             WHERE email.id=$1
               ...
               AND email.task_completion_notice_request_id IS NOT DISTINCT FROM $5::UUID
               AND (
                 email.status<>'sent'
                 OR outbox.status<>'processed'
                 OR (
            ...
            [
              ...
              candidate.task_completion_notice_request_id,
            ],
          );
          ...
          if (exact.task_completion_notice_request_id) {
            await query(
      ...
      log.warn(
        {
          ...
          channelOutboxId: candidate.channel_outbox_id,
          taskCompletionNoticeRequestId: candidate.task_completion_notice_request_id,
          error: error instanceof Error ? error.message : String(error),
        },
        'Provider receipt reconciliation candidate deferred',
      );
    }
  }
  return reconciled;
...
export async function reconcileTerminalNotificationOutboxEvents(limit = 100): Promise<number> {
  const boundedLimit = Math.max(1, Math.min(Number(limit) || 100, 100));
  // Completion notices have no notification_deliveries aggregate. Recover
  // their exact terminal email/outbox pair first and independently so a
  // saturated or malformed generic notification cohort cannot starve this
  // causal seam. This repair deliberately ignores stale BullMQ dispatch
  // metadata: the immutable request/email/outbox envelope and terminal email
  // truth are the authority after the worker transaction commits.
  const completionCandidates = await db.query<{ outbox_id: string }>(
    `SELECT outbox.id AS outbox_id
     FROM outbox_events outbox
     JOIN email_outbox email
       ON email.id=outbox.aggregate_id
      AND email.idempotency_key=outbox.idempotency_key
     JOIN task_completion_notice_requests notice_request
       ON notice_request.id=email.task_completion_notice_request_id
      AND notice_request.email_idempotency_key=outbox.idempotency_key
     WHERE outbox.status IN ('pending','enqueued','processing','failed')
       ...
       AND (
         (
           ...
           AND NULLIF(BTRIM(email.suppressed_reason),'') IS NOT NULL
           AND email.suppressed_at IS NOT NULL
           ...
           AND email.provider_name IS NULL
           AND email.provider_receipt_at IS NULL
         )
         OR (
           email.status='provider_outcome_unknown'
           AND email.provider_io_started_at IS NOT NULL
           AND email.notification_provider_attempt_id IS NOT NULL
           AND email.pre_provider_claim_id IS NOT NULL
           AND email.pre_provider_claimed_at IS NOT NULL
           AND email.pre_provider_claim_deadline_at IS NOT NULL
           AND NULLIF(BTRIM(email.provider_msg_id),'') IS NULL
           AND email.provider_name IS NULL
           AND email.provider_receipt_at IS NULL
         )
         OR (
           email.status='sent'
           AND email.provider_name='smtp_sink'
           AND NULLIF(BTRIM(email.provider_msg_id),'') IS NOT NULL
           AND email.provider_receipt_at IS NOT NULL
           AND EXISTS (
             SELECT 1
             FROM task_completion_delivery_events delivery
             WHERE delivery.completion_notice_request_id=notice_request.id
               ...
               AND delivery.expected_completion_fact_id=
                   notice_request.submitted_completion_fact_id
  ...
  let completionReconciled = 0;
  for (const candidate of completionCandidates.rows) {
    try {
      const committed = await db.transaction(async (query) => {
        const result = await query<{ id: string }>(
          `UPDATE outbox_events outbox
           SET status='processed',processed_at=COALESCE(processed_at,clock_timestamp()),
      ...
      if (committed) completionReconciled += 1;
    } catch (error) {
      log.warn(
        {
        ...
        'Completion notice terminal outbox reconciliation candidate deferred',
      );
    }
  }
  ...
  return completionReconciled
    + (Number.isSafeInteger(reconciled) && reconciled >= 0 ? reconciled : 0);
}
 ...
 */
export async function promoteExpiredDirectProviderClaims(
  limit = 100,
  database: Database = db
): Promise<number> {
  const boundedLimit = Math.max(1, Math.min(Number(limit) || 100, 100));
  const completionNoticePromoted = await database.transaction(async (query) => {
    const candidates = await query<{
      ...
      `SELECT email.id AS email_id,outbox.id AS outbox_id,
       ...
       JOIN outbox_events outbox
        ...
        AND outbox.aggregate_id=email.id
       WHERE email.notification_id IS NULL
         AND email.task_completion_notice_request_id IS NOT NULL
         AND email.status='sending'
         AND email.provider_io_started_at IS NOT NULL
         AND email.notification_provider_attempt_id IS NOT NULL
         AND email.pre_provider_claim_deadline_at <= clock_timestamp()
         AND NULLIF(BTRIM(email.provider_msg_id),'') IS NULL
         AND outbox.status IN ('pending','enqueued','processing','failed')
       ORDER BY email.pre_provider_claim_deadline_at,email.id
       LIMIT $1
       FOR UPDATE OF email,outbox SKIP LOCKED`,
      [boundedLimit],
    );

    let promoted = 0;
    for (const candidate of candidates.rows) {
      const unknown = await query<{ id: string }>(
        `UPDATE email_outbox
         ...
         WHERE id=$1
           AND notification_id IS NULL
           AND task_completion_notice_request_id IS NOT NULL
           AND status='sending'
           AND provider_io_started_at IS NOT NULL
           AND notification_provider_attempt_id=$2::UUID
           AND pre_provider_claim_deadline_at <= clock_timestamp()
           AND NULLIF(BTRIM(provider_msg_id),'') IS NULL
         RETURNING id`,
        [candidate.email_id, candidate.provider_attempt_id],
      );
      if (!unknown.rows[0]) continue;

      const closed = await query<{ id: string }>(
        `UPDATE outbox_events
         SET status='processed',processed_at=COALESCE(processed_at,clock_timestamp()),
             error_message='provider_outcome_unknown',updated_at=clock_timestamp()
         WHERE id=$1
           AND idempotency_key=$2
           AND event_type='email.send_requested'
           AND aggregate_type='email'
           AND aggregate_id=$3
           AND status IN ('pending','enqueued','processing','failed')
         RETURNING id`,
        [candidate.outbox_id, candidate.idempotency_key, candidate.email_id],
      );
      if (!closed.rows[0]) {
        throw new Error('Completion notice provider expiry lost exact outbox authority while locked');
      }
      promoted += 1;
    }
    return promoted;
  ...
  const result = await database.transaction(async (query) => query<{ promoted: string | number }>(
  ...
  return completionNoticePromoted
    + (Number.isSafeInteger(promoted) && promoted >= 0 ? promoted : 0);
}
