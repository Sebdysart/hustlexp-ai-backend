...
import type { ProviderFinancialObservationState } from './SyntheticFinancialCommandSchemas.js';

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu;
const WORKER_ID = /^[A-Za-z0-9][A-Za-z0-9:._-]{2,127}$/u;
const DETAIL_CODE = /^[A-Z][A-Z0-9:_-]{2,127}$/u;
const PROVIDER_OBSERVATION_STATES = new Set<ProviderFinancialObservationState>([
  'UNKNOWN', 'PENDING', 'RETRYABLE_FAILURE', 'SUCCEEDED', 'DECLINED', 'FAILED',
  'VOIDED', 'REFUNDED', 'PARTIALLY_REFUNDED', 'REVERSED',
]);

export const SYNTHETIC_PROVIDER_EVENT_KIND = 'FINANCIAL_OPERATION_OBSERVED';
...
export function assertProviderEventSuccess(result: ProviderEventProcessingSuccess): void {
  if (
    ...
    || result.version < 0
    || !PROVIDER_OBSERVATION_STATES.has(result.state)
