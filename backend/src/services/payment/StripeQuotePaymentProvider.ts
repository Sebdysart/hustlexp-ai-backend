...
import { getSharedStripe } from '../../lib/stripe-client.js';
...
function terminalRefundResult(
  ...
  | { success: true; data: RecoverLegacyQuoteFinancialSecurityResult }
  | ReturnType<typeof recoveryFailure> {
  if (refund.amount !== amountCents) {
    return recoveryFailure(
      'PAYMENT_REFUND_AMOUNT_MISMATCH',
      'Refund amount does not match quote amount'
    );
  }
...
export const StripeQuotePaymentRecoveryProvider: LegacyQuotePaymentRecoveryPort = {
  persistedProvider: 'stripe',

  async recoverOrphanPayment(input) {
    try {
      const stripe = getSharedStripe();
      if (!stripe) {
        return recoveryFailure(
          'PAYMENT_PROVIDER_NOT_CONFIGURED',
          'Historical payment recovery provider is not configured'
        );
      }
      const paymentIntent = await stripeBreaker.execute(() =>
        stripe.paymentIntents.retrieve(input.externalReference)
      );
      ...
      if (paymentIntent.status === 'succeeded') {
        ...
        if (refundedAmount === input.amountCents) {
          const reconciledRefund =
            existingRefund ?? existingRefunds.data.find((refund) => refund.status === 'succeeded');
          if (!reconciledRefund) {
...
/** @deprecated Use the recovery-only export above. */
export const StripeQuotePaymentProvider = StripeQuotePaymentRecoveryProvider;
