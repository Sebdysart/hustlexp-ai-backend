...
import type {
  ...
  FinancialProviderCommandActorKind,
  ...
  FinancialProviderCommandReleaseAuthenticationStatus,
  FinancialProviderCommandReleaseEnvironment,
...
export interface FinancialProviderCommandRecoveryCommand {
  ...
  readonly recordedActorId: string | null;
  readonly recordedActorKind: FinancialProviderCommandActorKind | null;
  readonly releaseManifestDigest: string | null;
  readonly releaseId: string | null;
  readonly releaseRevision: string | null;
  readonly releaseEnvironment: FinancialProviderCommandReleaseEnvironment | null;
  readonly releaseAuthenticationStatus: FinancialProviderCommandReleaseAuthenticationStatus | null;
...
export interface ForegroundFinancialProviderCommandRepository extends Pick<
...
interface RecoveryCommandRow {
  ...
  recorded_actor_kind: FinancialProviderCommandActorKind | null;
  release_manifest_digest: string | null;
  release_id: string | null;
  release_revision: string | null;
  release_environment: FinancialProviderCommandReleaseEnvironment | null;
  release_authentication_status: FinancialProviderCommandReleaseAuthenticationStatus | null;
...
function commandFromRow(row: RecoveryCommandRow): FinancialProviderCommandRecoveryCommand {
  ...
  return {
    ...
    currency: row.currency,
    recordedActorId: row.recorded_actor_id,
    recordedActorKind: row.recorded_actor_kind,
    releaseManifestDigest: row.release_manifest_digest,
    releaseId: row.release_id,
    releaseRevision: row.release_revision,
    releaseEnvironment: row.release_environment,
    releaseAuthenticationStatus: row.release_authentication_status,
    ...
    return this.database.transaction(async (query) => {
      const commandResult = await query<RecoveryCommandRow>(
        `SELECT command_id, operation_kind, operation_id, provider_kind,
                ...
                recorded_actor_id, recorded_actor_kind,
      ...
      return {
        ...
        lastDispatchAttempt: attemptResult.rows[0]
          ...
          : null,
        latestOutcome: outcomeResult.rows[0] ? outcomeFromRow(outcomeResult.rows[0], false) : null,
      };
    });
  }
    ...
    return this.database.transaction(async (query) => {
      const candidates = await query<CandidateRow>(
        `SELECT command.command_id, command.operation_kind, command.operation_id,
                ...
                command.currency, command.recorded_actor_id,
                command.recorded_actor_kind, command.release_manifest_digest,
                command.release_id, command.release_revision,
                command.release_environment,
                command.release_authentication_status,
                command.recorded_at AS requested_at,
...
const PRE_WORK_ORDER_FAKE_OPERATION_KINDS = new Set<FinancialOperationKind>([
  ...
  'AUTHORIZE',
  'SECURE',
  // Migration 131 permits only the exact claimed pre-WorkOrder compensation
  // VOID derived from the committed WorkOrder witness and SECURE bridge.
  'VOID',
]);
const WORK_ORDER_REQUIRED_FAKE_OPERATION_KINDS = new Set<FinancialOperationKind>([
  // Migration 132 binds ADJUST to one committed price-amendment witness.
  'ADJUST',
  // Migration 133 permits only the exact WorkOrder-bound terminal reversal
  // derived from a successful ADJUST and immutable recovery witness.
  'REVERSAL',
  'CAPTURE',
  'REFUND',
  'SETTLE',
...
function sameProviderProjection(
  left: FinancialOperationResult,
  right: FinancialOperationResult
): boolean {
  return (
    financialProviderOutcomeProjectionSha256(left) ===
      financialProviderOutcomeProjectionSha256(right) &&
 ...
 */
export class DurableFakeFinancialProviderCommandCoordinator implements ForegroundFinancialProviderCommandCoordinator {
  ...
  private async dispatch<TRequest, TResult>(
    ...
    try {
      ...
      if (reconciled) {
        ...
        return this.coordinatedResult(
          context,
          attempt,
          outcome,
          reconciled
        ) as unknown as ForegroundFinancialProviderCommandResult<TResult>;
      }
