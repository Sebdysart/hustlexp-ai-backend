...
export const providerFinancialObservationStateSchema = z.enum([
  'UNKNOWN',
  'PENDING',
  'RETRYABLE_FAILURE',
  'SUCCEEDED',
...
/**
 * Provider observations are facts, never commands. In particular this closed
 * envelope has no participant, task, actor, idempotency, scenario, or
 * lifecycle-version field that a provider could use to claim domain authority.
 */
export const syntheticFinancialObservationSchema = z
  .object({
    version: z.literal(SYNTHETIC_FINANCIAL_OBSERVATION_VERSION),
    kind: z.literal(SYNTHETIC_FINANCIAL_OBSERVATION_KIND),
    providerKind: z.literal('FAKE'),
  ...
  .strict()
  .superRefine((value, context) => {
    if (value.observedProviderVersion !== value.predecessorProviderVersion + 1) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['observedProviderVersion'],
        message: 'PROVIDER_OBSERVATION_VERSION_NOT_CAUSAL',
      });
    }
    ...
    if (
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['amountCents'],
        message: 'PROVIDER_OBSERVATION_VALUE_SHAPE_INVALID',
      });
    }
  });
