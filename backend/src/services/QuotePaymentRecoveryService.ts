...
} from './payment/LegacyQuotePaymentRecoveryPort.js';
import {
  isSupportedLegacyQuotePaymentProvider,
  resolveQuotePaymentRecoveryProvider,
} from './payment/QuotePaymentRecoveryProviderResolver.js';
...
function failure<T>(code: string, message: string): Extract<ServiceResult<T>, { success: false }> {
...
function ineligiblePayment(
  ...
  if (payment.poster_email.trim().toLowerCase() !== payment.lead_email.trim().toLowerCase()) {
    return failure(
      'QUOTE_PAYMENT_POSTER_MISMATCH',
      'This quote payment does not belong to the authenticated poster.'
    );
  }
  if (!isSupportedLegacyQuotePaymentProvider(payment.provider)) {
    return failure(
      'QUOTE_PAYMENT_PROVIDER_MISMATCH',
      'Quote payment recovery does not support the exact recorded provider.'
    );
  }
...
async function auditBackedReplay(
  ...
  const expectedAction =
    payment.status === 'FAILED' ? 'VOIDED' : payment.status === 'REFUNDED' ? 'REFUNDED' : null;
  if (
    !expectedAction ||
  ...
  return {
    success: true,
    data: resultFromTerminal(input, payment.status as 'FAILED' | 'REFUNDED', expectedAction, true),
  };
}
...
async function finalizeRecovery(
  ...
  return db.transaction(async (query) => {
    ...
    if (paymentFailure) {
      return markReconciliationRequired(query, claim, input, recovered, payment.status);
    }

    const nextStatus: 'FAILED' | 'REFUNDED' =
      recovered.disposition === 'VOIDED' ? 'FAILED' : 'REFUNDED';
    if (
      (payment.status !== claim.payment.status ||
        payment.provider !== claim.payment.provider ||
        payment.provider_payment_id !== claim.payment.provider_payment_id ||
        Number(payment.amount_cents) !== Number(claim.payment.amount_cents) ||
...
export async function recoverOrphanQuotePayment(
  ...
  try {
    ...
    const recoveryProvider =
      provider ?? resolveQuotePaymentRecoveryProvider(claim.payment.provider);
    if (recoveryProvider.persistedProvider !== claim.payment.provider) {
      return failure(
        'QUOTE_PAYMENT_PROVIDER_MISMATCH',
        'Quote payment recovery adapter does not match the recorded provider.'
      );
    }

    const recovered = await recoveryProvider.recoverOrphanPayment({
      quoteId: input.quoteId,
      quoteVersionId: input.quoteVersionId,
      posterId: input.posterId,
      externalReference: input.paymentIntentId,
