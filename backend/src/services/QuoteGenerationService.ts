...
export class QuoteGenerationService {
   ...
   *   -> automated quote
   */
  private static assertConfidence(confidence: SupplyConfidence): void {
  ...
  static async generateForDraft(
    ...
    return db.transaction(async (query) => {
      ...
      const priceBook = await this.loadPriceBook(query, draft.category);

      this.assertPriceBookUsable(priceBook, environment);

      const pricing = await this.calculatePriceBook(query, draft, priceBook, environment);

      const confidence = await this.reconcileSupplyConfidence(
        query,
        draft,
        scope.answers,
        pricing,
      ...
      if (!record) {
        return this.previewQuote(draft, priceBook, pricing, confidence);
      }
  ...
  private static async loadDraft(
    ...
    if (!draft) {
      throw new QuoteGenerationError('NOT_FOUND', 'Task draft not found.');
    }
  ...
  private static assertDraftEligible(draft: TaskDraftRow): void {
    if (!draft.lead_id) {
      throw new QuoteGenerationError('BLOCKED_TASK_NO_LEAD', 'Task draft is not linked to a lead.');
    }

    if (draft.status === 'abandoned') {
      throw new QuoteGenerationError('TASK_CLOSED', 'Task draft is closed.');
    }
  }
  ...
  private static validateScope(draft: TaskDraftRow): {
    ...
    const answers = isObject(structured.answers) ? structured.answers : {};

    const missingQuestions = Array.isArray(structured.missing_questions)
      ? structured.missing_questions
      : null;

    const riskFlags = Array.isArray(structured.risk_flags) ? structured.risk_flags : null;
    ...
    const preferredWindow = String(answers.preferred_window ?? '');

    if (!['today_or_tomorrow', 'this_week', 'next_week', 'flexible'].includes(preferredWindow)) {
      throw new QuoteGenerationError('BLOCKED_SCOPE_EVIDENCE', 'Task preferred window is invalid.');
    }

    const includedWork = Array.isArray(answers.included_work) ? answers.included_work : [];

    const excludedWork = Array.isArray(answers.excluded_work) ? answers.excluded_work : [];

    const safetyRestrictions = Array.isArray(answers.safety_restrictions)
    ...
    const scope: JsonObject = {
      policy_version: String(answers.scope_policy_version ?? 'task_scope_v1'),
  ...
  private static async calculatePriceBook(
    ...
    const structured = isObject(draft.structured) ? draft.structured : {};
    ...
    const requiredVehicle = String(answers.required_vehicle ?? '');
    ...
    const requiredWorkerCountRaw = String(answers.required_worker_count ?? '');
    ...
    if (
      ...
      !['today_or_tomorrow', 'this_week', 'next_week', 'flexible'].includes(preferredWindow) ||
      !['none', 'any_vehicle', 'cargo_vehicle'].includes(requiredVehicle)
    ) {
      throw new QuoteGenerationError(
        'BLOCKED_SCOPE_EVIDENCE',
    ...
    const tools = [
      ...new Set(
        answers.required_tools.filter((value): value is string => typeof value === 'string')
      ),
    ].sort();
    ...
    if (tools.some((tool) => !allowedTools.has(tool))) {
      throw new QuoteGenerationError('BLOCKED_SCOPE_EVIDENCE', 'Task requires unsupported tools.', {
        tools,
      });
    }
    ...
    const travelPremiumCents = Math.ceil(
      Math.max(0, Number(travelMiles) - Number(book.included_travel_miles)) *
        Number(book.travel_premium_cents_per_mile)
    );

    const equipmentPremiumCents = tools.length * Number(book.equipment_premium_cents);

    const vehiclePremiumCents =
      requiredVehicle === 'cargo_vehicle' ? Number(book.cargo_vehicle_premium_cents) : 0;

    let scopeAddonCount = 0;

    if (draft.category === 'yard' && answers.debris === 'haul_away') {
      scopeAddonCount += 1;
    }

    if (
      draft.category === 'yard' &&
      ['true', 'yes', '1'].includes(String(answers.pressure_washing ?? '').toLowerCase())
    ) {
      scopeAddonCount += 1;
    }
    ...
    if (
      ...
      ['true', 'yes', '1'].includes(String(answers.old_item_removal ?? '').toLowerCase())
    ) {
      scopeAddonCount += 1;
    }

    if (draft.category === 'moving' && answers.move_type === 'transport') {
      scopeAddonCount += 1;
    }

    if (draft.category === 'moving' && answers.size_weight === 'medium') {
      scopeAddonCount += 1;
    }

    const scopeAddonPremiumCents = scopeAddonCount * Number(book.scope_addon_premium_cents);
    ...
    const urgencyPremiumCents =
      preferredWindow === 'today_or_tomorrow'
        ? Math.ceil((baseBeforeUrgency * Number(book.same_day_premium_bps)) / 10000)
        : 0;

    const rawTotal = baseBeforeUrgency + urgencyPremiumCents;

    // Match:
    // greatest(price_min_cents, ceil(raw_total / 100) * 100)
    const totalCents = Math.max(Number(book.price_min_cents), Math.ceil(rawTotal / 100) * 100);
    ...
    const marginFloorBps = Math.round(Number(book.platform_margin_floor_pct) * 100);

    const payoutCents = Math.floor((totalCents * (10000 - marginFloorBps)) / 10000);

    const marginCents = totalCents - payoutCents;

    if (
      payoutCents < Number(book.min_hustler_payout_cents) ||
      marginCents * 10000 < totalCents * marginFloorBps
    ) {
      throw new QuoteGenerationError(
    ...
    const decision = {
      ...
      urgency_premium_bps:
        preferredWindow === 'today_or_tomorrow' ? Number(book.same_day_premium_bps) : 0,
      provider_floor_cents: Number(book.min_hustler_payout_cents),
      expected_customer_low_cents: Number(book.price_min_cents),
      expected_customer_high_cents: Number(book.price_max_cents),
      customer_maximum_cents: Number(book.price_cap_cents),
      ...
      quote_expires_hours: Number(book.quote_expires_hours),
      refund_policy_version: book.refund_policy_version,
      calibration_state: book.automation_evidence_state,
      completed_paid_task_count: Number(book.completed_paid_task_count),
    };
    ...
    return {
      ...
      quoteExpiresHours: Number(book.quote_expires_hours),
      refundPolicyVersion: book.refund_policy_version,
      ...
      minHustlerPayoutCents: Number(book.min_hustler_payout_cents),
  ...
  private static async reconcileSupplyConfidence(
    ...
    const requiredWorkers = resolveRequiredWorkers(draft, answers);

    const requiredVehicle = resolveRequiredVehicle(draft, answers);

    const requiredTools = resolveRequiredTools(draft, answers);
    ...
    for (const candidate of candidates) {
      ...
      const workerBaseEligible =
        ...
        !(
          ...
          (candidate.trust_hold_until === null || candidate.trust_hold_until > new Date())
        );
      ...
      const softAvailableCurrent =
        blockers.length === 0 &&
        (await this.currentSoftAvailability(query, candidate.id, draft, environment));
    ...
    const softCount = evaluations.filter((e) => e.softAvailableCurrent).length;
    ...
    const blockers =
      state === 'CONFIDENT'
        ? []
        : buildSupplyBlockers(evaluations, requiredWorkers, candidateCount);

    const validUntil = new Date(Date.now() + 10 * 60 * 1000);

    await this.replaceCandidateEvaluations(query, draft.id, evaluations, inputFingerprint);
    ...
    await query(
      ...
      [
        ...
        pricing.marginCents,
        inputFingerprint,
        state === 'CONFIDENT' ? 'GENERATE_POLICY_QUOTE' : 'AUTOMATIC_SUPPLY_RECOVERY',
        validUntil,
      ]
    );
  ...
  private static async createOrRepairQuote(
    ...
    const timing = computeQuoteTiming(
      ...
      String(
        isObject(draft.structured?.answers) ? draft.structured.answers.preferred_window : 'flexible'
      )
    );

    const idempotencyKey = `autoquote:${draft.id.toLowerCase()}:v1`;
    ...
    if (!quoteId) {
      throw new QuoteGenerationError('QUOTE_CREATE_FAILED', 'Failed to create quote.');
    }
  ...
  private static async loadMinimumTrustTier(
    ...
    return Math.max(1, Number(result.rows[0]?.min_trust_tier ?? 1));
  }
  ...
  private static async areaMatches(
    ...
    const workerState = String(worker.location_state ?? '')
      .trim()
      .toUpperCase();

    const taskState = String(taskRegion ?? '')
      .trim()
      .toUpperCase();
     ...
     */
    const workerCity = String(worker.location_city ?? '')
      .trim()
      .toLowerCase();
  ...
  private static async windowMatches(
    ...
    return ['today_or_tomorrow', 'this_week', 'next_week', 'flexible'].includes(preferredWindow);
  }
  ...
  private static async hasActiveCommitment(
    ...
    const result = await query<{ ok: boolean }>(
      ...
      `,
      [hustlerId, environment === 'TEST', draftId]
    );
...
function scopeConfirmed(answers: JsonObject): boolean {
  const value = typeof answers.scope_confirmed_at === 'string' ? answers.scope_confirmed_at : '';
  ...
  return Number.isFinite(time) && time <= Date.now() + 300_000;
}
...
function resolveRequiredWorkers(draft: TaskDraftRow, answers: JsonObject): number {
  const raw = String(answers.required_worker_count ?? '');
  ...
  if (
    ...
    ['heavy', 'very_heavy'].includes(String(answers.size_weight ?? ''))
  ) {
    return 2;
  }
...
function resolveRequiredVehicle(
  ...
  if (explicit === 'none' || explicit === 'any_vehicle' || explicit === 'cargo_vehicle') {
    return explicit;
  }

  if (
    (draft.category === 'moving' && answers.move_type === 'transport') ||
    (draft.category === 'yard' && answers.debris === 'haul_away')
  ) {
  ...
  if (
    ...
    ['true', 'yes', '1'].includes(String(answers.vehicle_needed ?? '').toLowerCase())
  ) {
    return 'any_vehicle';
  }
...
function resolveRequiredTools(draft: TaskDraftRow, answers: JsonObject): string[] {
  ...
  if (
    draft.category === 'yard' &&
    !['true', 'yes', '1'].includes(String(answers.equipment_provided ?? '').toLowerCase())
  ) {
...
function resolveMinimumTrust(answers: JsonObject): number {
  return answers.trust_requirement === 'home_entry' || answers.indoor_outdoor === 'indoor' ? 2 : 1;
}
...
function buildSupplyBlockers(
  ...
  if (evaluations.every((e) => e.blockers.includes('BLOCKED_CATEGORY'))) {
    blockers.push('BLOCKED_CATEGORY_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_AREA'))) {
    blockers.push('BLOCKED_AREA_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_WINDOW'))) {
    blockers.push('BLOCKED_WINDOW_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_TOOLS'))) {
    blockers.push('BLOCKED_TOOL_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_VEHICLE'))) {
    blockers.push('BLOCKED_VEHICLE_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_MIN_PAYOUT'))) {
    blockers.push('BLOCKED_PAYOUT_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_TRUST'))) {
    blockers.push('BLOCKED_TRUST_SUPPLY');
  }

  if (evaluations.every((e) => e.blockers.includes('BLOCKED_CONSENT'))) {
    blockers.push('BLOCKED_CONSENT_SUPPLY');
  }

  if (evaluations.some((e) => e.blockers.includes('BLOCKED_ACTIVE_COMMITMENT'))) {
    blockers.push('BLOCKED_COMMITTED_SUPPLY');
  }

  if (evaluations.some((e) => e.blockers.includes('BLOCKED_ENVIRONMENT_MISMATCH'))) {
...
function computeQuoteTiming(
  ...
  const [startHours, endHours] = windows[preferredWindow] ?? windows.flexible;

  const arrivalStart = new Date(now + startHours * 60 * 60 * 1000);

  const arrivalEnd = new Date(now + endHours * 60 * 60 * 1000);

  const dispatchExpires = new Date(
    arrivalStart.getTime() - dispatchExpiresHoursBeforeWindow * 60 * 60 * 1000
  );

  const requestedQuoteExpiry = new Date(now + quoteExpiresHours * 60 * 60 * 1000);

  const quoteExpires = new Date(
    Math.min(requestedQuoteExpiry.getTime(), dispatchExpires.getTime() - 60 * 60 * 1000)
  );
