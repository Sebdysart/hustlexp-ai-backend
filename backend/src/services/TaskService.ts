...
export const TaskService = {
  ...
  getByPoster: async (
    ...
    try {
      const result = cursor
        ? await db.query<Task>(
            `SELECT t.*, COALESCE(pu.name, pu.email) AS poster_name, COALESCE(pu.rating, 5.0) AS poster_rating, COALESCE(wu.name, wu.email) AS worker_name
  ...
  listOpen: async (options: {
    ...
    try {
      let sql = `
        SELECT t.*, COALESCE(pu.name, pu.email) AS poster_name, COALESCE(pu.rating, 5.0) AS poster_rating
        FROM tasks t
        LEFT JOIN users pu ON pu.id = t.poster_id
        WHERE t.state = 'OPEN'
      `;
