...
interface DeliveryContextRow {
  ...
  completion_notice_request_exists: boolean;
}
    ...
    `SELECT work_order.id AS work_order_id,
            ...
            EXISTS (
              SELECT 1
               ...
               WHERE notice_request.work_order_id = work_order.id
                 AND notice_request.submitted_completion_fact_id = completion.id
            ) AS completion_notice_request_exists,
            EXISTS (
              SELECT 1
...
function assertDeliveryContext(
  ...
  if (context.completion_notice_request_exists) {
    throw new UniversalV1CompletionDeliveryError(
      'COMPLETION_DELIVERY_CONTEXT_UNAVAILABLE',
      'This completion uses the durable notice outbox and rejects legacy callback writers.'
    );
  }
  if (
