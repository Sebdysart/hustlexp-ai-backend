...
async function startManualBusinessWorkTransaction(
  query: QueryFn,
  taskId: string,
  actorId: string,
): Promise<ServiceResult<Task>> {
  const lock = await query<{
    ...
    SELECT
      ...
      EXISTS (
        SELECT 1
        FROM task_scope_change_proposals p
  ...
  const membership = await query<{ id: string }>(
    ...
    LIMIT 1
    `,
    [task.business_fulfiller_organization_id, actorId],
  );
  ...
  if (!membership.rows[0]) {
    return {
      ...
      error: {
        ...
        message: 'Only the active Business owner can start manual Business work.',
      },
    };
  }
  ...
  if (task.state !== 'ACCEPTED') {
    return {
      ...
      error: {
        code: ErrorCodes.INVALID_STATE,
        message: `Task must be ACCEPTED before work starts. Current state: ${task.state}`,
      },
    };
  }
  ...
  if (task.started_at && task.progress_state === 'WORKING') {
    const existing = await query<Task>(
  ...
  if ((update.rowCount ?? 0) === 0) {
    return {
      ...
      error: {
        code: ErrorCodes.INVALID_STATE,
        message: 'Task changed before manual Business work could be started.',
      },
    };
  }
  ...
  await writeToOutbox({
    ...
    aggregateId: taskId,
    eventVersion: 1,
    idempotencyKey: `task.progress_updated:${taskId}:ACCEPTED:WORKING`,
    payload: {
      taskId,
      from: 'ACCEPTED',
      to: 'WORKING',
...
export const TaskExecutionService = {
  ...
  startManualBusinessWork: async (
    taskId: string,
    actorId: string,
  ): Promise<ServiceResult<Task>> => {
    try {
      return await db.transaction((query) =>
        startManualBusinessWorkTransaction(query, taskId, actorId),
      );
    } catch (error) {
      log.error(
        { err: error instanceof Error ? error.message : String(error) },
        'Manual Business task start failed',
      );
