...
import {
  ...
  freezeChangeOrderHistory,
  type ChangeOrderHistory,
  type ChangeOrderHistoryPayload,
...
const receiptSchema = z
  .object({
    history: ChangeOrderHistorySchema.nullable(),
    actor_user_id: uuid,
...
function refuse(reason: string): never {
  throw new Error('CHANGE_ORDER_HISTORY_' + reason);
}

export interface UniversalV1ChangeOrderHistoryReader {
  read(
    payload: ChangeOrderHistoryPayload,
    actor: string,
    attestation: UniversalV1ActorAttestationHandle
  ): Promise<Readonly<ChangeOrderHistory> | null>;
}

export class PostgresUniversalV1ChangeOrderHistoryReader implements UniversalV1ChangeOrderHistoryReader {
  constructor(
  ...
  async read(
    raw: ChangeOrderHistoryPayload,
    actor: string,
    attestation: UniversalV1ActorAttestationHandle
  ): Promise<Readonly<ChangeOrderHistory> | null> {
    try {
      const payload = Object.freeze(ChangeOrderHistoryPayloadSchema.parse(raw));
      ...
      if (
        !parsed.success ||
        parsed.data.command_kind !== 'READ_FAKE_CHANGE_ORDER_HISTORY' ||
      ...
      const history = await this.database.transaction(async (query) => {
        ...
        if (
          ...
          (receipt.data.history !== null &&
            !changeOrderHistoryMatchesPayload(receipt.data.history, payload, expectedActor))
        )
          return refuse('RECEIPT_BINDING_MISMATCH');
        const found = receipt.data.history;
        if (
          found &&
          found.phase.context.adjustmentOperationId !==
            deterministicUuid(payload.idempotency_key, 'adjust')
        )
          return refuse('RECEIPT_BINDING_MISMATCH');
        return found === null ? null : freezeChangeOrderHistory(found);
      });
      ...
      if (
        ...
        /^CHANGE_ORDER_HISTORY_(ASSERTION_INVALID|RECEIPT_INVALID|RECEIPT_BINDING_MISMATCH|RELEASE_AUTHORITY_CHANGED)$/u.test(
          error.message
        )
      )
        throw error;
