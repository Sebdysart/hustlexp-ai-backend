...
const ROUTE_CONTEXT_EVIDENCE_KEYS = [
...
interface EstimateAcceptanceContextRow {
  ...
  prior_category_snapshot: string | null;
  prior_service_cell_snapshot: string | null;
  prior_service_cell_authority_id: string | null;
  prior_route_evidence: Record<string, unknown>;
  provider_estimate_submission_id: string;
  quote_id: string;
  quote_version_id: string;
  quote_kind: string;
...
function preservedRouteContextEvidence(
  evidence: Record<string, unknown>,
): Record<string, unknown> {
  const preserved: Record<string, unknown> = {};
  for (const key of ROUTE_CONTEXT_EVIDENCE_KEYS) {
    if (Object.prototype.hasOwnProperty.call(evidence, key)) {
      preserved[key] = evidence[key];
    }
  }
  return preserved;
}
...
async function loadAcceptanceContext(
  ...
  const result = await query<EstimateAcceptanceContextRow>(
    `SELECT draft.id AS task_draft_id, draft.poster_user_id, draft.task_id,
            ...
            route.category_snapshot AS prior_category_snapshot,
            route.service_cell_snapshot AS prior_service_cell_snapshot,
            route.service_cell_authority_id AS prior_service_cell_authority_id,
            route.evidence AS prior_route_evidence,
...
async function appendFulfillmentCandidateRoute(
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO task_routing_decisions (
       ...
       $6, $7, $8, $9, 'DETERMINISTIC_POLICY', NULL, $10::jsonb, $11
     )
     RETURNING id`,
    [
      ...
      context.prior_category_snapshot,
      context.prior_service_cell_snapshot,
      context.prior_service_cell_authority_id,
      JSON.stringify({
        ...preservedRouteContextEvidence(context.prior_route_evidence),
