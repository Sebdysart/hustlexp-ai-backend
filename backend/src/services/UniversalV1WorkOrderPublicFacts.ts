...
export class PostgresUniversalV1WorkOrderPublicFactReader {
  ...
  async interest(actor: string, taskId: string): Promise<ProviderInterestContext | null> {
    const result = await this.query<ProviderInterestContext>(
      ...
      WHERE t.id=$1 AND t.state='OPEN' AND t.universal_contract_version=1
        ...
        AND NOT EXISTS(SELECT 1 FROM task_provider_eligibility_decisions n
          ...
          AND NOT (n.supersedes_decision_id=e.id
            AND n.decision_version=e.decision_version+1
            AND n.task_id=t.id AND n.scope_version_id=s.id
            AND n.routing_decision_id=r.id AND n.interest_application_id IS NOT NULL
            AND n.policy_version='universal-v1-post-estimate-1.2.0'))
      ...
      ORDER BY e.decision_version DESC
      LIMIT 1`,
      [taskId, actor]
    );
  ...
  async hold(actor: string, interestId: string): Promise<HoldContext | null> {
    const r = await this.query<HoldContext>(
      ...
      WHERE a.id=$1 AND t.poster_id=$2 AND t.state='OPEN'
        AND t.worker_id IS NULL AND t.work_order_id IS NULL
        AND t.universal_contract_version=1 AND t.automation_classification='CONTROLLED_TEST'
        AND a.status='pending' AND e.task_eligible AND e.valid_until>clock_timestamp()
        ...
        AND NOT EXISTS(SELECT 1 FROM task_provider_eligibility_decisions n WHERE n.task_draft_id=e.task_draft_id
          ...
          AND n.decision_version>e.decision_version) LIMIT 1`,
      [interestId, actor]
    );
  ...
  async workOrder(actor: string, holdId: string): Promise<WorkOrderContext | null> {
    const completed = await this.query<WorkOrderContext>(
      ...
      FROM task_work_orders w JOIN tasks t ON t.id=w.task_id AND t.poster_id=$2
        AND t.worker_id IS NULL AND t.work_order_id=w.id
    ...
    const r = await this.query<WorkOrderContext>(
      ...
      JOIN task_scope_versions s ON s.id=t.active_scope_version_id
      JOIN task_estimate_acceptance_materializations m ON m.task_id=t.id AND m.scope_version_id=s.id
      WHERE h.id=$1 AND t.poster_id=$2 AND t.state='OPEN'
        AND t.worker_id IS NULL AND t.work_order_id IS NULL
        AND t.universal_contract_version=1 AND t.automation_classification='CONTROLLED_TEST'
        AND h.status='ACTIVE' AND h.expires_at>clock_timestamp()
        AND e.task_eligible AND e.valid_until>clock_timestamp()
        AND NOT EXISTS(SELECT 1 FROM task_provider_eligibility_decisions n WHERE n.task_draft_id=e.task_draft_id
          AND n.provider_user_id=e.provider_user_id AND n.provider_organization_id IS NOT DISTINCT FROM e.provider_organization_id
          AND n.decision_version>e.decision_version) LIMIT 1`,
      [holdId, actor]
