        ...
        for (let i = 0; i < CONQ; i++) {
            attackPromises.push((async (index) => {
                ...
                try {
                    ...
                    const txWrapper = async (strings, ...values) => {
                        let text = strings[0];
                    ...
                    const result = await StripeMoneyEngine.handle(taskId, 'RELEASE_PAYOUT', context, { tx: txWrapper, disableRetries: true, stripeClient: mockStripe });
...
//# sourceMappingURL=race-release.js.map