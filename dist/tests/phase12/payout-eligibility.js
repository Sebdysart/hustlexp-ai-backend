 ...
 */
import { PayoutEligibilityResolver, PayoutDecision, BlockReason } from '../../services/PayoutEligibilityResolver.js';
...
// TEST UTILITIES
// ============================================================
async function createTestTask(status = 'in_progress') {
    const taskId = ulid();
...
async function createMoneyStateLock(taskId, state = 'held') {
    await sql `
...
async function createProofRequest(taskId) {
...
async function submitAndVerifyProof(taskId, requestId) {
    ...
    await ProofService.finalizeProof(submitResult.submissionId, 'verified', 'test_system', 'system');
    // Update task freeze state
    await ProofFreezeService.setFreezeState(taskId, 'PROOF_VERIFIED');
}
async function rejectProof(taskId, requestId) {
    ...
    // Reject the proof
    await ProofService.finalizeProof(submitResult.submissionId, 'rejected', 'test_system', 'system', 'Invalid proof - test rejection');
}
async function createDispute(taskId) {
...
// TESTS
// ============================================================
async function test1_ProofRequestedBlocksPayout() {
...
async function test2_ProofRejectedBlocksPayout() {
...
async function test3_ProofVerifiedAllowsPayout() {
...
async function test4_DisputeBlocksPayout() {
...
async function test5_AdminOverrideAllowsPayout() {
...
//# sourceMappingURL=payout-eligibility.js.map