...
async function main() {
    ...
    if (pass) {
        ...
        console.log('❌ A2 FAIL');
        if (!validFinalState)
            console.log('  - Invalid final state:', finalTask.status);
        if (!noEscrowLeak)
            console.log('  - Escrow leak detected');
        if (!stableEvents)
            console.log('  - Event count changed');
    }
    // Save artifact
    const artifact = {
...
//# sourceMappingURL=a2-cancel-accept-thrash.js.map