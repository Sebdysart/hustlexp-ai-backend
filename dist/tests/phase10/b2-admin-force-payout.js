...
async function main() {
    ...
    // 6. Verdict
    console.log('\n=== VERDICT ===');
    const hustlerPaid = balancesAfter.find((b) => b.owner_id === hustlerId)?.balance === '1000';
    const escrowZero = balancesAfter.find((b) => b.owner_id === taskId)?.balance === '0';
    ...
    if (pass) {
        ...
        console.log('❌ B2 FAIL');
        if (!hustlerPaid)
            console.log('  - Hustler not paid');
        if (!escrowZero)
            console.log('  - Escrow not zero');
        if (!exactlyOneTransfer)
            console.log('  - Transfer count:', thisTaskTransfers.length);
        if (!auditExists)
            console.log('  - Admin audit missing');
    }
    // Save artifact
    const artifact = {
...
//# sourceMappingURL=b2-admin-force-payout.js.map