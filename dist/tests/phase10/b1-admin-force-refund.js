...
async function main() {
    ...
    const posterRefunded = balancesAfter.find((b) => b.owner_id === posterId)?.balance === '50000';
    ...
    if (pass) {
        ...
        console.log('❌ B1 FAIL');
        if (!taskCancelled)
            console.log('  - Task not cancelled:', finalTask.status);
        if (!posterRefunded)
            console.log('  - Poster not refunded');
        if (!escrowZero)
...
//# sourceMappingURL=b1-admin-force-refund.js.map