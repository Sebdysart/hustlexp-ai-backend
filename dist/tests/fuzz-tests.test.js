...
import { TaskStateMachine, TASK_TRANSITIONS } from '../services/TaskStateMachine.js';
import { EscrowStateMachine, ESCROW_TRANSITIONS } from '../services/EscrowStateMachine.js';
...
describe('Fuzz Test 1: Random State Transitions', () => {
    it(`should maintain invariants across ${FUZZ_CONFIG.iterations} random transitions`, async () => {
        ...
        const allTaskStates = ['OPEN', 'ACCEPTED', 'PROOF_SUBMITTED', 'DISPUTED', 'COMPLETED', 'CANCELLED', 'EXPIRED'];
        const allEscrowStates = ['pending', 'funded', 'locked_dispute', 'released', 'refunded', 'partial_refund'];
...
describe('Fuzz Test 3: Concurrent Conflicts', () => {
    it('should handle concurrent state transitions safely', async () => {
        ...
        for (let i = 0; i < FUZZ_CONFIG.maxConcurrent; i++) {
            concurrentPromises.push(TaskStateMachine.transition(taskId, 'ACCEPTED', { hustlerId: testUserId }));
        }
    ...
    it('should handle concurrent XP awards (idempotency)', async () => {
        ...
        const concurrentPromises = [];
        for (let i = 0; i < FUZZ_CONFIG.maxConcurrent; i++) {
            concurrentPromises.push(awardXPForTask(taskId, testUserId));
        }
        const results = await Promise.all(concurrentPromises);
...
describe('Fuzz Test 5: State Machine Exhaustive', () => {
    ...
    it('should correctly identify all terminal states', () => {
        ...
        const terminalEscrowStates = ['released', 'refunded', 'partial_refund'];
...
//# sourceMappingURL=fuzz-tests.test.js.map