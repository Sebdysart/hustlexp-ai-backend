...
const mockStripe = {
    paymentIntents: {
        ...
        confirm: async (id) => ({ id, status: 'requires_capture', latest_charge: 'ch_mock_123' }),
        retrieve: async (id, opts) => ({ id, status: 'requires_capture', latest_charge: 'ch_mock_123' }),
        capture: async (id) => ({ id, status: 'succeeded' }),
        cancel: async (id) => ({ id, status: 'canceled' })
    },
    transfers: {
        create: async (payload) => ({ id: 'tr_mock_123' }),
        createReversal: async (id) => ({ id: 'trr_mock_123' })
    },
    refunds: {
        create: async (payload) => ({ id: 're_mock_123' })
    }
};
...
//# sourceMappingURL=invariant-break-split-brain.js.map