...
export class RiskScoreService {
     ...
     */
    static async scoreUser(userId, role) {
     ...
     * SCORE A TASK
     */
    static async scoreTask(context) {
        const evaluationId = ulid();
        const reasons = [];
        let baseScore = 0;
        // Category risk
    ...
    // -----------------------------------------------------------
    static async getUserHistory(userId, role) {
        const db = getDb();
    ...
    // -----------------------------------------------------------
    static calculateUserScore(history, role, evaluationId) {
        const reasons = [];
        let baseScore = 0;
        // New user penalty
    ...
    static calculateCombinedRisk(taskRisk, posterRisk, hustlerRisk) {
        const evaluationId = ulid();
    ...
    static buildScore(rawScore, reasons, evaluationId, confidence) {
    ...
    static calculateConfidence(history) {
        ...
        if (history.totalTasks < 3)
            return 0.5; // Limited history
        if (history.totalTasks < 10)
            return 0.7; // Some history
        if (history.totalTasks < 25)
            return 0.85; // Good history
        return 0.95; // Strong history
    }
    static generateRecommendation(combinedScore) {
        if (combinedScore < 15)
            return 'PROCEED_NORMAL';
        if (combinedScore < 30)
            return 'REQUIRE_PROOF';
        if (combinedScore < 50)
            return 'REQUIRE_ENHANCED_PROOF';
        if (combinedScore < 75)
    ...
    // -----------------------------------------------------------
    static async logScore(entityType, entityId, score) {
        const db = getDb();
        if (!db)
            return;
        try {
