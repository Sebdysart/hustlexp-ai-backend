...
const xpEventsFallback = [];
const questsFallback = new Map();
...
class GamificationServiceClass {
     ...
     */
    async awardXP(userId, amount, reason, taskId) {
        const event = {
            userId,
            amount,
            reason,
    ...
    async awardTaskCompletionXP(userId, taskId, rating) {
    ...
    async checkAndAwardStreakBonus(userId, streak) {
        let bonus = 0;
        if (streak === 3) {
     ...
     */
    async getUserXPEvents(userId, limit = 20) {
        if (isDatabaseAvailable() && sql) {
            try {
    ...
    async getActiveQuests(userId) {
        ...
        // Fallback to in-memory
        return questsFallback.get(userId)?.filter((q) => !q.isCompleted && q.expiresAt > new Date()) || [];
    }
    async generateQuestForUser(userId, userStats) {
        try {
