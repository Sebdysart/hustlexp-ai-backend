...
// ============================================
const profileDataStore = new Map();
// ============================================
// Profile Optimizer Service
// ============================================
class ProfileOptimizerServiceClass {
     ...
     */
    getProfileData(userId, profileType = 'hustler') {
     ...
     * Update profile data
     */
    updateProfileData(userId, updates) {
     ...
     */
    async generateBioSuggestion(userId, context) {
        const data = this.getProfileData(userId);
...
Make it catchy and Seattle-appropriate!`,
     ...
     */
    async generateHeadlineSuggestion(userId, context) {
        const data = this.getProfileData(userId);
        const skills = context?.skills || data.skills;
        try {
            const result = await routedGenerate('small_aux', {
...
- Current headline: "${context?.currentHeadline || 'none'}"`,
    ...
    getSkillRecommendations(userId) {
        ...
        const highDemandSkills = [
            { skill: 'IKEA Assembly', demand: 'high', avgPay: 55 },
            { skill: 'Pet Sitting', demand: 'high', avgPay: 45 },
            { skill: 'Deep Cleaning', demand: 'high', avgPay: 80 },
            { skill: 'Moving Help', demand: 'high', avgPay: 75 },
            { skill: 'Grocery Delivery', demand: 'medium', avgPay: 30 },
            { skill: 'Tech Support', demand: 'medium', avgPay: 50 },
            { skill: 'Yard Work', demand: 'medium', avgPay: 45 },
            { skill: 'Dog Walking', demand: 'high', avgPay: 25 },
        ];
        // Filter out skills user already has
        const recommended = highDemandSkills.filter(s => !data.skills.some(us => us.toLowerCase().includes(s.skill.toLowerCase()))).slice(0, 5);
        return {
            recommended,
     ...
     */
    predictEarningsImpact(userId, changes) {
        const data = this.getProfileData(userId);
    ...
    generateSuggestions(data, components) {
        const suggestions = [];
        // Photo suggestion
...
//# sourceMappingURL=ProfileOptimizerService.js.map