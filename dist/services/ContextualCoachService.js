...
const SCREEN_TIPS = {
    ...
    complete_task: (ctx) => {
        ...
        if (nextMilestone) {
            const bonuses = { 3: 25, 7: 75, 14: 150, 30: 500 };
            return {
...
class ContextualCoachServiceClass {
     ...
     */
    recordActivity(userId, earnings) {
     ...
     */
    buildContext(userId, screen, additionalContext) {
        const now = new Date();
     ...
     */
    getTipForScreen(userId, screen, additionalContext) {
     ...
     */
    getContextualTip(userId, additionalContext) {
     ...
     */
    getStreakTip(userId, currentStreak) {
        const stats = this.getUserDayStats(userId);
        // Streak at risk
