...
const proofFeed = new Map(); // hustlerId -> proofs
// ============================================
// AI Proof Service
// ============================================
class AIProofServiceClass {
     ...
     */
    startProofSession(taskId, hustlerId, category) {
     ...
     */
    async submitProof(sessionId, requirementId, photoUrl, caption) {
        const session = proofSessions.get(sessionId);
        if (!session)
        ...
        session.completedCount = session.proofs.filter(p => session.requirements.find(r => r.id === p.requirementId)?.required).length;
        session.progressPercent = Math.round((session.proofs.length / session.requirements.length) * 100);
     ...
     */
    calculateAnimations(session, requirement) {
        const animations = ['xp_burst', 'sparkle'];
        // First proof
     ...
     */
    calculateTrustPoints(requirement) {
        let points = 5;
        if (requirement.required)
            points += 5;
        if (requirement.type === 'before')
            points += 3;
     ...
     */
    getBadgeProgress(hustlerId, category) {
     ...
     */
    updateTrustProfile(hustlerId, proof, category) {
     ...
     * Add proof to feed
     */
    addToProofFeed(hustlerId, proof) {
...
//# sourceMappingURL=AIProofService.js.map