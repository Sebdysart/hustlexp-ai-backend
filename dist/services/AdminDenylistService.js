...
class AdminDenylistServiceClass {
     ...
     */
    async addToDenylist(uid, reason, addedBy, ttlSeconds = DEFAULT_TTL_SECONDS) {
        if (!redis) {
        ...
        try {
            ...
            await redis.setex(`${DENYLIST_PREFIX}${uid}`, ttlSeconds, JSON.stringify(entry));
     ...
     */
    async emergencyLock(uid, reason, lockedBy = 'SYSTEM') {
        if (!redis) {
        ...
        try {
            ...
            await redis.set(`${DENYLIST_PREFIX}${uid}`, JSON.stringify(entry));
            // Add to set
            await redis.sadd(DENYLIST_SET_KEY, uid);
     ...
     */
    async removeFromDenylist(uid, removedBy) {
        if (!redis) {
...
//# sourceMappingURL=AdminDenylistService.js.map