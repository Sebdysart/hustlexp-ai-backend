...
export class DisputeServiceClass {
    ...
    async createDispute(data) {
        ...
        try {
            ...
            `;
            if (hustler.length === 0)
                return { success: false, message: 'No hustler assigned to task' };
     ...
     */
    async addEvidence(disputeId, userUid, urls) {
        if (!sql)
            throw new Error('Database not initialized');
        try {
            ...
            if (dispute.status !== 'pending')
                return { success: false, message: 'Dispute is not pending' };
            if (dispute.poster_uid !== userUid)
                return { success: false, message: 'Only poster can add evidence' }; // Per spec
            await sql `
                UPDATE disputes 
     ...
     * Hustler Response
     */
    async submitResponse(disputeId, hustlerUid, message) {
        if (!sql)
            throw new Error('Database not initialized');
        try {
            const [dispute] = await sql `
            ...
            if (dispute.status !== 'pending')
                return { success: false, message: 'Dispute is not pending' };
            if (dispute.hustler_uid !== hustlerUid)
     ...
     */
    async resolveRefund(disputeId, adminId) {
        if (!sql)
            throw new Error('Database not initialized');
        try {
            ...
            if (!dispute)
                return { success: false, message: 'Dispute not found' };
            if (dispute.locked_at)
                return { success: false, message: 'Dispute already resolved/locked' };
     ...
     */
    async resolveUphold(disputeId, adminId) {
        if (!sql)
            throw new Error('Database not initialized');
        try {
            const [dispute] = await sql `
            ...
            if (!task.assignedHustlerId)
                return { success: false, message: 'No hustler assigned' };
     ...
     * Safety: Add Strike
     */
    async addStrike(userUid, reason, severity, source, meta) {
        if (!sql)
            return;
        try {
    ...
    async submitHustlerResponse(disputeId, hustlerId, message) {
        return this.submitResponse(disputeId, hustlerId, message);
    }
    async resolveDispute(disputeId, adminId, resolution, meta) {
