...
class FeatureFlagServiceClass {
    ...
    isEnabled(key, context) {
        ...
        if (context?.userId) {
            const userOverride = overrides.find(o => o.flagId === flag.id && o.userId === context.userId);
            if (userOverride) {
        ...
        // Check city override
        if (context?.cityId) {
            const cityOverride = overrides.find(o => o.flagId === flag.id && o.cityId === context.cityId && !o.userId);
            if (cityOverride) {
     ...
     */
    setFlag(key, enabledGlobal, description) {
     ...
     * Add a city override
     */
    setCityOverride(key, cityId, enabled) {
        const flag = flags.get(key);
        if (!flag)
            return null;
        // Remove existing city override
        const existingIdx = overrides.findIndex(o => o.flagId === flag.id && o.cityId === cityId && !o.userId);
        if (existingIdx !== -1) {
     ...
     * Add a user override
     */
    setUserOverride(key, userId, enabled) {
        const flag = flags.get(key);
        if (!flag)
            return null;
        // Remove existing user override
        const existingIdx = overrides.findIndex(o => o.flagId === flag.id && o.userId === userId);
        if (existingIdx !== -1) {
            overrides.splice(existingIdx, 1);
        }
        const override = {
     ...
     */
    removeCityOverride(key, cityId) {
        const flag = flags.get(key);
        if (!flag)
            return false;
        const idx = overrides.findIndex(o => o.flagId === flag.id && o.cityId === cityId && !o.userId);
        if (idx !== -1) {
     ...
     */
    removeUserOverride(key, userId) {
        const flag = flags.get(key);
        if (!flag)
            return false;
        const idx = overrides.findIndex(o => o.flagId === flag.id && o.userId === userId);
        if (idx !== -1) {
            overrides.splice(idx, 1);
            return true;
        }
