...
class VerificationServiceClass {
     ...
     */
    async initializeVerification(userId, email, phone) {
        if (!isDatabaseAvailable() || !sql)
            return;
        try {
            await sql `
    ...
    async sendSmsCode(userId, phone, ip) {
        ...
        try {
            ...
            if (twilioConfigured && smsAllowed) {
                ...
                `;
                serviceLogger.info({ userId, phone: normalizedPhone, code }, `📱 SMS VERIFICATION CODE (${currentEnv.toUpperCase()}): ${code}`);
            }
     ...
     */
    async canStartOnboarding(userId) {
        const status = await this.getStatus(userId);
        if (!status) {
