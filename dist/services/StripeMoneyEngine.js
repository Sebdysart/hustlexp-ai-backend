...
function getNextState(current, event) {
    switch (current) {
        case 'open':
            if (event === 'HOLD_ESCROW')
                return 'held';
            break;
        case 'held':
            if (event === 'RELEASE_PAYOUT')
                return 'released';
            if (event === 'REFUND_ESCROW')
                return 'refunded';
            if (event === 'DISPUTE_OPEN')
                return 'pending_dispute';
            break;
        case 'pending_dispute':
            if (event === 'RESOLVE_REFUND')
                return 'refunded';
            if (event === 'RESOLVE_UPHOLD')
                return 'upheld';
            break;
        case 'released':
            if (event === 'FORCE_REFUND')
                return 'refunded';
            break;
...
async function logOutbound(key, stripeId, type, payload) {
    await sql `
...
async function effectHoldEscrow(ctx, eventId, stripeClient) {
    const { amountCents, paymentMethodId, posterId, hustlerId, taskId } = ctx;
    if (!amountCents || !paymentMethodId)
        throw new Error("HoldEscrow: Missing amount or payment method");
    ...
    return { piId: confirmed.id, chargeId: chargeId };
}
async function effectReleasePayout(ctx, eventId, stripeClient, lock) {
    const { piId, chargeId } = lock;
    const { hustlerStripeAccountId, payoutAmountCents, taskId } = ctx;
    if (!piId)
        throw new Error("Missing PI ID");
    if (!chargeId)
        throw new Error("Missing charge_id");
    if (!hustlerStripeAccountId)
        throw new Error("ReleasePayout: No Hustler Stripe Account ID provided");
...
async function effectRefund(ctx, eventId, stripeClient, lock) {
    const { piId, chargeId, transferId, currentState } = lock;
    const { refundAmountCents, reason, taskId } = ctx;
    if (!piId)
        throw new Error("Missing PI ID for refund");
    if (currentState === 'held' || currentState === 'pending_dispute') {
        const cancelled = await stripeClient.paymentIntents.cancel(piId, { cancellation_reason: reason ?? 'requested_by_customer' }, { idempotencyKey: `${eventId}-cancel` });
        if (cancelled.status !== 'canceled')
            throw new Error(`RefundEscrow: Expected canceled but got ${cancelled.status}`);
        return { piId: piId };
    }
    if (!chargeId || !transferId)
        throw new Error("Cannot post-refund without charge_id and transfer_id");
    const reversal = await stripeClient.transfers.createReversal(transferId, { amount: refundAmountCents, metadata: { taskId, reason } }, { idempotencyKey: `${eventId}-reversal` });
...
async function executeStripeEffects(eventType, context, eventId, stripeClient, lockData) {
    ...
    if (mirror) {
        ...
        if (mirror.type === 'pi')
            return { piId: mirror.stripe_id, chargeId: mirror.payload?.chargeId };
        if (mirror.type === 'transfer')
            return { transferId: mirror.stripe_id };
        if (mirror.type === 'refund')
            return { refundId: mirror.stripe_id };
        // Fallback
...
async function validateGuards(tx, taskId, eventType, context, lock, eventId, adminOverride) {
    // 1. Invariant Checks
    if (!lock && eventType !== 'HOLD_ESCROW')
        throw new Error(`money_state_lock missing for task ${taskId}`);
    if (lock && !lock.next_allowed_event.includes(eventType))
    ...
    if (eventType === 'RELEASE_PAYOUT' || eventType === 'RESOLVE_UPHOLD') {
        ...
        if (eligibility.decision === PayoutDecision.BLOCK) {
            throw new Error(`PAYOUT BLOCKED [${eligibility.blockReason}]: ${eligibility.reason}`);
        }
        if (eligibility.decision === PayoutDecision.ESCALATE) {
            // Escalation without admin override = block
            if (!adminOverride?.enabled) {
                throw new Error(`PAYOUT REQUIRES ADMIN REVIEW [${eligibility.blockReason}]: ${eligibility.reason}`);
            }
...
// SAGA 3.0 ENGINE (ATOMIC HARDENING)
// ---------------------------------------------------------
export async function handle(taskId, eventType, context, options) {
    ...
    if (context.posterId)
        resourcesToLock.push(`user:${context.posterId}`);
    if (context.hustlerId)
        resourcesToLock.push(`user:${context.hustlerId}`);
    ...
    try {
        ...
        const prepResult = await transaction(async (tx) => {
            ...
            if (eventType === 'HOLD_ESCROW') {
                ...
                if (!context.hustlerId)
                    throw new Error("Missing hustlerId in context");
                const taskEscrow = await LedgerAccountService.getAccount(taskId, 'task_escrow', tx);
            ...
            // NEW: DISPUTE LOGIC (Phase 9C)
                ...
                if (!context.amountCents)
                    throw new Error("DISPUTE_OPEN requires amountCents");
                const lTx = await LedgerService.prepareTransaction({
        ...
        if (prepResult.type === 'ALREADY_DONE')
        ...
        const effects = await executeStripeEffects(eventType, context, eventId, stripeClient, preparationPayload);
        // -------------------------------------------------
...
//# sourceMappingURL=StripeMoneyEngine.js.map