...
export class LedgerService {
     ...
     */
    static async prepareTransaction(input, client // Transaction Client (Mandatory for Ring 2)
    ) {
        ...
        try {
            ...
            const accountsRaw = await client `
            ...
            const isMatch = input.entries.every(inputEntry => {
                return existingEntries.some((dbEntry) => dbEntry.account_id === inputEntry.account_id &&
     ...
     * Must be called within an active DB Transaction (`client`).
     */
    static async commitTransaction(txId, stripeMetadata, client // Transaction Client
    ) {
        ...
        try {
            const [updated] = await client `
             ...
             `;
            if (!updated) {
                const [check] = await client `SELECT status FROM ledger_transactions WHERE id = ${txId}`;
     ...
     */
    static async markFailed(txId, reason, client) {
        await client `
            UPDATE ledger_transactions 
            SET status = 'failed', 
     ...
     */
    static async setExecuting(txId, client) {
        await client `
            UPDATE ledger_transactions
