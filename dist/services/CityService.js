...
class CityServiceClass {
     ...
     */
    getZoneBySlug(cityId, zoneSlug) {
        return zones.get(`${cityId}:${zoneSlug}`);
    }
    /**
     * Get zone name from slug (for display)
     */
    getZoneName(zoneSlug, cityId = 'city_seattle') {
    ...
    getCoverageStats() {
        ...
        return {
            ...
            totalZones: Array.from(zones.values()).filter((z, i, arr) => arr.findIndex(x => x.id === z.id) === i).length,
