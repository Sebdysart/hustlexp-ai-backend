...
class UserBrainServiceClass {
     ...
     */
    async updateFromChat(userId, userMessage, aiResponse) {
    ...
    extractPreferencesFromMessage(message) {
        ...
        const categories = ['delivery', 'cleaning', 'moving', 'pet_care', 'errands', 'handyman', 'yard_work'];
     ...
     */
    calculateLearningScore(brain) {
        let score = 0;
        // Goals (20 points)
        if (brain.goals.monthlyIncomeTarget || brain.goals.weeklyIncomeTarget)
            score += 10;
        ...
        if (brain.constraints.availableTimes.length > 0)
            score += 10;
        if (brain.constraints.maxDistanceKm !== undefined)
            score += 5;
        if (brain.constraints.hasCar !== undefined)
            score += 5;
        ...
        if (brain.taskPreferences.avoidedCategories.length > 0)
            score += 10;
        if (brain.taskPreferences.prefersShortTasks || brain.taskPreferences.prefersIndoorTasks)
            score += 5;
        // Engagement (20 points)
        if (brain.engagementStyle.activeHours.length > 0)
            score += 10;
        if (brain.engagementStyle.avgTasksPerWeek > 0)
            score += 10;
        // Interactions (10 points)
        if (brain.totalInteractions >= 5)
            score += 5;
        if (brain.totalInteractions >= 20)
            score += 5;
     ...
     */
    updateFromAction(userId, action) {
        const brain = this.getUserBrain(userId);
        switch (action.type) {
            case 'accepted_task':
            case 'completed_task':
...
//# sourceMappingURL=UserBrainService.js.map