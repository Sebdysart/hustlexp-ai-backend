 ...
 */
function isHighRiskEmailDomain(email) {
    const domain = email.split('@')[1]?.toLowerCase();
 ...
 */
async function checkMultipleAccounts(userId, phone, deviceFingerprint) {
 ...
 */
async function checkVelocity(email, phone, ip) {
    if (!isDatabaseAvailable() || !sql) {
        return false;
    }
    try {
...
class FraudDetectionServiceClass {
     ...
     */
    async assessRisk(userId, email, phone, ip, deviceFingerprint) {
        const signals = {
     ...
     */
    async getIdentityContext(userId) {
        if (!isDatabaseAvailable() || !sql) {
            return null;
        }
        try {
            ...
            const verificationAge = verifiedAt
                ...
                : 0;
            // Get risk assessment
            const riskAssessment = await this.assessRisk(userId, row.email, row.phone, undefined, // IP not stored in users
     ...
     */
    async updateTrustScore(userId, adjustment, reason) {
        if (!isDatabaseAvailable() || !sql) {
            return 0;
        }
        try {
     ...
     */
    async recordFraudSignal(userId, signalType, metadata) {
        if (!isDatabaseAvailable() || !sql)
            return;
        try {
            await sql `
