...
const connectAccounts = new Map();
const processedEvents = new Set();
// ============================================
// Stripe Service Class
// ============================================
class StripeServiceClass {
    isAvailable() { return isStripeConfigured && stripe !== null; }
    // ============================================
    // Connect Accounts
    // ============================================
    async createConnectAccount(userId, email, metadata) {
        if (!stripe)
    ...
    getConnectAccountId(userId) { return connectAccounts.get(userId); }
    setConnectAccountId(userId, accountId) { connectAccounts.set(userId, accountId); }
    // ============================================
    // Escrow & Payment Intent
    // ============================================
    async createEscrowHold(taskId, posterId, hustlerId, amount, paymentMethodId) {
        if (!stripe || !sql)
            return null;
    ...
    async releaseEscrow(taskId, type = 'standard') {
        if (!stripe || !sql)
            return null;
    ...
    async refundEscrow(taskId, isAdmin = false) {
        // Delegate to Engine
        try {
    ...
    // ============================================
    async recoverHoldEscrow(pi, taskId) {
    ...
    async recoverReleaseEscrow(transfer, taskId) {
    ...
    async getPayoutHistory(hustlerId) {
        if (!sql) {
    ...
    async getPayout(payoutId) {
        ...
        try {
            const [p] = await sql `SELECT * FROM hustler_payouts WHERE id = ${payoutId}`;
            if (!p)
                return null;
            return {
                id: p.id,
