...
export class ProofDisputeService {
     ...
     */
    static async snapshotForDispute(disputeId, taskId) {
        const db = getDb();
        if (!db)
     ...
     */
    static async getDisputeSnapshot(disputeId) {
        const db = getDb();
        if (!db)
            return null;
        const [snapshot] = await db `
     ...
     */
    static async isLockedByDispute(taskId) {
        const db = getDb();
        if (!db)
            return false;
...
//# sourceMappingURL=ProofDisputeService.js.map