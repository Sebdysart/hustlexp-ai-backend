...
import { ProofReason, ProofState } from './types.js';
...
export class ProofRequestService {
     ...
     */
    static async aiRequestProof(params) {
        const db = getDb();
        if (!db)
        ...
        try {
            ...
            }, userContext, proofType, params.reason, existingProofCount);
     ...
     */
    static async autoRequestIfRequired(taskId) {
        const db = getDb();
        if (!db)
        ...
        try {
            ...
            // Create auto-request
            const proofType = ProofPolicy.getRecommendedProofType(task.category, ProofReason.TASK_COMPLETION);
            const instructions = ProofPolicy.generateInstructions(task.category, ProofReason.TASK_COMPLETION, proofType);
            const result = await ProofService.createRequest({
                taskId,
                proofType,
     ...
     */
    static async getPendingRequests(taskId) {
        const db = getDb();
        if (!db)
            return [];
...
//# sourceMappingURL=ProofRequestService.js.map