...
export class ImageForensicsService {
     ...
     */
    static async analyze(fileUrl, mimeType, metadata, taskTimeline) {
        const signals = [];
        const anomalies = [];
        ...
        const likelyScreenshot = signals.some(s => s.name === 'resolution_check' && s.suspicious) || (mimeType === 'image/png' && !metadata.exifPresent);
        const likelyAIGenerated = signals.some(s => s.name === 'ai_generation' && s.suspicious);
    ...
    static checkResolution(resolution) {
        const isScreenshotRes = SCREENSHOT_RESOLUTIONS.some(sr => (sr.w === resolution.width && sr.h === resolution.height) ||
    ...
    static checkTimestamp(captureTime, timeline) {
    ...
    static checkAIGeneration(metadata) {
        const exifSoftware = metadata.exifData?.Software || metadata.exifData?.software || '';
        const suspicious = AI_GENERATION_SIGNATURES.some(sig => exifSoftware.toLowerCase().includes(sig.toLowerCase()));
        return {
            name: 'ai_generation',
            value: exifSoftware,
            weight: 5,
