...
// Configuration
const POLICY_MODE = process.env.PROOF_POLICY_MODE || 'shadow';
// Hard limit guard
function assertNoMoneyPathImpact(action) {
...
export class AdaptiveProofPolicy {
     ...
     */
    static async getRequirements(taskId, category, price, posterId) {
     ...
     */
    static async evaluateShadowPolicy(taskId, category, price, posterId, hustlerId) {
        ...
        const recommendation = this.generateRecommendation(assessment.combinedRisk.tier, delta, price);
        const result = {
            taskId,
     ...
     */
    static async logOutcome(taskId, proofOutcome, disputeOutcome, payoutDelayHours) {
        const db = getDb();
        if (!db)
            return;
        try {
            ...
            const wouldHaveDiffered = this.outcomeWouldDiffer(enforcedPolicy, shadowPolicy, proofOutcome, disputeOutcome);
            // Calculate potential benefit
            const potentialBenefit = this.calculatePotentialBenefit(enforcedPolicy, shadowPolicy, proofOutcome, disputeOutcome, payoutDelayHours);
            await db `
     ...
     */
    static async getShadowAnalysis(days = 7) {
        const db = getDb();
        const defaultResult = {
            totalEvaluations: 0,
        ...
        try {
            ...
            `;
            // Build tier counts
            const byRiskTier = { minimal: 0, low: 0, medium: 0, high: 0, critical: 0 };
            for (const row of tierStats) {
                if (row.risk_tier in byRiskTier) {
                    byRiskTier[row.risk_tier] = parseInt(row.count);
                }
            }
    ...
    static explainDelta(enforced, shadow) {
        const details = [];
    ...
    static generateRecommendation(tier, delta, price) {
        if (delta === 'SAME') {
    ...
    static outcomeWouldDiffer(enforced, shadow, proofOutcome, disputeOutcome) {
    ...
    static calculatePotentialBenefit(enforced, shadow, proofOutcome, disputeOutcome, payoutDelayHours) {
    ...
    static async logShadowEvaluation(result) {
        const db = getDb();
        if (!db)
            return;
        try {
...
//# sourceMappingURL=AdaptiveProofPolicy.js.map