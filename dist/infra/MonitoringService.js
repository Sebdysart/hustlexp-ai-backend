...
class MonitoringServiceClass {
    requestMetrics = [];
    MAX_METRICS_AGE_MS = 5 * 60 * 1000; // 5 minutes
    MAX_METRICS_COUNT = 10000;
    /**
     * Record a request metric
     */
    recordRequest(endpoint, durationMs, success) {
        const metric = {
            timestamp: Date.now(),
            duration: durationMs,
            success,
     ...
     */
    async recordInvariantViolation(invariantId, details) {
        const sql = getSql();
        // Log to database
        await sql `
     ...
     */
    async recordPaymentFailure(taskId, error) {
        const sql = getSql();
        await sql `
    ...
    async runCheck() {
        ...
        if (!metrics.db_connection_healthy) {
            await AlertingService.critical('db_connection', 'Database Connection Failed', 'Cannot connect to database');
        }
        // Check webhook delivery
        if (metrics.webhook_delivery_rate < 95) {
            await AlertingService.warning('webhook_delivery', 'Webhook Delivery Degraded', `Webhook delivery rate is ${metrics.webhook_delivery_rate.toFixed(1)}%`);
        }
        logger.info({
