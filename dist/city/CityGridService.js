...
export class CityGridService {
     ...
     * GET ZONE DETAIL
     */
    static async getZoneDetail(city, zone) {
     ...
     */
    static async getCriticalCells(city) {
        const grid = await this.getGrid(city);
        return {
            supplyShortage: grid.cells.filter(c => c.supplyIndex < 30 && c.demandIndex > 50),
            highDispute: grid.cells.filter(c => c.disputeRate > 0.05),
            highChurn: grid.cells.filter(c => c.churnRisk > 60)
        };
    }
    /**
     * GET TIME PATTERN FOR ZONE
     */
    static async getTimePattern(city, zone) {
    ...
    // -----------------------------------------------------------
    static async generateGridCells(city) {
        const db = getDb();
        const cells = [];
    ...
    static buildCell(city, zone, microZone, zoneData) {
        // Distribute zone data across micro-zones with variance
        const variance = 0.2 + Math.random() * 0.6; // 20-80% of zone total
        const supplyIndex = Math.min(100, Math.round((zoneData.hustlerCount / Math.max(zoneData.taskCount / 4, 1)) * 50 * variance));
        const demandIndex = Math.min(100, Math.round((zoneData.taskCount / 10) * 20 * variance));
        const liquidityRatio = demandIndex > 0 ? supplyIndex / demandIndex : 1;
        // Churn risk based on completion rate and disputes
        const churnRisk = Math.min(100, Math.round((1 - zoneData.completionRate) * 50 + zoneData.disputeRate * 500));
        return {
            id: ulid(),
            city,
            zone,
    ...
    static generateHourlyPattern(type) {
        const pattern = [];
...
//# sourceMappingURL=CityGridService.js.map