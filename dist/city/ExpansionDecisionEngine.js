...
import { DefensibilityScoreService } from './DefensibilityScoreService.js';
...
export class ExpansionDecisionEngine {
     ...
     */
    static async getExpansionPlan(city) {
        const id = ulid();
        // Gather intelligence
        ...
        const decisions = defensibility.zones.map(zone => this.generateZoneDecision(zone, heat, grid));
        ...
        // Metrics
        const avgDef = Math.round(decisions.reduce((sum, d) => sum + d.signals.defensibility, 0) / decisions.length);
        const avgLiq = Math.round(decisions.reduce((sum, d) => sum + d.signals.liquidityHeat, 0) / decisions.length);
        const plan = {
            id,
            city,
     ...
     */
    static async getZoneDecision(zone) {
        const city = 'seattle';
        const [defensibility, heat, grid] = await Promise.all([
     ...
     */
    static async getPriorityActions(city) {
    ...
    // INTERNAL
    // -----------------------------------------------------------
    static generateZoneDecision(zone, heat, grid) {
        // Get zone heat
        const zoneHeat = heat?.cells.find(c => c.zone === zone.zone);
        const heatScore = zoneHeat?.heatScore || 50;
        // Get zone grid data
        const gridCells = grid.cells.filter((c) => c.zone === zone.zone);
        const avgSupply = gridCells.reduce((sum, c) => sum + c.supplyIndex, 0) / gridCells.length;
        const avgDemand = gridCells.reduce((sum, c) => sum + c.demandIndex, 0) / gridCells.length;
        const supplyDemandRatio = avgDemand > 0 ? avgSupply / avgDemand : 1;
        const churnRisk = gridCells.reduce((sum, c) => sum + c.churnRisk, 0) / gridCells.length;
        // Determine action
        const { action, confidence, reasoning } = this.decideAction(zone, heatScore, supplyDemandRatio, churnRisk);
        // Competition level
        const competitionLevel = zone.defensibilityScore > 60 ? 'low'
            : zone.defensibilityScore > 40 ? 'medium'
                : 'high';
        // Recommended actions
    ...
    static decideAction(zone, heat, supplyDemand, churn) {
        let action;
        let confidence = 50;
        let primaryFactor;
    ...
    static getRecommendedActions(action, zone, supplyDemand) {
        const actions = [];
        switch (action) {
            case 'expand':
    ...
    static getBudgetRecommendation(action, confidence) {
        if (action === 'retreat')
            return 'pause';
        if (action === 'expand' && confidence > 60)
            return 'increase';
        if (action === 'fortify')
            return 'maintain';
        return 'maintain';
    }
    static determineStrategy(decisions, defensibility) {
    ...
    static projectGrowth(summary) {
        ...
        if (expandRatio > 0.4)
            return '+15-25% MoM projected';
        if (expandRatio > 0.2)
            return '+5-15% MoM projected';
        if (summary.retreatZones.length > 2)
            return 'Flat to -5% MoM projected';
        return '+0-5% MoM projected';
    }
    static async persistPlan(plan) {
        const db = getDb();
        if (!db)
            return;
        try {
...
//# sourceMappingURL=ExpansionDecisionEngine.js.map