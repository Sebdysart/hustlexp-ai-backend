...
export default async function debugRoutes(fastify) {
    ...
    fastify.get('/verification-status', async (request, reply) => {
        ...
        if (userId) {
            ...
            if (status) {
                if (!status.emailVerified) {
                    diagnostics.troubleshooting.push('⚠️ Email not verified - user must complete email verification before phone');
                }
                if (status.emailVerified && !status.phoneVerified) {
                    diagnostics.troubleshooting.push('📱 Ready for phone verification - email is verified');
                }
                if (status.isLocked) {
                    diagnostics.troubleshooting.push('🔒 Account is locked due to too many failed attempts');
                }
                if (status.canProceedToOnboarding) {
                    diagnostics.troubleshooting.push('✅ All verification complete - user can proceed to AI onboarding');
                }
            }
        }
        else {
        ...
        if (!TwilioVerifyService.isConfigured()) {
            diagnostics.troubleshooting.push('⚠️ Twilio not configured - check TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_VERIFY_SERVICE_SID');
        }
        if (!canSendRealSms()) {
            diagnostics.troubleshooting.push(`ℹ️ Real SMS blocked by safety guard (environment: ${env.mode}) - codes will be logged to console`);
        }
        return diagnostics;
    });
}
