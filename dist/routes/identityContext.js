...
export default async function identityContextRoutes(fastify) {
    /**
     * GET /api/onboarding/identity-context/:userId
     *
     * Returns the full identity context for AI onboarding personalization.
     */
    fastify.get('/identity-context/:userId', async (request, reply) => {
     ...
     */
    fastify.get('/risk-assessment/:userId', async (request, reply) => {
        const { userId } = request.params;
        try {
     ...
     */
    fastify.post('/record-behavior', async (request, reply) => {
