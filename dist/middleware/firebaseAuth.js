...
export async function requireAuth(request, reply) {
    ...
    if (process.env.NODE_ENV === 'development' && process.env.ALLOW_DEV_AUTH_BYPASS === 'true') {
        ...
        const testRole = request.headers['x-test-role'] || 'poster';
        ...
        if (testRole === 'admin')
            uid = DEV_ADMIN_ID;
        const testDbUid = request.headers['x-test-db-uid'];
        request.user = {
            uid: uid,
    ...
    const dbUser = await UserService.getOrCreate(decodedToken.uid, decodedToken.email || `${decodedToken.uid}@hustlexp.com`, decodedToken.name);
    if (dbUser) {
 ...
 */
export async function requireAdminFromJWT(request, reply) {
    // DEVELOPMENT BYPASS: explicit guard required
    if (process.env.NODE_ENV === 'development' && process.env.ALLOW_DEV_AUTH_BYPASS === 'true') {
