...
class IdentityServiceClass {
     ...
     */
    async initializeIdentity(userId, email) {
        if (!sql)
            return;
        try {
            await sql `
     ...
     */
    async sendEmailCode(userId, email, ip) {
        ...
        if (!sql)
            return { success: false, error: 'Database unavailable', code: 'SERVICE_ERROR' };
        try {
            // Store attempt
            await sql `
     ...
     * Verify email code
     */
    async verifyEmailCode(userId, email, code, ip) {
        const normalizedEmail = normalizeEmail(email);
        if (!sql)
     ...
     */
    async sendSmsCode(userId, phone, ip) {
     ...
     * Verify SMS code
     */
    async verifySmsCode(userId, phone, code, ip) {
        const normalizedPhone = normalizePhone(phone);
        if (!sql)
            return { verified: false, error: 'Database unavailable' };
        // Rate limit
     ...
     * Log identity event
     */
    async logEvent(userId, eventType, channel, metadata, ip) {
        if (!sql)
            return;
        try {
            await sql `
