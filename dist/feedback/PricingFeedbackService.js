...
export class PricingFeedbackService {
     ...
     */
    static async getPosterAnalytics(posterId) {
        const db = getDb();
        const defaultResult = {
            totalTasks: 0,
        ...
        try {
            ...
            const breakdown = { underpriced: 0, optimal: 0, overpriced: 0 };
            let totalDelta = 0;
            ...
            for (const event of events) {
                ...
                if (event.verdict === 'underpriced') {
                    underpriced_total++;
                    if (event.outcome?.disputed)
                        underpriced_disputes++;
                }
            }
    ...
    // INTERNAL
    // -----------------------------------------------------------
    static calculateVerdict(actual, suggested, min, max) {
        const tolerance = 0.1; // 10% tolerance
        if (actual < suggested * (1 - tolerance))
            return 'underpriced';
        if (actual > suggested * (1 + tolerance * 2))
            return 'overpriced';
        return 'optimal';
    }
    static estimatePercentile(actual, min, max) {
        if (max === min)
            return 50;
        const percentile = ((actual - min) / (max - min)) * 100;
        return Math.max(0, Math.min(100, Math.round(percentile)));
    }
    static calculateOutcomeDelta(event) {
    ...
    static emitMetric(verdict, deltaPercent) {
...
//# sourceMappingURL=PricingFeedbackService.js.map