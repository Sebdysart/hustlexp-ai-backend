 ...
 */
export function getContextAwarePrompt(context) {
    const sections = [];
    // Core identity
    ...
    if (!context.constraints.hasCar)
        constraintNotes.push('No car');
    if (!context.constraints.petFriendly)
        constraintNotes.push('No pets');
    if (!context.constraints.canDoHeavyLifting)
        constraintNotes.push('No heavy lifting');
    if (context.constraints.maxDistanceKm)
        constraintNotes.push(`Max ${context.constraints.maxDistanceKm}km`);
    ...
    if (context.taskPreferences.preferredCategories.length > 0 || context.taskPreferences.avoidedCategories.length > 0) {
        ...
        if (context.taskPreferences.prefersShortTasks)
            sections.push('- Prefers quick tasks');
        if (context.taskPreferences.prefersIndoorTasks)
            sections.push('- Prefers indoor work');
        sections.push('');
    }
    // Where they are now
 ...
 */
function getScreenGuidance(screen, profile) {
    switch (screen) {
        case 'home':
...
//# sourceMappingURL=contextAwarePrompt.js.map