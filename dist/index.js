...
import { ContextualCoachService } from './services/ContextualCoachService.js';
...
fastify.get('/api/control-plane/risk/user/:userId', async (request, reply) => {
...
// Score a task
fastify.get('/api/control-plane/risk/task/:taskId', async (request, reply) => {
...
fastify.get('/api/control-plane/risk/assess/:taskId', async (request, reply) => {
    const { taskId } = request.params;
    const task = await TaskService.getTask(taskId);
    if (!task) {
        reply.code(404);
...
fastify.get('/api/control-plane/shadow/evaluate/:taskId', async (request, reply) => {
    const { taskId } = request.params;
    const task = await TaskService.getTask(taskId);
    if (!task) {
        reply.code(404);
    ...
    const comparison = await AdaptiveProofPolicy.evaluateShadowPolicy(taskId, task.category, task.recommendedPrice, task.clientId, task.assignedHustlerId || undefined);
    return {
        success: true,
        taskId,
        comparison
...
fastify.get('/api/control-plane/shadow/analysis', async (request) => {
...
import { AIRecommendationService } from './control-plane/AIRecommendationService.js';
// List snapshots
fastify.get('/api/control-plane/snapshots', async (request) => {
    const { type, limit } = request.query;
    const snapshots = await AnalysisSnapshotService.listSnapshots(type, limit || 50);
    return { success: true, snapshots };
});
// Get snapshot by ID
fastify.get('/api/control-plane/snapshots/:id', async (request, reply) => {
...
fastify.get('/api/control-plane/snapshots/:id/export', async (request, reply) => {
...
fastify.post('/api/control-plane/snapshots/generate', async (request) => {
    const type = request.body.type || 'manual';
    const snapshot = await AnalysisSnapshotService.generateSnapshot(type);
    return { success: true, snapshot };
});
// Get latest snapshot
fastify.get('/api/control-plane/snapshots/latest', async (request) => {
    const snapshot = await AnalysisSnapshotService.getLatest(request.query.type);
    return { success: true, snapshot };
});
// List recommendations
fastify.get('/api/control-plane/recommendations', async (request) => {
...
fastify.get('/api/control-plane/recommendations/:id', async (request, reply) => {
...
fastify.post('/api/control-plane/recommendations/ingest', async (request) => {
...
fastify.post('/api/control-plane/recommendations/:id/action', async (request, reply) => {
...
fastify.post('/api/control-plane/simulate/:recommendationId', async (request, reply) => {
...
fastify.get('/api/control-plane/simulations/:id', async (request, reply) => {
...
fastify.get('/api/control-plane/recommendations/:recommendationId/simulations', async (request, reply) => {
...
fastify.get('/api/control-plane/recommendations/:recommendationId/should-accept', async (request, reply) => {
...
// Get category health
fastify.get('/api/market/categories/:category', async (request, reply) => {
...
// Get zone health
fastify.get('/api/market/zones/:zone', async (request, reply) => {
...
fastify.get('/api/market/pricing/:category', async (request) => {
    const guidance = await MarketSignalEngine.getPricingGuidance(request.params.category, request.query.zone);
    return { success: true, guidance };
});
// Detect churn risk
fastify.get('/api/market/churn-risk', async (request) => {
...
fastify.get('/api/strategy/pricing-guidance/:category', async (request) => {
    const guidance = await StrategicOutputEngine.getPricingGuidance(request.params.category, request.query.zone);
    return { success: true, guidance };
});
// 2. Hustler Opportunity Routing
fastify.get('/api/strategy/hustler-opportunities/:userId', async (request) => {
    const opportunities = await StrategicOutputEngine.getHustlerOpportunities(request.params.userId, request.query.zone || 'Capitol Hill');
    return { success: true, opportunities };
});
// 3. Trust Friction Recommendations (UX-only, cannot block payouts)
fastify.post('/api/strategy/trust-friction', async (request) => {
    const { taskId, category, price, posterId, hustlerId } = request.body;
    const friction = await StrategicOutputEngine.getTrustFriction(taskId, category, price, posterId, hustlerId);
...
fastify.get('/api/feedback/pricing/:taskId', async (request) => {
    const feedback = await PricingFeedbackService.getFeedback(request.params.taskId);
    return { success: true, feedback };
});
fastify.get('/api/feedback/pricing/poster/:posterId', async (request) => {
...
fastify.get('/api/feedback/performance/:userId', async (request) => {
    const days = request.query.days || 30;
    const summary = await PerformanceFeedbackService.getSummary(request.params.userId, days);
    return { success: true, summary };
});
fastify.get('/api/feedback/performance/:userId/recent', async (request) => {
...
fastify.get('/api/city/grid/:city', async (request) => {
    const grid = await CityGridService.getGrid(request.params.city);
    return { success: true, grid };
});
fastify.get('/api/city/grid/:city/:zone', async (request) => {
...
fastify.get('/api/city/heat/:city', async (request) => {
    const snapshot = await LiquidityHeatEngine.generateSnapshot(request.params.city);
    return { success: true, snapshot };
});
fastify.get('/api/city/heat/:city/critical', async (request) => {
...
fastify.get('/api/city/opportunities/:userId', async (request) => {
...
fastify.post('/api/city/opportunities/:city/generate', async (request) => {
...
fastify.get('/api/city/defensibility/:city', async (request) => {
...
fastify.get('/api/city/defensibility/:city/threats', async (request) => {
...
fastify.get('/api/city/expansion/:city', async (request) => {
    const plan = await ExpansionDecisionEngine.getExpansionPlan(request.params.city);
    return { success: true, plan };
});
fastify.get('/api/city/expansion/:city/priorities', async (request) => {
...
fastify.get('/api/dominance/reputation/:zone', async (request) => {
    const metrics = await ReputationCompoundingService.getZoneMetrics(request.params.zone);
    return { success: true, metrics };
});
fastify.get('/api/dominance/reputation/user/:userId', async (request) => {
...
fastify.get('/api/dominance/exit-friction/:zone', async (request) => {
    const analysis = await ExitFrictionAnalyzer.analyzeZone(request.params.zone);
    return { success: true, analysis };
});
fastify.get('/api/dominance/exit-friction/user/:userId', async (request) => {
...
fastify.get('/api/dominance/takeover/:zone', async (request) => {
    const state = await ZoneTakeoverEngine.getZoneState(request.params.zone);
    return { success: true, state };
});
fastify.get('/api/dominance/takeover/city/:city', async (request) => {
...
fastify.post('/ai/confirm-task', async (request, reply) => {
    try {
        ...
        const taskDraft = body.taskDraft;
        // ============================================
        // TPEE GATE: Trust & Pricing Enforcement
        // ============================================
        const tpeeInput = TPEEService.taskDraftToTPEEInput(taskDraft, dbUser.id, 'city_seattle' // Seattle beta
        );
        ...
        if (tpeeResult.decision === 'ACCEPT') {
            try {
                ...
                const escalation = await TPEEAIEscalation.escalate(tpeeResult, tpeeInput, taskDraft.recommendedPrice, // median price fallback
        ...
        // END AI ESCALATION
        // ============================================
        const task = await TaskService.createTaskFromDraft(dbUser.id, taskDraft);
        // ============================================
        ...
        try {
            ...
            policyAssignment = await PolicySnapshotService.assignPolicyToTask(task.id, { city_id: 'city_seattle', category: taskDraft.category });
        }
        catch (policyErr) {
...
fastify.post('/ai/onboarding/answer', async (request, reply) => {
    try {
        ...
        const result = await OnboardingService.answerQuestion(body.sessionId, body.questionKey, body.answer, body.skip);
        return result;
    }
    catch (error) {
...
fastify.post('/api/onboarding/:userId/role', { preHandler: [requireAuth] }, async (request, reply) => {
    try {
        ...
        const result = await OnboardingService.chooseRole(body.sessionId, normalizedRole);
        return result;
    }
    catch (error) {
...
fastify.post('/api/planner/generate', async (request, reply) => {
    try {
        ...
        const plan = await HustlerTaskPlanner.generatePlan(body.taskId, body.hustlerId, body.task, body.boostMultiplier);
        return plan;
    }
    catch (error) {
...
fastify.post('/api/planner/objective', async (request, reply) => {
    try {
        ...
        const result = await HustlerTaskPlanner.updateObjective(body.planId, body.objectiveId, body.status, body.photoUrl, body.notes);
        return result;
    }
    catch (error) {
...
fastify.get('/api/planner/hustler/:hustlerId', async (request) => {
    const { hustlerId } = request.params;
...
fastify.post('/api/proof/submit', async (request, reply) => {
    try {
        ...
        const result = await AIProofService.submitProof(body.sessionId, body.requirementId, body.photoUrl, body.caption);
        return result;
    }
    catch (error) {
...
fastify.post('/api/proof/validated/:taskId/reject', async (request, reply) => {
    try {
        ...
        const result = await ProofValidationService.rejectTask(taskId, body.posterId, body.reason, body.action);
        if (!result.success) {
            reply.status(400);
            return { error: result.message };
        }
...
fastify.get('/api/pricing/calculate/:price', async (request) => {
    ...
    const boostTier = (query.boost || 'normal');
...
fastify.get('/api/tips/:userId/screen/:screen', async (request) => {
    const { userId, screen } = request.params;
    const tip = ContextualCoachService.getTipForScreen(userId, screen);
...
fastify.get('/api/tips/:userId/streak', async (request) => {
    const { userId } = request.params;
    const { currentStreak } = request.query;
...
fastify.post('/api/cards/:userId/generate', async (request, reply) => {
    try {
        ...
        const cardData = {
            ...body.data,
            taskCategory: body.data.taskCategory,
        };
...
fastify.get('/api/cards/:cardId/share/:platform', async (request) => {
    const { cardId, platform } = request.params;
    const card = SocialCardGenerator.getCard(cardId);
    if (!card) {
    ...
    const shareText = SocialCardGenerator.getShareTextForPlatform(card, platform);
...
fastify.get('/api/proof/requirements/:category', async (request) => {
    ...
    const requirements = EnhancedAIProofService.getRequirements(category);
...
fastify.get('/api/cost/:userId/check/:provider', async (request) => {
    const { userId, provider } = request.params;
    const result = AICostGuardService.checkLimit(userId, provider);
    return result;
});
...
fastify.post('/api/match/:taskId/rerank', async (request, reply) => {
    try {
        const { taskId } = request.params;
        const { task, candidates, limit } = request.body;
...
fastify.post('/api/match/quick-score', async (request) => {
    const { task, candidate } = request.body;
...
fastify.post('/api/stripe/connect/create', { preHandler: [requireRole('hustler')] }, async (request, reply) => {
    try {
        ...
        const result = await StripeService.createConnectAccount(body.userId, body.email, { name: body.name, phone: body.phone });
        if (!result.success) {
            reply.status(400);
...
// ============================================
function packHoldEscrowContext(body, posterId) {
    return {
...
function packReleaseContext(task, hustlerStripeAccountId) {
    return {
        eventId: crypto.randomUUID(),
...
function packRefundContext(task, amount, reason) {
    return {
        eventId: crypto.randomUUID(),
...
fastify.post('/api/stripe/webhook', {
    ...
    const event = StripeService.verifyWebhook(request.rawBody, signature);
    if (!event) {
        reply.status(400);
...
fastify.get('/api/admin/disputes', { preHandler: [requireAdminFromJWT] }, async (request) => {
    const { status, posterId, hustlerId, limit } = request.query;
...
fastify.post('/api/admin/disputes/:disputeId/resolve', { preHandler: [requireAdminFromJWT] }, async (request, reply) => {
    try {
        ...
        const result = await DisputeService.resolveDispute(disputeId, adminId, body.resolution, {
...
fastify.get('/api/admin/moderation/logs', { preHandler: [requireAdminFromJWT] }, async (request) => {
    const { userId, taskId, type, severity, limit } = request.query;
...
fastify.post('/api/admin/user/:userId/strikes', { preHandler: [requireAdminFromJWT] }, async (request, reply) => {
    try {
        ...
        const strike = await DisputeService.addStrike(userId, body.reason, body.severity, 'manual', { taskId: body.taskId });
        const suspension = await DisputeService.isUserSuspended(userId);
        return {
            strike,
            suspension,
...
fastify.get('/api/admin/events', { preHandler: [requireAdminFromJWT] }, async (request) => {
    const { eventType, userId, taskId, source, limit } = request.query;
    const range = parseDateRange(request.query);
...
fastify.get('/api/admin/metrics/daily', { preHandler: [requireAdminFromJWT] }, async (request) => {
    const { cityId, limit } = request.query;
...
fastify.get('/api/flags/:key', async (request) => {
    ...
    const { cityId, userId } = request.query;
...
fastify.post('/api/admin/flags/:key/city-override', { preHandler: [requireAdminFromJWT] }, async (request, reply) => {
    const { key } = request.params;
    const { cityId, enabled } = request.body;
    if (!cityId) {
        reply.status(400);
...
fastify.post('/api/beta/validate-invite', async (request) => {
    const { code, role, cityId } = request.body;
    if (!code || !role) {
...
fastify.post('/api/beta/check-signup', async (request) => {
    const { role, cityId, inviteCode } = request.body;
...
fastify.post('/api/admin/beta/invites', { preHandler: [requireAdminFromJWT] }, async (request) => {
    const { code, role, cityId, maxUses, expiresAt } = request.body;
    if (!code || !role) {
