...
import { ControlGroupSelector } from './ControlGroupSelector.js';
...
export class CausalImpactAnalyzer {
    ...
    static async analyze(correctionId) {
        ...
        try {
            ...
            const controlResult = await ControlGroupSelector.selectControlGroup(correctionId, treatedZone, category, appliedAt);
            if (!controlResult.found || !controlResult.controlGroup) {
                // No control group - result is inconclusive
                const inconclusiveAnalysis = await this.storeInconclusiveResult(db, correctionId, 'NO_CONTROL_GROUP');
                return inconclusiveAnalysis;
            }
            const controlGroup = controlResult.controlGroup;
            // 4. Compute metrics for treated group
            const treatedBaseline = await this.gatherMetrics(db, treatedZone, category, new Date(appliedAt.getTime() - this.WINDOW_HOURS * 60 * 60 * 1000), appliedAt);
            const treatedPost = await this.gatherMetrics(db, treatedZone, category, appliedAt, minAnalysisTime);
            const treatedDelta = this.computeDelta(treatedBaseline, treatedPost);
            // 5. Compute metrics for control group
            const controlBaseline = await this.gatherMetrics(db, controlGroup.zone, category, new Date(appliedAt.getTime() - this.WINDOW_HOURS * 60 * 60 * 1000), appliedAt);
            const controlPost = await this.gatherMetrics(db, controlGroup.zone, category, appliedAt, minAnalysisTime);
     ...
     * GATHER METRICS
     */
    static async gatherMetrics(db, zone, category, windowStart, windowEnd) {
        const defaults = {
            taskFillRate: 0,
            completionRate: 0,
            disputeRate: 0,
     ...
     * COMPUTE DELTA
     */
    static computeDelta(baseline, post) {
        return {
     ...
     * net_lift = delta_treated − delta_control
     */
    static computeNetLift(treatedDelta, controlDelta) {
        return {
     ...
     */
    static determineVerdict(netLift, treatedDelta, controlDelta) {
        // Count positive net lifts on core metrics
        let positiveLifts = 0;
        if (netLift.taskFillRate > 0.02)
            positiveLifts++;
        if (netLift.completionRate > 0.02)
            positiveLifts++;
        if (netLift.disputeRate < -0.01)
            positiveLifts++; // Lower disputes = positive
        if (netLift.hustlerEngagement > 0.02)
            positiveLifts++;
        // Check if control group improved more than treated
        const controlImprovedMore = (controlDelta.taskFillRate || 0) >= (treatedDelta.taskFillRate || 0) &&
     ...
     */
    static calculateConfidence(treatedBaseline, controlBaseline, matchQuality) {
        // Base confidence on data availability + match quality
        let dataScore = 0;
        if (treatedBaseline.taskFillRate > 0)
            dataScore += 0.2;
        if (controlBaseline.taskFillRate > 0)
            dataScore += 0.2;
        if (treatedBaseline.completionRate > 0)
            dataScore += 0.1;
        if (controlBaseline.completionRate > 0)
            dataScore += 0.1;
     ...
     */
    static async storeInconclusiveResult(db, correctionId, reason) {
        const emptyMetrics = {
     ...
     * STORE ANALYSIS
     */
    static async storeAnalysis(db, analysis) {
        try {
            await db `
                INSERT INTO causal_outcomes (
                    id, correction_id, treated_metrics, control_metrics,
     ...
     * GET VERDICT RATES
     */
    static async getVerdictRates(windowHours = 24) {
        const db = getDb();
        if (!db) {
     ...
     */
    static async checkSafeModeThreshold() {
        ...
        if (rates.total >= 5 && rates.nonCausalRate > NON_CAUSAL_RATE_THRESHOLD) {
            ...
            await CorrectionEngine.enterSafeMode(`Non-causal rate ${(rates.nonCausalRate * 100).toFixed(1)}% exceeds ${NON_CAUSAL_RATE_THRESHOLD * 100}% threshold`);
            await AlertService.fire('STUCK_SAGA', `Corrections not causing improvements: ${(rates.nonCausalRate * 100).toFixed(1)}% non-causal in last 24h`, { rates });
            return true;
        }
        return false;
    }
}
//# sourceMappingURL=CausalImpactAnalyzer.js.map