...
export class ControlGroupSelector {
     ...
     */
    static async selectControlGroup(correctionId, treatedZone, treatedCategory, correctionAppliedAt) {
        const db = getDb();
        if (!db) {
        ...
        try {
            ...
            const candidateZones = await this.findCandidateZones(db, correctionId, treatedZone, treatedCategory, correctionAppliedAt);
            ...
            const treatedBaseline = await this.getBaselineMetrics(db, treatedZone, treatedCategory, correctionAppliedAt);
            // 3. Find best matching control zone
            let bestMatch = null;
            let bestMatchQuality = 0;
            for (const candidateZone of candidateZones) {
                const candidateBaseline = await this.getBaselineMetrics(db, candidateZone, treatedCategory, correctionAppliedAt);
     ...
     */
    static async findCandidateZones(db, correctionId, treatedZone, category, correctionAppliedAt) {
     ...
     */
    static async getBaselineMetrics(db, zone, category, referenceTime) {
     ...
     */
    static computeBaselineDelta(treated, control) {
        const diffs = [
...
//# sourceMappingURL=ControlGroupSelector.js.map