...
export class ReputationCompoundingService {
     ...
     */
    static async getCompoundingLeaders(zone) {
    ...
    // INTERNAL
    // -----------------------------------------------------------
    static async calculateAvgCompoundingRate(zone) {
        const db = getDb();
        if (!db)
            return 2.5;
        try {
    ...
    static async getCompoundingFactors(zone) {
        return {
    ...
    static async getUserCompoundingRate(userId) {
    ...
    static async calculateUserPortabilityPenalty(userId, trustScore, percentile) {
    ...
    static projectFuture(currentScore, rate) {
        const projected30Days = Math.min(100, currentScore + rate * 10);
        let nextTierIn = 'Already at highest tier';
        if (currentScore < 35)
            nextTierIn = `~${Math.ceil((35 - currentScore) / rate)} tasks`;
        else if (currentScore < 55)
            nextTierIn = `~${Math.ceil((55 - currentScore) / rate)} tasks`;
        else if (currentScore < 75)
            nextTierIn = `~${Math.ceil((75 - currentScore) / rate)} tasks`;
        else if (currentScore < 90)
            nextTierIn = `~${Math.ceil((90 - currentScore) / rate)} tasks`;
    ...
    static assessCompetitiveAdvantage(rate, velocity) {
    ...
    static assessLockInImplication(penalty) {
        if (penalty > 60) {
...
//# sourceMappingURL=ReputationCompoundingService.js.map