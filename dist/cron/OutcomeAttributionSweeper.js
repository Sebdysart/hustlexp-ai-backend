...
import { CorrectionOutcomeAnalyzer } from '../engine/CorrectionOutcomeAnalyzer.js';
...
export class OutcomeAttributionSweeper {
    ...
    static async run() {
        ...
        try {
            ...
            for (const correction of corrections) {
                ...
                if (analysis) {
                    if (analysis.netEffect === 'positive')
                        positive++;
                    else if (analysis.netEffect === 'neutral')
                        neutral++;
                    else
                        negative++;
                }
            ...
            if (analyzed >= 5) { // Minimum sample size
                ...
                if (rates.negativeRate > NEGATIVE_RATE_THRESHOLD) {
                    ...
                    await CorrectionEngine.enterSafeMode(`Negative outcome rate ${(rates.negativeRate * 100).toFixed(1)}% exceeds ${NEGATIVE_RATE_THRESHOLD * 100}% threshold`);
                    await AlertService.fire('STUCK_SAGA', `Corrections causing harm: ${(rates.negativeRate * 100).toFixed(1)}% negative outcomes in last 24h`, {
                        negativeRate: rates.negativeRate,
     ...
     * - Limit 50 per sweep
     */
    static async findCorrectionsToAnalyze(db) {
...
//# sourceMappingURL=OutcomeAttributionSweeper.js.map