...
export class EscrowTimeoutSweeper {
     ...
     * FIND STUCK ESCROWS
     */
    static async findStuckEscrows(db) {
        try {
    ...
    static async resolveEscrow(db, escrow, stripeClient) {
        ...
        if (canRelease) {
            ...
            if (success) {
                ...
                await AlertService.fire('ESCROW_TIMEOUT_ACTION', `Escrow ${escrowId} auto-released to hustler after ${this.TIMEOUT_HOURS}h`, { escrowId, taskId, action: 'released', amount: grossAmountCents });
            ...
            if (success) {
                ...
                await AlertService.fire('ESCROW_TIMEOUT_ACTION', `Escrow ${escrowId} auto-refunded to poster after ${this.TIMEOUT_HOURS}h`, {
                    escrowId,
                    taskId,
                    action: 'refunded',
     ...
     * GET TASK STATE
     */
    static async getTaskState(db, taskId) {
        try {
            // Get task status
            const [task] = await db `
     ...
     * GET REFUND REASON
     */
    static getRefundReason(taskState) {
     ...
     */
    static async executeRelease(db, escrow) {
     ...
     * EXECUTE REFUND (to poster)
     */
    static async executeRefund(db, escrow, stripeClient) {
        try {
     ...
     * NOTIFY USERS
     */
    static async notifyUsers(db, escrow, action) {
        try {
...
//# sourceMappingURL=EscrowTimeoutSweeper.js.map