...
async function resolveIds() {
    ...
    if (!poster || !hustler)
        throw new Error('Test users not found in DB. Run gate2-prep.ts first.');
...
async function financialDestructionSuite() {
    ...
    const killServer = () => { if (!serverProcess.killed)
        serverProcess.kill(); };
    ...
    try {
        ...
        if (escrowRes.ok) {
            ...
            passed += (await runTest('Double Payout Race Condition', async () => {
                ...
                const requests = Array(5).fill(0).map((_, i) => fetch(`${BASE_URL}/api/tasks/${taskId}/approve`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'x-test-role': 'poster',
        ...
        passed += (await runTest('Webhook Replay Attack (Crash Check)', async () => {
            ...
            // Check viability
            if (res1.reason)
                console.log(`Webhook 1 Error: ${res1.reason}`);
            if (res2.reason)
                console.log(`Webhook 2 Error: ${res2.reason}`);
...
//# sourceMappingURL=gate2-financial.js.map